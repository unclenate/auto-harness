...
**Last Updated:** 2026-06-05 *(Install prerequisites: appended the observation that declared dependencies surfaced only in reference sections — not at the point of first contact — and preflighted asymmetrically fail the same way on every platform, surfaced only when the operator hits them mid-install. Satisfies the PRD-0004 distillation rule fired by the new OPP-0040 (cross-platform install prerequisites). Prior: 2026-06-04 AEC wedge Phase 2 — second-domain-instance observation (compound forcing artifact + first domain × cross-cutting composition); the adopter-artifact/host-IP attribution-boundary observation behind OPP-0038; and two consumer-adoption observations from the fork-held-consumer pin-bump session.)*
