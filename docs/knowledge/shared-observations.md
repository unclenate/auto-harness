...
**Last Updated:** 2026-06-02 *(Consumer-adoption distillation: appended two observations surfaced while bumping an external fork-held consumer's `.harness` submodule pin — (1) a submodule pin bump silently widens the recommend-vs-adopt gap between the CI template and the consumer's invocation list (governance-relevant; OPP candidate), and (2) forked consumers hit a wrong-base default in `gh pr create` (informational; onboarding note). First observations sourced from consumer-side adoption rather than harness-self development, mirroring the brownfield-onboarding entry at the top. Prior update this date: healthcare wedge Tasks 3–4 — jurisdiction-neutral-core observation.)*
