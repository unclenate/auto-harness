...
* [OPP-0054: Status-Parity Validator (OPP record status vs. derived index surfaces)](docs/opportunities/OPP-0054-status-parity-validator.md) — Always-on BLOCK `validate-status-parity.sh` reconciling each record's `Status` against its `candidates.md` and `docs/README.md` mirrors; shipped via PRD-0036 (accepted)
* [OPP-0055: SUMMARY Nav-List Completeness](docs/opportunities/OPP-0055-summary-nav-list-completeness.md) — Extends `validate-list-completeness.sh` to gate ADR/PRD/OPP record presence in the SUMMARY nav (the § 3 "mechanize" arm; path-anchored, BLOCK); OPP-direct (accepted)
* [OPP-0056: Agent Client Protocol (ACP) Governance Bridge](docs/opportunities/OPP-0056-agent-client-protocol-governance-bridge.md) — A 9th agent-adapter (`agents/acp`) mapping trust tiers + companion rules onto ACP's `request_permission` seam; runtime enforcement + ecosystem inducement to the JetBrains/ACP community; strategic, PRD recommended (accepted)
* [OPP-0057: ACP Audit → Knowledge-Capture Bridge](docs/opportunities/OPP-0057-acp-audit-knowledge-capture-bridge.md) — Session-boundary distillation of the ACP proxy's runtime audit into ADR-0002 observations (governance-relevant subset, draft-then-ratify, mechanical tier→Severity map); closes the runtime→policy loop; `agents/acp` follow-on (proposed)
* [OPP-0058: Out-of-Band Memory Consolidation (Dreaming)](docs/opportunities/OPP-0058-out-of-band-memory-consolidation-dreaming.md) — Governed, propose-only batch consolidation of cross-session transcripts into an evidence-backed proposed knowledge-tree diff a human promotes; the out-of-band complement to in-band PRD-0004 distillation (proposed)
* [OPP-0059: Live Inter-Agent Control-Loop & Cross-Vendor Bus](docs/opportunities/OPP-0059-inter-agent-control-loop-cross-vendor-bus.md) — Contracts the live coordination channel OPP-0046 left ungoverned: vendor-neutral control semantics (dispatch/ack/progress/done/block/sync/verdict, `tier_ceiling` caps-never-grants) split from a file inbox/outbox transport seam; native-bus reference adapter + loop machinery; one file-poll shape reused across local-CLI agents (Codex first); un-defers OPP-0046's cross-agent memory bus (proposed)
* [OPP-0060: Non-Native (Local-CLI) Transport Adapter](docs/opportunities/OPP-0060-cli-transport-adapter.md) — A headless CLI (Codex/Copilot/Grok) participates in the bus over the frozen store + contracts (no contract change): a per-dispatch runner (poll → ack → `tier_ceiling` → invoke-as-data → done/block; Tier ≥4 auto-blocks) + a `build_argv` defense-in-depth second gate; fills PRD-0039's deferred non-native-adapter slot; Grok live round-trip proven; shipped #198/#201 (accepted)
* [OPP-0061: `validate-agent-bus.sh` Contract-Conformance Linter](docs/opportunities/OPP-0061-validate-agent-bus-linter.md) — The deferred bus linter: a `--scan-file` conformance checker over a bus-transcript JSONL (envelope/payload-keys/correlation/lifecycle/`tier_ceiling`/`sync` caps), operator-run against a live session + CI-tested against fixtures; moves the schema/lifecycle/`tier_ceiling` checks §10 Asserted-only → Half-enforced and sets the `sync` caps the contract deferred to it (proposed)
* [OPP-0062: A2A Agent-Card Descriptor Layer + `tier_ceiling` Extension](docs/opportunities/OPP-0062-a2a-agent-card-descriptor-layer.md) — Align the agent-coordination bus's ad-hoc per-agent descriptors to the A2A Agent Card without changing transport or the control schema (A2A has no local binding, so the file bus stays; `tier_ceiling` has no standard equivalent → a named extension); annotate the 7 message types to A2A task states, `sync` stays bespoke; field-reported + independently re-derived, external-spec claims pending primary confirmation (proposed)
* [OPP-0063: Consumer-Upgrade Safety: Compat Affordances + Layout-Agnostic Validators](docs/opportunities/OPP-0063-consumer-upgrade-safety.md) — One root cause across four consumer-repo upgrades: the harness assumes its dogfooded root `platform/`/`SUMMARY.md`/`docs/` layout is the consumer's, and a breaking rename shipped with no compat affordance. F2 (defect) `relational-postgres`→`relational-sql` hard cutover crashes 9/14 validators for a consumer selecting the old id → deprecation alias + BREAKING note; F1 `validate-list-completeness` hard-codes root `SUMMARY.md`; F3 `validate-catalog-counts` misreads a `.harness/` mount as drift; F4 `validate-placeholders` ISO-date-arm over-match; F5 augment the existing upgrade runbook; field-reported + re-derived against the real validators on disk (proposed)
* [OPP-0064: AntiGravity Headless (`agy`) CLI — Correct the Stale "No Headless CLI" Claim + Non-Native Adapter](docs/opportunities/OPP-0064-antigravity-headless-adapter.md) — The agent-coordination docs use AntiGravity as the canonical "no headless CLI / IDE-only" example in ≥5 places, now factually false: `agy -p "<prompt>"` is a documented headless mode. F1 correct the stale claim (§10-inward, like #207); F2 add a prompt-collapsing permission archetype; F3 judge headless success on output not exit code (issue #408 exit-0-empty, #318 hang); F4 an `agy` adapter is design-only/deferred — the peer's `--print=` argv is contradicted by the vendor's own docs (space-separated `-p`), resolve against a live run first; field-reported + web-verified against AntiGravity's own headless docs + issue tracker (proposed)
* [OPP-0065: Govern the ruvnet/claude-flow (`ruflo`) Runtime as a Non-Native Adapter + Positioning Artifact](docs/opportunities/OPP-0065-govern-ruflo-runtime.md) — `ruflo` is `claude-flow` rebranded, a runtime agent meta-harness (swarms, vector memory, federation, ~210–314 MCP tools; MIT) — the LangGraph/CrewAI genre, the opposite of auto-harness's governance genre. Its trust/audit are algorithmic/security, not human-ratified, so it's a governed TARGET. Move 1: govern it via the existing non-native CLI adapter (its headless CLI) + ACP bridge (its MCP server) under trust tiers, near-zero new primitives; Move 2: a positioning artifact (OPP-0056 analog). Out of scope: don't vendor/build-on it (Node+WASM+web-server+keys inverts the dependency-free posture) or adopt its ADR/metaharness plugins; pin a version, never track main (proposed)
