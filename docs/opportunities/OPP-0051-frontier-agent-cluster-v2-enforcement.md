...
**Owner:** @unclenate
**Created:** 2026-06-28
**Last Updated:** 2026-06-28 *(flipped `exploring` → `accepted` at implementation-merge: `validate-trace-contract.sh` shipped — the artifact-content half's anchor validator (module-gated/predict-clean, mirrors `validate-sast-coverage`; parses the trace-contract frontmatter and asserts a pinned semconv version + a conventional GenAI span + a `content_capture ∈ {opt-in,none}` posture; presence + shape only). Added the machine-checkable frontmatter to the trace-contract template; validator count 20 → 21; fixture tests + full propagation. The code-cross-reference half + companion rule stay deferred. **Follow-on phases** (reuse the shape-assertion skeleton): `validate-foundry-target.sh`, `validate-model-routing.sh`, `validate-agent-defense-in-depth.sh` — ratified together by [PRD-0032](../requirements/PRD-0032-cluster-content-validators.md) and **SHIPPED 2026-06-28** (one PR; validator count 21 → 24; frontmatter added to all three templates; predict-clean). **The cluster's artifact-content v2 enforcement is now COMPLETE across all four satellites** — only the code-cross-reference half (declarations match running code) remains deferred (still needs consumer code paths). Prior: 2026-06-28 promoted `proposed` → `exploring` when [PRD-0031](../requirements/PRD-0031-validate-trace-contract.md) was drafted + accepted.)*
**Confidence:** medium
