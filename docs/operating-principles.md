...
- **Any source-of-truth field mirrored into N derived surfaces creates N−1 unenforced drift sites — mechanize the reconciliation or stop treating the mirror as authoritative.** The three bullets above are instances of one law: `validate-catalog-counts.sh` recomputes *counts* and `validate-list-completeness.sh` checks *row presence*, but a field's *value* mirrored into a derived view — a status column, a navigation list, a restated sub-claim — is guarded by neither, and a person reconciling "the index" by hand fixes whichever surface is top-of-mind and silently leaves the rest, so the miss-rate scales with the number of surfaces. Two instances proved it the hard way. (1) An OPP's `Status` is mirrored into both `docs/opportunities/candidates.md` (a cluster annotation) and `docs/README.md` (a status column); 10 of 53 annotations had disagreed with their record for up to six weeks, and the from-memory reconciliation that fixed the first surface (PR #177) silently left one record drifted in the second — a manual sweep of one mirror is not a fix. (2) This file's own promotion change also corrected a `SUMMARY.md` navigation whose Opportunity and Requirement lists had lagged six records behind disk, because `validate-list-completeness.sh` gates the `docs/README.md` index but not the nav enumeration — an unguarded mirror hiding in plain sight. The durable move is recompute-and-compare across *all* mirrors at once: the count and enumeration validators are the pattern, and `validate-status-parity.sh` (OPP-0054) is the status instance. Until a mirror is mechanized, treat it as decorative, not authoritative — and prefer designs that remove the mirror entirely (a nav that links an index page rather than re-listing every record cannot drift from that record set).

- **A derived table states the kernel MODEL; an operational tightening is stated once, as a tightening, never by giving a tier a different value in a model cell.** The trust-tier model lives in the kernel (`trust-model.md`): Tier 3 (git-writing) is *authorized within the agent's ceiling*, with the PR-review gate before merge and the human-authorization gate proper at Tier 4. That model is mirrored into three derived tables (`README.md`, the `harness-governance` skill, `AGENTS.md`) — and it drifted into a **three-way contradiction** (a 2026-09-01 audit): README said the Tier-3 gate was "None", the skill said "Yes, with care", and AGENTS.md said "Requires explicit instruction". The first two mirror the kernel model; the third had written this repo's *operational policy* (its maintainer requires explicit instruction before any git-writing — a legitimate tightening the kernel's own "the operator may lower the ceiling" rule permits) **into a model-table cell**, where it read as a fourth conflicting definition rather than a deliberate tightening — and the *permissive* reading lived in the skill agents load on demand, so an agent following the docs would git-write unprompted. The fix keeps every model table faithful to the kernel and states the operational tightening exactly once, labelled as a tightening, in the operating manual (`AGENTS.md`) with a pointer from the skill. Generalizes: never encode a deployment policy by mutating a value in a table whose job is to state the shared model — the model tables must agree, and policy is a separate, singly-sourced layer on top.
