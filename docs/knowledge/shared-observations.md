...
**Last Updated:** 2026-08-15 *(GROK CLIENT-PACK ANALYSIS: first Grok Build TUI session on auto-harness; three observations — native-AGENTS.md client-pack gap (Grok is a client pack, not a model-routing row), vendor-compat dual-authority (Grok also loads Claude/Cursor surfaces), compact-ahead capture belongs in the harness ledger not tool-local memory. Audit-trail companion: docs/project/change-log.md. Next artifact is an OPP for agents/grok-cli (not a GROK.md, not a skill). Prior: 2026-06-30 *(OPP-0027/0047 BACKLOG-HYGIENE RECONCILIATION: the 3-lens triage surfaced two stale-status loose ends from the cluster wave; reconciled both. OPP-0027 (frontier-agent posture anchor) flipped `proposed`→`accepted` — its four satellites all shipped (OPP-0028/0029/0030/0031 + v2 enforcement OPP-0051), and the `management/frontier-agent-posture` umbrella overlay it also proposed was deliberately NOT built (per the OPP's own OQ1 bias toward incremental à-la-carte adoption; an umbrella would add a forcing dep with no new capability). OPP-0047 (delivery-cost) stays `proposed` (wedge unbuilt) but its Disposition now cross-links PRD-0025, which adopted it as a deferred v2 phase of `management/work-package`. NEW OBSERVATION (anchor-OPP disposition): a cluster-anchor OPP is "accepted" when its satellites ship even if a standalone umbrella it also proposed is dropped-as-redundant — "accepted" requires value delivered + every sub-proposal explicitly dispositioned (built / dropped / split), never silent residue; anchors drift worst because their value arrives distributed across satellites so no single merge flips them. Satisfies the PRD-0004 distillation rule fired by the two modified OPPs. Prior: 2026-06-28 OPP-0025 IMPLEMENTATION (consumer-side integration smoke test): after the frontier-agent cluster thread closed, a 3-lens backlog triage (value / readiness / staleness, 3 parallel agents) converged on OPP-0025 as the highest value×readiness item with no maintainer-strategic gate; shipped it directly per the half-day-fully-biased-OPP discipline (no PRD). Added a `submodule-smoke-test` job to both consumer CI templates (github-actions.yml + gitlab-ci.yml) — clean-checkout separate job that asserts `.harness/` materialized + smoke-runs one validator — plus a CI-README § and a present-tense recipe in submodule-integration.md §6. Additive (no validator/module/§10/count change). Flipped OPP-0025 proposed→accepted. NEW OBSERVATION (cross-repo enforcement topology): some failure modes are structurally invisible to the PRODUCER's own CI (the harness IS the upstream, so it has no consumer-side clone) — the only mechanical check is one the producer SHIPS INTO the consumer's CI as a templated primitive; the producer authors+distributes the enforcement rather than running it (npm-pack/cold-pull/clean-VM-tap shape, but the shipped artifact is the test itself). The intra-repo sibling gap is already closed producer-side by validate-list-completeness; the cross-repo one fundamentally can't be. Triage also surfaced 2 hygiene loose ends (OPP-0027 anchor = closeable now its 4 satellites shipped; OPP-0047 needs a PRD-0025 cross-link) — noted for a maintainer pass, not bundled here. Satisfies the PRD-0004 distillation rule fired by the modified OPP-0025. Prior: 2026-06-28 PRD-0032 IMPLEMENTATION (cluster content validators phases 2-4, OPP-0051): shipped the THREE remaining artifact-content validators in ONE PR — validate-foundry-target.sh (foundries enum + live/roadmap status), validate-model-routing.sh (≥1 task→model route, providers free-form), validate-agent-defense-in-depth.sh (all four pattern keys) — each a near-exact copy of the validate-trace-contract.sh skeleton with artifact + invariants swapped, + machine-checkable frontmatter added to its template (self-validating). 16 new fixture tests (203 total, 0 fail), shellcheck-clean, predict-clean. Validator count 21→24; full propagation across ~8 surfaces ×3. **The cluster's artifact-content v2 enforcement is now COMPLETE across all four satellites** (trace-contract + foundry-target + model-routing + defense-in-depth); only the code-cross-reference half remains deferred. NEW OBSERVATION (validator-family cost is propagation, not logic): the per-instance logic is ~30 trivial lines but propagation is O(N×surfaces) — bundling the family in one PR touches each surface ONCE (one README-table edit for 3 rows, one count bump 21→24) instead of 3×8 surface visits + 3 count reconciliations across 3 PRs; the design-time one-change-class→one-PRD call pays off at impl as propagation amortization. Satisfies the PRD-0004 distillation rule fired by the modified kernel/base module.yaml + OPP-0051. Prior: 2026-06-28 PRD-0032 (cluster content validators, phases 2-4 of OPP-0051, design-only): ratified the THREE remaining artifact-content validators in ONE family PRD — validate-foundry-target.sh (foundries enum + live/roadmap status), validate-model-routing.sh (≥1 task→model route; providers free-form, no enum), validate-agent-defense-in-depth.sh (all four pattern keys present). Each mirrors the shipped validate-trace-contract.sh skeleton EXACTLY (requirement-set gate, Bash 3.2 + Ruby YAML.safe_load, 3-state, --scan-file, predict-clean) + adds machine-checkable frontmatter to its template. Impl may be one PR or staged; validator count 21→24. Code-cross-reference half + companion rules stay deferred for all three. NEW OBSERVATION (one change-class = one PRD even for N instances): the three differ only in artifact + invariants — the skeleton is identical — so per §7 they are ONE change-class (the artifact-content-shape pattern), folded into one PRD with a per-instance invariant table rather than three near-copies; the design-doc analog of factoring a shared function. Design-only per §9 (counts unchanged this PR). Satisfies the PRD-0004 distillation rule fired by the modified OPP-0051. Prior: 2026-06-28 PRD-0031 IMPLEMENTATION (`validate-trace-contract.sh`, OPP-0051 first deliverable): shipped the frontier-agent cluster's FIRST v2-enforcement validator — the artifact-content/shape check for `docs/observability/trace-contract.md`, mirroring validate-sast-coverage (Bash 3.2 + inline Ruby YAML.safe_load, 3-state exit, --scan-file fixture seam). Asserts 3 invariants: semconv_version pinned, ≥1 conventional GenAI span (chat/invoke_agent/execute_tool/create_agent/embeddings/invoke_workflow), content_capture ∈ {opt-in,none}. Added the machine-checkable YAML frontmatter to the trace-contract TEMPLATE (line-1, before the copyright comment, real example values so it self-validates — exactly how sast-coverage.md does it). 7 fixture tests (TestValidateTraceContract), shellcheck-clean, predict-clean on the harness. Full propagation: harness.yml CI step, AGENTS.md, harness-governance SKILL chain+bullet, validators/README table+run+test lists, kernel/base validators catalog, README table+mermaid+word-forms, SUMMARY list+count; validator count 20→21 across all catalog-count sites. Flipped OPP-0051 exploring→accepted. NEW OBSERVATION (gate on the artifact's requirement-set, not a module id): unlike sast-coverage (single-owner artifact → gate on module id), trace-contract.md is a SHARED artifact (agent-observability owns it, ai-foundry-target reuses it via deferred-dependency), so the gate must be "any active module requiring the artifact" — keying off requiredArtifacts membership keeps it correct as more modules share the artifact, with no validator edit. Two module-gated variants: single-owner vs shared-artifact. Code-cross-reference half + companion rule stay deferred. Satisfies the PRD-0004 distillation rule fired by the new validator + the modified kernel/base module.yaml + OPP-0051. Prior: 2026-06-28 PRD-0031 (trace-contract content validator, OPP-0051 first deliverable, design-only): ratified `validate-trace-contract.sh` — the artifact-content half's anchor validator, module-gated/predict-clean, mirroring the shipped validate-sast-coverage (parse a small YAML frontmatter block on the artifact, assert presence+shape via Ruby YAML.safe_load, Bash 3.2, 3-state exit, --scan-file fixture seam). Asserts THREE load-bearing invariants: semconv_version pinned, ≥1 span in a conventional GenAI operation shape (chat/invoke_agent/execute_tool/create_agent/embeddings/invoke_workflow), content_capture ∈ {opt-in,none}. CLUSTER-AWARE GATE: activates when ANY active module requires docs/observability/trace-contract.md (agent-observability OR ai-foundry-target, since the latter reuses the artifact via deferred-dependency) — not a single module name. Impl adds a frontmatter block to the trace-contract template (machine-checkable mirror of the prose) + bumps validator count 20→21. NEW OBSERVATION (§10 sub-axis decomposition): split PRD-0014's one Half-enforced claim into C-TRACE-1 (artifact-shape → now Enforced, buildable today) + C-TRACE-2 (runtime-conformance → stays Asserted, needs consumer code path); a §10 claim isn't one dial at the midpoint, it's multiple axes averaged — classify per-axis, and the §10 table becomes the design tool that locates the next buildable validator. Code-cross-ref half + companion rule stay deferred. OPP-0051 proposed→exploring (flips accepted at this validator's impl-merge); other 3 content validators = follow-on phases reusing the skeleton. Design-only per §9 (counts unchanged this PR). Satisfies the PRD-0004 distillation rule fired by the modified OPP-0051. Prior: 2026-06-28 OPP-0051 (frontier-agent cluster v2 enforcement, design-only): opened the deferred v2-enforcement thread the four cluster PRDs each named. KEY MOVE: "v2 enforcement" is NOT a monolith — split it into (1) artifact-content/shape conformance (is the declared artifact internally well-formed? — fixture-testable TODAY, no consumer code, like the shipped validate-sast-coverage) and (2) code-cross-reference (does the declaration match running code? — still needs a fixed consumer code path, STAYS deferred). The original v1 deferral blocked (2) not (1); ship (1). Anchor on validate-trace-contract.sh (OTel trace conformance = the cross-foundry anchor the other three lean on), module-gated/predict-clean; the other three content validators reuse its shape-assertion skeleton as follow-on phases. Asserts load-bearing invariants only (presence+shape), never exhaustive correctness, mirroring validate-module-stability. NEW OBSERVATION (deferral-is-not-a-monolith): harvesting a "deferred to vN" item means re-deriving the SPECIFIC blocker that caused the deferral and testing each sub-capability against it — often a fixture-testable sub-part became buildable while the rest stays blocked; split on that predicate. Proposed (not yet PRD/built). Satisfies the PRD-0004 distillation rule fired by the new OPP-0051. Prior: 2026-06-28 PRD-0030 IMPLEMENTATION (`architectures/agent-defense-in-depth`, OPP-0031): shipped the defense-in-depth v1 — a new opt-in architecture module + TWO templates (`templates/security/agent-defense-in-depth.md` single artifact w/ four named sections one-per-Microsoft-pattern + `append-only-action-log.md`) declaring how a project realizes scope-containment / least-permissions / human-in-the-loop / agent-identity; declarative v1, NO companion rule/validator (v2). **COMPLETES the frontier-agent cluster — all four OPP-0027 satellites (observability/foundry-target/model-routing/defense-in-depth) now built.** Fourth back-to-back cluster module; both templates joined the EXISTING `templates/security/` subdir (no new list-completeness row). Propagation + counts (modules_profiles 51→52, modules_all 60→61, templates 96→98); flipped OPP-0031 exploring→accepted; roadmap now records the cluster fully built. NEW OBSERVATION (conditional-requirement → reviewGate): the action-log is required ONLY WHEN the project declares autonomous actions — a condition module.yaml's two levers (requiredArtifacts always / optionalArtifacts never) can't express and the harness can't observe; encoded it as optionalArtifacts + a reviewGate stating the conditional (schema-permissive, human-enforced), the §10 Half-enforced-by-design shape for any requirement contingent on unobservable consumer behavior. Satisfies the PRD-0004 distillation rule fired by the new `platform/profiles/architectures/agent-defense-in-depth/module.yaml` + modified OPP-0031. Prior: 2026-06-28 PRD-0029 IMPLEMENTATION (`architectures/intelligent-model-routing`, OPP-0030): shipped the model-routing v1 — a new opt-in architecture module + `templates/architecture/model-routing.md` declaring a project's task→model routing table (structured task·model·rationale·constraints), routing criteria (capability/cost/privacy/regulatory/deployment-context), a FREE-FORM provider list (enum rejected — landscape moves), and foundry-routing seams; `model-routing-rationale.md` optional; declarative v1, NO companion rule/validator (v2). Third back-to-back cluster module (after foundry-target) = lowest-drift mirror again; template joined the EXISTING `templates/architecture/` subdir (no new list-completeness row). Propagation + counts (modules_profiles 50→51, modules_all 59→60, templates 95→96); flipped OPP-0030 exploring→accepted; roadmap records it as the third shipped satellite. NEW OBSERVATION (loop-closure-is-also-deferred): this module OWNS the model-routing.md that ai-foundry-target lists OPTIONAL, but shipping the owner did NOT flip foundry-target's optional→required — that would be a BREAKING change for existing foundry-target consumers; hardening optional→required is its own opt-in change-class (§7) gated behind its own version bump, not a silent side effect of a sibling landing. A cluster tightens internal coupling monotonically + deliberately. Satisfies the PRD-0004 distillation rule fired by the new `platform/profiles/architectures/intelligent-model-routing/module.yaml` + modified OPP-0030. Prior: 2026-06-27 PRD-0029 + PRD-0030 DESIGN PASS (frontier-agent cluster completion, OPP-0030 + OPP-0031): authored the two remaining satellite design contracts in one pass — PRD-0029 `architectures/intelligent-model-routing` (task→model routing table, free-form provider list, rationale doc optional) and PRD-0030 `architectures/agent-defense-in-depth` (Microsoft's four patterns as one artifact w/ four sections + optional-but-review-gated append-only-action-log). Both declarative-v1 mirroring the shipped siblings; both reuse EXISTING template subdirs (`templates/architecture/`, `templates/security/`) so no new list-completeness rows; both flip their OPP proposed→exploring (accepted at impl-merge). KEY RATIFICATION JUDGMENT: each OPP PROPOSED a companion rule, but the PRD pass DEFERRED both to v2 — (1) the two shipped siblings are declarative-v1 (enforcing v1 would break cluster consistency) and (2) a declarative architecture overlay has no fixed consumer code path to anchor triggerPaths on, so the proposed rule isn't even well-formed at v1. NEW OBSERVATION: an OPP's proposed enforcement mechanism is a HYPOTHESIS the PRD ratifies against shipped precedent + schema-well-formedness, not a spec to transcribe verbatim — the design-time analog of declare-first-enforce-later. PRD-0029 also closes the deferred-dependency loop (owns the model-routing.md that ai-foundry-target lists optional). Design-only per §9 — modules/templates ship in later impl PRs (counts unchanged this PR). Satisfies the PRD-0004 distillation rule fired by the modified OPP-0030 + OPP-0031. Prior: 2026-06-27 PRD-0028 IMPLEMENTATION (`architectures/ai-foundry-target`, OPP-0028): shipped the foundry-target v1 — a new opt-in architecture module + `templates/architecture/foundry-targets.md` declaring which enterprise AI foundries a project targets (web-verified enum azure-ai-foundry/nvidia-ai-foundry/palantir-aip/aws-bedrock-agentcore/google-vertex-agent-engine/custom, in the artifact not module.yaml) + the three portable evidence axes (OTel trace conformance, portable evals, open-protocol routing/MCP/A2A); declarative v1, NO companion rule/validator (deferred to v2); requires foundry-targets.md + trace-contract.md (reused from shipped OPP-0029), model-routing.md OPTIONAL via optionalArtifacts until OPP-0030 ships. Built back-to-back with its agent-observability sibling = copy-exact, lowest-drift mirror; template subdir `templates/architecture/` mirrors the artifact's docs/ destination dir (new list-completeness row). Propagation + counts (modules_profiles 49→50, modules_all 58→59, templates 94→95); flipped OPP-0028 exploring→accepted. NEW OBSERVATION: the second cluster module built right after a fresh sibling is the lowest-drift build there is — conventions resolved one PR earlier are inherited from an on-disk exemplar, so sequence cluster siblings back-to-back; the deferred-dependency wiring carries zero enforcement cost on the harness's own CI (module-graph checks only dependsOn, required-artifacts is predict-clean). Satisfies the PRD-0004 distillation rule fired by the new `platform/profiles/architectures/ai-foundry-target/module.yaml` + the modified OPP-0028. Prior: 2026-06-27 PRD-0028 (AI foundry target awareness, OPP-0028, design-only): continued the frontier-agent cluster — filed PRD-0028 for an opt-in `architectures/ai-foundry-target` module declaring which enterprise AI foundries a project targets + portable evidence per foundry, mirroring the just-shipped agent-observability sibling (declarative v1, no companion rule/validator). Two agents groundworked; web research reliable (Microsoft rebranded Azure AI Foundry→"Microsoft Foundry"; AWS Bedrock AgentCore GA 2025-10 most-portable, Palantir AIP least; `create_agent`/`invoke_agent` OTel spans = cross-foundry conformance anchor), build-map agent's counts again garbled (claimed 100/101, real is 94→95) — verified, used the real numbers. KEY DESIGN OBSERVATION (the DEFERRED-DEPENDENCY model): a satellite module in a cluster can require artifacts owned by SIBLING modules at different build stages — ai-foundry-target wants trace-contract.md (OPP-0029, SHIPPED) + model-routing.md (OPP-0030, NOT built). Resolution: require the SHIPPED sibling's artifact, make the UNBUILT sibling's artifact OPTIONAL (becomes required-by-convention when its owner ships). This lets a mutually-reinforcing cluster build INCREMENTALLY without a strict dependency chain or blocking each module on all its siblings — the inverse failure of over-coupling. Also: the `foundries` enum lives IN the artifact (frontmatter), NOT module.yaml (the schema rejects arbitrary fields — would've failed validate-manifest). v1 enum web-verified: azure-ai-foundry/nvidia-ai-foundry/palantir-aip/aws-bedrock-agentcore/google-vertex-agent-engine/custom. OPP-0028 promoted proposed→exploring (flips accepted at impl-merge). Satisfies the PRD-0004 distillation rule fired by the modified OPP-0028. Prior: 2026-06-27 PRD-0014 IMPLEMENTATION (`architectures/agent-observability`, OPP-0029): shipped the agent-observability v1 — a new opt-in architecture module declaring a project's OpenTelemetry multi-agent TRACE CONTRACT via two required-artifact templates (`docs/observability/{trace-contract.md,exporters.md}`), NO v1 enforcement (companion rule + validate-trace-contract.sh deferred to v2). Used TWO agents for groundwork: one WEB-VERIFIED the current OTel GenAI semconv (still Development/Experimental, now in the dedicated `semantic-conventions-genai` repo, semconv v1.42.0; live churn `gen_ai.system`→`gen_ai.provider.name`, `gen_ai.prompt`→`gen_ai.input.messages`; content attrs opt-in/privacy-sensitive), one mapped the build. TWO META-OBSERVATIONS: (1) web-grounding a fast-moving external standard AT IMPLEMENTATION beats copying a months-old draft's example pin — the May-drafted PRD's `2026-05-15` pin example was already stale; the template now pins the verified-current version + flags the churn + ties content-capture to privacy-by-design. (2) AGENT OUTPUT MUST BE VERIFIED AGAINST THE SOURCE OF TRUTH: the build-map agent invented module details that CONTRADICTED the PRD — it added companion rules the PRD explicitly defers to v2, the wrong required artifacts, and THREE differently-named templates vs the PRD's TWO. Followed the PRD over the agent; the OTel research + propagation/count map were the reliable agent outputs, the invented module.yaml was not. Propagation + counts (modules_profiles 48→49, modules_all 57→58, templates 92→94); flipped OPP-0029 exploring→accepted. Satisfies the PRD-0004 distillation rule fired by the new `platform/profiles/architectures/agent-observability/module.yaml`. Prior: 2026-06-27 PRD-0014 FINALIZED + ACCEPTED (agent observability, v1.1): picked OPP-0029 as the next thread; reviewed its month-old drafted PRD (2026-05-26) against `main`. LESS drift than PRD-0007 (the ADR-0013 ref is correct — it really is the visual-program ADR; catalog counts were relative not hardcoded), but three real corrections: `type: architectures`→`architecture` (family dir is plural, type value singular — same convention trap as canonical-position), validator count 8→20, and the deferred rationale-expansion operating-principle promotion retargeted §9→§13 (§9 now "Split Design from Implementation"). KEY NEW OBSERVATION: a freshly-shipped always-on structural validator immediately constrains the NEXT module's design — PRD-0014 (drafted 2026-05-26) couldn't have known about `stability` (shipped 2026-06-26 via PRD-0027), but the finalization HAD to add a `stability: beta` requirement to the new module because `validate-module-stability.sh` now blocks any module without it. The catalog's own governance is a moving floor: each always-on validator added becomes a constraint every subsequent module/PRD must satisfy, so finalizing a stale PRD means reconciling against not just renamed conventions but NEWLY-REQUIRED fields that postdate the draft. Core design unchanged (opt-in `architectures/agent-observability` module + trace-contract/exporters templates, no v1 enforcement, OTel version-pin re-verified at impl). OPP-0029 stays exploring (flips at impl-merge). Satisfies the PRD-0004 distillation rule fired by the modified OPP-0029. Prior: 2026-06-26 PRD-0027 IMPLEMENTATION (`validate-module-stability.sh` + 57-module backfill, OPP-0050): shipped module stability tiers end-to-end — an always-on structural validator asserting every module.yaml declares `stability: {experimental|beta|stable}` (presence+enum only), and backfilled all 57 modules against the rubric: 10 STABLE (the harness-dogfooded core — kernel/base, agents base/generic-llm/openclaw/claude-code, delivery/internal-platform, management project-standard/product-lite/knowledge-capture/opportunity-capture), 2 EXPERIMENTAL (stacks/coffeescript, domains/agentic-interfaces), 45 BETA. Wired always-on (kernel/base, harness.yml, AGENTS.md, harness-governance chain, READMEs; validator count 19→20); rubric in extending-the-harness.md + --help; onboarding surfaces stability + an honest stack-parity note; §10 gains a bullet (stability = §10 classify-before-enforcing at the MODULE level). OPP-0050 COMPLETE. Appended the observation that the honest backfill distribution for an alpha platform skews heavily BETA — only 10/57 earned `stable` (those the harness itself dogfoods via its own manifest, the strongest battle-testing signal available), and resisting the temptation to inflate `stable` is the whole point: a module-readiness signal is only worth shipping if it's conservative, so "dogfooded-by-the-harness-manifest OR foundational" was the bright-line test for stable, everything-shipped-but-unproven defaulted to beta, and only genuine scaffolds got experimental. Satisfies the PRD-0004 distillation rule fired by the 57 modified module.yaml files (+ the §10 edit). Prior: 2026-06-26 PRD-0027 (module stability tiers, OPP-0050 wedge): ratified the v1 — a per-module `stability: {experimental|beta|stable}` field + an ALWAYS-ON structural `validate-module-stability.sh` enumerating every module.yaml and blocking any that omits `stability` or is out-of-enum. Because it is always-on (a catalog check like list-completeness, NOT predict-clean), it forces its OWN backfill: the implementing PR must populate all 57 harness modules against a coarse 3-tier rubric (stable=shipped+enforced+dogfooded/founded; beta=shipped/thin-enforcement-or-no-consumer; experimental=scaffold/niche) or the harness's own CI fails. The validator checks presence+enum ONLY, never correctness of the judgment — honesty is an authoring act, as §10 classification is. Stability is a THIRD axis independent of trust tier (risk) and §10 (per-claim enforcement). Flipped OPP-0050 proposed→accepted. Appended the observation that an ALWAYS-ON structural validator is self-forcing: unlike a predict-clean module-gated validator (which no-ops on the harness because the harness doesn't activate the module), a structural catalog check runs against the harness's own catalog, so you CANNOT ship the field-requirement without simultaneously satisfying it everywhere — the validator and the backfill are inseparable in one PR. Deferred (separate change-classes per §7): behavior-gating on experimental, a `deprecated` value + lifecycle policy, stack build-out. Satisfies the PRD-0004 distillation rule fired by the modified OPP-0050. Prior: 2026-06-26 OPP-0050 (module stability tiers & parity normalization): evaluated an external (ChatGPT-lane) leadership platform-review against the actual repo and filed the one structural gap that survived. KEY: the review was a generic outside-in template that pattern-matched "growing platform → under-normalized/sprawling" without seeing the controls — but a field audit showed ~70% already-built (required metadata uniform 6-fields-in-57/57; compatibility via dependsOn+conflictsWith; composition governed by §12 + validate-module-graph; new-module-justification = the §12 inclusion test). The surviving 10%: the catalog has NO module-readiness signal — `stability`/`maturity` in 0/57 modules. Filed OPP-0050 proposing a per-module `stability: {experimental|beta|stable}` field — a THIRD axis INDEPENDENT of trust tier (=risk/autonomy) and §10 (=per-claim enforcement strength); a module can be high-tier+experimental or low-tier+stable. Named `stability` not `maturity` (collision: manifest has `maturity: platform`, digital-twin has consumer twin-maturity). Appended the observation that an outside-in review must be evaluated against the controls that ALREADY EXIST before acting — most "normalize the framework" recommendations were already met by §12/§10/companion-rules/uniform-metadata, and treating them as undone would generate the taxonomy-museum the review itself warned against; the discipline is to distill the surviving signal, not execute the template. Deferred (separate change-classes per §7): behavior-gating on stability, module deprecation/lifecycle policy, stack-parity build-out (stacks/ is 3/4 JS-family — OPP-0011/0012 territory). Satisfies the PRD-0004 distillation rule fired by the new OPP-0050. Prior: 2026-06-26 PRD-0007 IMPLEMENTATION (`management/canonical-position` overlay, OPP-0007): shipped the canonical-position v1 — a new opt-in management overlay (`dependsOn: [kernel/base, project-standard]`) giving a project a single ratified strategic north-star (`docs/canonical-position.md`) every strategy-shaped artifact must cite, revised only via a paired review-artifact (the ratification companion rule); two templates (`templates/canonical-position/{canonical-position.md,review.md}`); propagation + counts (modules_profiles 47→48, modules_all 56→57, templates 90→92); flipped OPP-0007 exploring→accepted. Predict-clean: the harness does NOT activate the module so its citation/ratification rules never fire on the harness's own PRs (the citation rule's `^docs/opportunities/OPP-` trigger would otherwise fire on every OPP edit). TWO PRD-vs-reality reconciliations worth recording: (1) module ids are BARE (`canonical-position`, not `management/canonical-position`) and `dependsOn` references the bare id — the PRD's `management/` prefix was wrong on both; (2) there is NO type-grouped `templates/management/` subdir in the catalog — every template subdir is concern-named — so the templates live under `templates/canonical-position/`, not the PRD's `templates/management/`. Both are the kind of detail a finalized-but-not-against-disk PRD still gets wrong; the implementation pass is where module-convention drift surfaces (mirrors the design-artifact-staleness candidate stub). Satisfies the PRD-0004 distillation rule fired by the new `platform/profiles/management/canonical-position/module.yaml`. Prior: 2026-06-26 PRD-0007 FINALIZED + ACCEPTED (canonical-position artifact, v1.1): picked OPP-0007 as the next backlog thread; reviewing its month-old drafted PRD (2026-05-24) against current `main` surfaced substantial drift — "8 validators" (now 19), stale catalog counts (modules_profiles 26→27 / templates 56→58 vs real 47 / 90), an anticipated "Diagram 10" + "ADR-0013" both since REASSIGNED, and obsolete v0.6.0/v0.7.0 sequencing. Reconciled all of it and accepted PRD-0007 as the v1 design contract; SPLIT FR-006 out (the 3 orthogonal reconciliation-load patterns / Observation E → a follow-up §13) because §9 is now taken by "Split Design from Implementation" and bundling an unrelated operating-principle promotion into the module PR violates §7 (Align File Boundaries with Change-Class Boundaries). Appended the observation that a drafted-but-unaccepted PRD is a DRIFT LIABILITY: catalog counts, section numbers, ADR/diagram reservations, and version plans all rot while it waits, so "finalize-before-accept" must be a real reconciliation pass (re-derive counts vs main, re-check every reserved number, re-test bundled scope against operating-principles added since) — not a rubber-stamp; the same finalize-stale-plan discipline the cybersec-renumber rebase needed. Satisfies the PRD-0004 distillation rule fired by the modified OPP-0007. Prior: 2026-06-25 PRD-0026 IMPLEMENTATION (`validate-publication-boundary.sh`, OPP-0048 mechanism 1): shipped the always-on kernel-level publication-boundary gate — enumerates git-tracked / staged / explicit-path files and exits 1 if any declares a `do-not-publish` marker (frontmatter key or `<!-- do-not-publish: true -->` sentinel, line-start only); `.publication-boundary-ignore` exempts the files that discuss the marker; the parked seed brief now carries the marker locally and stays untracked (CI passes; staging it fails). Wired always-on into kernel/base + harness.yml + AGENTS.md + harness-governance chain + READMEs; validator count 18→19, template count 89→90 (new `templates/governance/` subdir). Appended the observation that the always-on-vs-predict-clean choice is determined by WHO is protected: a module-gated (predict-clean) validator no-ops when its module is inactive, which is correct for opt-in capability overlays (privacy/twin/work-package) but WRONG for a safety floor — a leak gate that only fires when a module is active would miss the very repos that never activate it, so a publication boundary must be always-on like validate-placeholders. Operating-principle §5 (Self-Governance) now names it (the harness turning its own declare-then-enforce contract on itself), which satisfies the AGENTS.md entrypoint companion AND the kernel/base module distillation in one substantive edit. Satisfies the PRD-0004 distillation rule fired by the modified `platform/core/kernel/base/module.yaml`. Prior: 2026-06-24 PRD-0026 (publication-boundary marker, OPP-0048 wedge): backlog triage (3 independent ranking passes over 19 open OPPs) converged on OPP-0048 as highest risk×readiness; filed PRD-0026 ratifying mechanism 1 — an always-on kernel-level `validate-publication-boundary.sh` that fails CI/pre-commit if any GIT-TRACKED file declares a `do-not-publish` marker (no name corpus needed; invisible while the file stays untracked); flipped OPP-0048 proposed→accepted; staged mechanism 2 (configurable content-denylist scan-scope) as a named phase-2 follow-up. Appended the observation that a publish-time gate is the INVERSE of `requiredArtifacts` (a must-NOT-be-tracked assertion), and that on-disk reality disproved the simpler directory-glob design — `docs/superpowers/specs/` holds TEN already-tracked published specs beside the ONE untracked unpublishable brief, so publication intent is a per-FILE property, not a per-directory one. Satisfies the PRD-0004 distillation rule fired by the modified OPP-0048. Prior: 2026-06-24 OPP-0049 Phase 3 (deep-governance-vertical authoring playbook): shipped `platform/workflow/deep-governance-vertical-authoring.md` — the step-by-step playbook §12 references (Step 0 inclusion-test gate, six build steps one-per-ingredient, a worked-examples matrix, the composition-shape + dual-spine decisions, a catalog-wide propagation checklist, a definition of done); flipped OPP-0049 Phase 3 → done; propagated the workflow count 22→23 across four prose sites + the SUMMARY row. The same pass corrected operating-principle §12's "built six" to the accurate **five built + one designed (cybersec, PRD-0022)** — authoring an accurate playbook surfaced that cybersec is designed-not-built on disk (no `domains/cybersec-*` module, no `validate-engagement-charter.sh`), a non-validator PROSE drift the catalog-count and list-completeness validators cannot see because cybersec was never counted as a module. Satisfies the PRD-0004 distillation rule fired by the modified OPP-0049. Prior: 2026-06-20 OPP-0049 (deep-governance-vertical authoring-pattern harvest): filed the design-only OPP that harvests the six-times-proven authoring skeleton (jurisdiction-neutral core + forcing artifact + default-deny bias guardrail + single-concern decomposition + one of three composition shapes + predict-clean module-gated validator) into a §12 operating principle (Phase 2) and an authoring playbook (Phase 3); meta-template deferred (Phase 4). Appended the observation that the skeleton generalizes across BOTH subject-matter domain families and cross-cutting discipline overlays — the inclusion test is neutral-core + forcing-artifact + standards-anchor, which is exactly what separates a deep governance vertical (healthcare/AEC/cybersec/geospatial/privacy/digital-twin) from a thin management overlay (work-package, the negative case, reuses only the predict-clean validator); overlays additionally carry a dual-spine anchor (interoperability standard + values framework) where domains carry one. Satisfies the PRD-0004 distillation rule fired by the new OPP-0049. Prior: 2026-06-19 PRD-0013 IMPLEMENTATION (session-shape workflow doc, origin OPP-0032): shipped `platform/workflow/session-shape.md` — the session-boundary checkpoint taxonomy + the six-class trigger taxonomy + the audit of declared-but-unfired reviews; flipped OPP-0032 → accepted and PRD-0013 → Accepted; appended the observation that all five currently-FIRED declared reviews are PR-boundary (companion rules) while every declared-but-UNFIRED review wants one of the other five trigger-classes the harness has on paper but never built. Satisfies the PRD-0004 distillation rule fired by the modified OPP-0032. Prior: 2026-06-19 PRD-0025 IMPLEMENTATION (Phase 2): shipped the `management/work-package` module + `validate-lane-integrity.sh` (validator chain 17→18) + lane template + idempotent worktree runbook + sample composition + diagram #16, catalog-only / predict-clean (the harness does not activate the module, so the new validator no-ops on the harness's own CI); appended the observation that a diff-checking validator stays cheaply fixture-testable by separating the diff-SOURCE (git, in main mode) from the diff-CHECKER (a pure function over an explicit changed-path list) — `--scan-file <spec> [path...]` exercises the exact lane-vs-diff logic CI runs without standing up a live branch. Satisfies the PRD-0004 distillation rule fired by the new `platform/profiles/management/work-package/module.yaml`. Prior: 2026-06-17 OPP-0048 (redaction-scope & publication-boundary hardening): filed the OPP-0036 follow-up after the 2026-06-17 GitBook / documentation pass re-confirmed a live gap — a public repo parks **untracked** private design material under `docs/superpowers/specs/` (excluded from both the placeholder validator and markdownlint) with zero publish-time gate, guarded today only by agent memory and manual `git add` discipline; appended the observation that the redaction primitive needs a file-level `do-not-publish` **blocking** marker (which requires no name corpus) plus a configurable wider-scope content scan, because per-file git-add discipline does not scale across a shared-identity multi-agent workspace. Satisfies the PRD-0004 distillation rule fired by the new OPP-0048. Prior: 2026-06-15 PRD-0025 promotes OPP-0046's lane wedge as `management/work-package` (design-only); flipped OPP-0046 → accepted (partial promotion) in the same PR; appended the observation that two tightly-coupled OPPs promote best as one module with staged phases — combine the home, sequence the depth — not two modules or two PRDs. Satisfies the PRD-0004 distillation rule fired by the modified OPP-0046. Prior: 2026-06-15 OPP backlog hygiene reconciliation: flipped six OPPs (0006/0033/0034/0036/0037/0045) + PRD-0006/0015 from proposed/exploring to accepted to match shipped reality; appended the observation that OPP/PRD status is un-validated prose that drifts after every implementation wave, the flip belongs in the implementing-PR checklist, and this is the live evidence for a backlog-review cadence (OPP-0032). Satisfies the PRD-0004 distillation rule fired by the six modified OPP files. Prior: 2026-06-15 OPP-0047 (delivery-cost & unit-economics governance): appended the observation that delivery cost is a new governance axis (economics) attaching to the same work-package unit OPP-0046 defines — lane = scope contract, cost record = economic contract — and follows the govern-the-contract-not-the-extraction boundary. Satisfies the PRD-0004 distillation rule fired by the new OPP-0047. Prior: 2026-06-15 OPP-0046 triage (#121 + #122 → parallel multi-agent work-package lane contract): appended the observation that the WP lane is the multi-agent analog of the module declare-then-enforce contract, and that shared-observations.md is already an emergent cross-agent memory bus. Satisfies the PRD-0004 distillation rule fired by the new OPP-0046. Prior: 2026-06-14 Geospatial / GIS wedge IMPLEMENTATION (Phase 2, PRD-0024): shipped three modules + four templates + diagram #15 + the 4-way geospatial-bim-twin composition catalog-only / predict-clean; appended the observation that the first cross-family dependsOn resolved with zero validator changes (a bridge module is a pure composition of existing primitives). Satisfies the PRD-0004 distillation rule fired by the three new module manifests. Prior: 2026-06-13 Geospatial / GIS deep-domain wedge design (OPP-0045 + PRD-0024): appended the observation that a deep-domain vertical can govern a seam between two domains via a cross-family module dependency — generalizing the harness composition model into a three-shape taxonomy (intra-family / domain×overlay / cross-family bridge) — and that the CRS forcing artifact adds a temporal axis to the jurisdiction-profile primitive. Satisfies the PRD-0004 distillation rule fired by the new OPP-0045. Prior: 2026-06-10 Issue #88 sensitivePaths catalog audit: folded every uncovered sensitivePath across seven modules into its own companion triggerPaths (self-coverage, generalizing PR #114), added the disabledValidations escape hatch to validate-sensitive-paths.sh, and gated the class by running the validator over shipped compositions in CI; appended the catalog-wide self-coverage observation. Satisfies the PRD-0004 distillation rule fired by the seven modified module.yaml files. Prior: 2026-06-10 privacy-by-design self-coverage fix: added `^auth/`, `^src/.*user`, and `tracking` to the module's own `companionRules.triggerPaths` so its WARN-layer `sensitivePaths` are self-enforced at the VALIDATE layer rather than relying on another active module to cross-cover them (which `validate-sensitive-paths` overlap-checking silently permitted); appended the observation naming the ambient-cross-coverage masking failure mode and the self-coverage principle. Satisfies the PRD-0004 distillation rule fired by the modified `platform/profiles/management/privacy-by-design/module.yaml`. Prior: 2026-06-10 Digital Twin overlay Phase 2: PRD-0023 implementation — module + 10 templates + 2 Half-enforced WARN validators + skill + composition + discoverability + count propagation shipped catalog-only/predict-clean; the two module-gated validators no-op on the harness's own CI; the dual-spine anchor is now concrete in templates. Prior: 2026-06-10 Digital Twin overlay Phase 1: OPP-0044 + ADR-0019 + PRD-0023 design contract; appended the observation that the deep-domain primitives generalize to a SECOND cross-cutting discipline overlay (after privacy-by-design), and that a twin-governance overlay needs a dual-spine anchor — interoperability/digital-thread standards plus a governance-values framework — to make planning→operational transformation a governed conformance question. Satisfies the PRD-0004 distillation rule fired by the new `docs/opportunities/OPP-0044-digital-twin-scenario-runtime.md` and `docs/adr/ADR-0019-digital-twin-scenario-runtime-overlay.md`. Prior: 2026-06-09 cybersec wedge (OPP-0043 + PRD-0022); 2026-06-07 greenfield conservatism (PRD-0021); 2026-06-06 bootstrap hardening (PRD-0020); 2026-06-05 onboarding safety + install prerequisites; 2026-06-04 AEC wedge Phase 2; the OPP-0038 attribution-boundary observation; and consumer-adoption observations from the fork-held-consumer pin-bump session.)*
...
### A cluster-anchor OPP is "accepted" when its satellites ship — even if the standalone umbrella it also proposed is deliberately dropped as redundant

- **Context:** OPP-0027 (frontier-agent posture) anchored a four-satellite cluster AND
  proposed a `management/frontier-agent-posture` umbrella overlay that would bundle them.
  The four satellites all shipped (OPP-0028/0029/0030/0031 + the OPP-0051 v2 enforcement);
  the umbrella overlay was never built. After the cluster closed, OPP-0027 sat stale at
  `proposed` — misleading, since its core value was fully delivered.
- **Observation:** An anchor OPP has two parts that resolve *independently*: the satellite
  cluster (the value) and any standalone bundling deliverable (often redundant once the
  satellites compose à la carte). Here the OPP's own Open Question 1 had already biased
  toward incremental adoption over a forced umbrella — so the umbrella was *consciously
  dropped*, not forgotten. The correct disposition is **`accepted`, with a Disposition that
  records the satellites as the realization AND states the umbrella was deliberately not
  built and why** (it adds a forcing dependency with no capability the satellites lack).
  "Accepted" does not require every proposed artifact to ship; it requires the OPP's value
  to be delivered and its remaining sub-proposals to be explicitly dispositioned (built,
  dropped-as-redundant, or split to a fresh OPP) — never left as silent residue.
- **Implication:** When a cluster anchor's satellites all land, close the anchor in the
  same hygiene pass — don't leave it `proposed`. Write the Disposition to distinguish what
  shipped from what was dropped, so a future reader doesn't mistake the dropped umbrella for
  unfinished work (and so a genuine future need for it becomes a clean new OPP, not residue
  on this one). This extends the earlier "OPP status is un-validated prose that drifts after
  every wave" lesson: anchors drift the worst, because their value arrives distributed across
  satellites and no single merge flips them.
- **Severity:** Low — a backlog-disposition lesson; leaving the anchor `proposed` reads as
  "the posture is unbuilt" when it is fully shipped.
- **Contributed-by:** Claude (OPP-0027/0047 hygiene reconciliation, 2026-06-30)

### Some failure modes are structurally invisible to the producer's own CI — the only enforcement is a check the producer ships *into the consumer's* environment

- **Context:** OPP-0025 (consumer-side integration smoke test). When a consumer adopts the
  harness via the git-submodule path, two failures are silent on the first developer's
  machine and only surface for the *second* developer or in *their* CI: a clone without
  `--recurse-submodules` leaves `.harness/` empty, and a pinned submodule SHA can become
  unreachable upstream. The auto-harness CI **cannot see either by construction** — the
  harness *is* the upstream the consumer pins to, so it has no consumer-side clone to test.
- **Observation:** This is a distinct enforcement topology from everything else in the
  catalog. Every other validator runs in the producer's (harness's) own CI against the
  harness's own tree. But a *cross-repo integration contract* has failure modes that live
  entirely in the consumer's environment — invisible to the producer no matter how many
  validators it runs on itself. The only mechanical check is one the producer **ships into
  the consumer's CI** as a copy-pasteable primitive (here, the `submodule-smoke-test` job
  in the consumer CI templates) that then runs on the consumer's every PR. The producer
  can't run the check; it can only *author and distribute* it. This is the same shape as
  `npm pack` smoke tests, container cold-pull verification, and Homebrew clean-VM taps —
  "consume your own artifact in a clean environment" — but the artifact you ship is the
  *test itself*, not a fix.
- **Implication:** When a harness (or any producer of a consumed artifact) has a contract
  whose failure surface is on the *consumer* side, don't reach for another producer-side
  validator — it structurally can't observe the failure. Ship the check as a templated
  primitive into the consumer's CI, with clean-checkout semantics (a separate job, not a
  step that inherits hidden local state) and an actionable failure message. The producer's
  job shifts from *running* the enforcement to *authoring and distributing* it. The
  intra-repo sibling of this gap (catalog-index declarations without inbound checks) was
  closeable producer-side and is already handled by `validate-list-completeness.sh`; the
  cross-repo instance fundamentally is not.
- **Severity:** Low — an enforcement-topology lesson; reaching for a producer-side
  validator here would have built something that can never fire on the real failure.
- **Contributed-by:** Claude (OPP-0025 implementation, 2026-06-28)

### For a validator family, the per-instance logic is trivial but propagation is O(N × surfaces) — bundle the family in one PR so each surface is touched once, not N times

- **Context:** Implementing PRD-0032 shipped three sibling validators
  (`validate-foundry-target.sh` / `validate-model-routing.sh` /
  `validate-agent-defense-in-depth.sh`) in **one** PR. The three scripts are
  near-identical copies of the `validate-trace-contract.sh` skeleton — each differs only
  in its artifact path and a small invariant block (≈ 30 lines of real logic per script).
- **Observation:** The validator *logic* was the cheap part. The dominant cost was
  **propagation**: every new validator must be wired into ~8 surfaces (CI workflow,
  AGENTS.md, the harness-governance SKILL chain + a bullet, `validators/README` table +
  run-list + test-list, `kernel/base` catalog, root README table + mermaid + word-form
  counts, SUMMARY list + count) and bump the validator count everywhere
  `validate-catalog-counts` asserts it. Done as three separate PRs, that is *3 × 8*
  surface visits — re-opening the same README table, the same CI file, the same count
  sites three times, with three count bumps (21→22→23→24) each needing its own
  reconciliation. Bundled in one PR, each surface is edited **once** (add three rows to
  the README table in a single edit; one count bump 21→24), and the propagation is thought
  through holistically instead of re-derived three times.
- **Implication:** The bundling decision the design PRD made (one change-class → one PRD)
  pays off concretely at *implementation* in **propagation amortization**, not just design
  tidiness. When a family of near-identical artifacts/validators/modules is in flight,
  bundle the implementation too — the marginal cost of "one more instance" is almost
  entirely wiring, and wiring amortizes when the surfaces are open once. (The boundary:
  bundle only when the instances are genuinely a family that touches the *same* surfaces;
  unrelated work in one PR just inflates the diff without amortizing anything.) The
  per-instance logic being trivial is *why* the propagation dominates — and is the signal
  that a single bundled PR is the right unit.
- **Severity:** Low — a PR-batching / cost-model lesson; three separate PRs would have
  triplicated the propagation work and the count-bump reconciliation for ~90 lines of
  shared-skeleton logic.
- **Contributed-by:** Claude (PRD-0032 implementation, 2026-06-28)

### When N artifacts share a validator shape, the change-class is the shape, not the artifact — collapse to one PRD + one skeleton + a per-instance invariant table

- **Context:** After shipping `validate-trace-contract.sh` (PRD-0031), three more cluster
  artifacts (`foundry-targets.md`, `model-routing.md`, `agent-defense-in-depth.md`) each
  need a content validator. The reflexive move is three PRDs mirroring PRD-0031 — but they
  differ only in *which artifact* and *which invariants*; the skeleton (Bash 3.2 + Ruby
  `YAML.safe_load`, 3-state exit, `--scan-file`, the requirement-set gate, predict-clean)
  is byte-for-byte identical.
- **Observation:** Three near-identical PRDs would be ~90% duplicated boilerplate, and
  § 7 (Align File Boundaries with Change-Class Boundaries) says the boundary should follow
  the *change-class*. Here the change-class is **the artifact-content-shape validator
  pattern**, not any one artifact — so the three are *one* change-class with three
  instances. The design collapses to **one PRD (PRD-0032) that references the skeleton
  precedent once and carries a per-instance invariant table** (validator → artifact →
  gate → the 1–3 shape assertions). Implementation can likewise be one PR (three small
  validators from the shared skeleton) or staged, but the *design* is singular.
- **Implication:** The unit of a PRD is a change-class, and a *pattern instantiated N
  times is one change-class*, not N. When you catch yourself about to write the second
  near-copy of a design doc, stop and fold it into a family PRD with a per-instance table —
  the skeleton is stated once, the differences are tabulated, and the reader sees the
  family as a family. This is the design-doc analog of factoring a shared function instead
  of copy-pasting it three times. (Inverse caution: only fold when the skeleton is truly
  identical — if instances diverge in mechanism, they are genuinely separate classes and
  deserve separate PRDs. Here the only per-instance variation is a small invariant set, so
  folding is correct.)
- **Severity:** Low — a PRD-granularity / change-class lesson; three separate PRDs would
  have triplicated the skeleton and obscured that this is one pattern, three instances.
- **Contributed-by:** Claude (PRD-0032 design, 2026-06-28)

### Gate a content validator on the artifact's requirement-set, not a single module id — so it stays correct as more modules come to share the artifact

- **Context:** `validate-trace-contract.sh` (PRD-0031 implementation) is the artifact-content
  validator for `docs/observability/trace-contract.md`. The obvious mirror of
  `validate-sast-coverage.sh` would gate it on one module id (`agent-observability`, which
  owns the artifact). But the cluster's deferred-dependency wiring means a *second* module,
  `ai-foundry-target`, also lists the same artifact in its `requiredArtifacts`.
- **Observation:** Gating on a single module id would silently skip the check for a consumer
  who activates `ai-foundry-target` but not `agent-observability` — yet that consumer still
  has a required `trace-contract.md` that should be well-formed. The correct gate is
  **"activate when *any* active module declares the artifact in `requiredArtifacts`"**
  (`active_modules.select { |m| Array(m["requiredArtifacts"]).include?(ARTIFACT_REL) }`),
  not a hardcoded module name. This keys the validator off the *artifact's requirement-set*
  — the same set `validate-required-artifacts` already computes — so it stays correct
  automatically as more modules come to require the same artifact, with no edit to the
  validator. `validate-sast-coverage` could hardcode its module id only because its artifact
  has exactly one owner; a *shared* artifact needs the set-based gate.
- **Implication:** When a content validator targets an artifact that more than one module
  can require (the norm inside a cluster wired by the deferred-dependency model), derive the
  activation predicate from `requiredArtifacts` membership, not a module id. The module-gated
  pattern has two variants: **single-owner** (gate on the id, like sast-coverage) and
  **shared-artifact** (gate on the requirement-set, like trace-contract). Pick by whether the
  artifact has one owner or many requirers. The set-based gate is strictly more general and
  is the safer default for any cluster artifact.
- **Severity:** Low — a validator-gating-design lesson; the single-id gate would have left a
  real activation path (foundry-target-only consumers) silently unchecked.
- **Contributed-by:** Claude (PRD-0031 implementation, 2026-06-28)

### A § 10 claim decomposes into sub-axes with independent enforcement states — classify per-axis, not per-claim, and the classification table is where you draw the buildable/deferred line

- **Context:** PRD-0031 (the trace-contract content validator) is the first v2-enforcement
  PRD for the frontier-agent cluster. Its § 10 table splits what PRD-0014 carried as one
  Half-enforced claim ("the trace contract is honored") into **two rows**: C-TRACE-1 (the
  contract is internally well-formed — pins a version, declares a conventional span) and
  C-TRACE-2 (the declared spans match the emitted telemetry).
- **Observation:** The two sub-axes have **different enforcement states and different
  readiness**. C-TRACE-1 (artifact-shape) goes to **Enforced** — a content validator can
  assert it today with no consumer code, like `validate-sast-coverage`. C-TRACE-2
  (runtime-conformance) stays **Asserted-only** — it needs a fixed consumer code path that
  a declarative overlay lacks. The original "Half-enforced" wasn't a single dial at the
  midpoint; it was *two dials* — one at "enforceable now," one at "deferred" — averaged
  into one label. Decomposing the claim in the § 10 table is what makes the buildable line
  visible and turns "Half-enforced" from a vague midpoint into a precise statement of which
  axis is covered.
- **Implication:** When a § 10 claim reads "Half-enforced," ask *which axis*. A claim about
  a declared artifact almost always has at least a **shape axis** (is the declaration
  well-formed? — usually enforceable with a content validator) and a **conformance axis**
  (does reality match the declaration? — usually needs the thing being described, often
  deferred). Classify and enforce per-axis. The § 10 table is not just documentation — it
  is the **design tool** that locates the next buildable validator (the highest sub-axis
  currently sitting at Asserted-but-enforceable). This is the per-claim sibling of the
  "deferred-to-v2 is not a monolith" backlog lesson below: there, split the deferral; here,
  split the claim.
- **Severity:** Low — a § 10-classification-discipline refinement; conflating the sub-axes
  is what made the v2 work look monolithic and un-startable.
- **Contributed-by:** Claude (PRD-0031 design, 2026-06-28)

### "Deferred to v2" is not a monolith — harvesting a deferral means re-checking which part now clears the bar that caused the deferral, and splitting on that line

- **Context:** All four frontier-agent-cluster satellites deferred "enforcement" to a v2
  OPP. Opening that thread (OPP-0051), the lazy move is to file "build the four
  validators + companion rules." But the reason v1 deferred was **well-formedness** (a
  declarative architecture overlay has no fixed consumer code path to anchor a companion
  rule on — see the OPP-as-hypothesis observation below), and that reason applies
  **unevenly** across what "enforcement" means.
- **Observation:** "Enforcement" of these artifacts splits cleanly into two halves with
  *different* readiness: (1) **artifact-content / shape conformance** — is the declared
  artifact internally well-formed (pins a version, declares a span in the conventional
  shape)? — which needs **no consumer code** and is **fixture-testable today**, exactly
  like the shipped `validate-sast-coverage` validates `sast-coverage.md` frontmatter; and
  (2) **code-cross-reference** — does the declaration match the running code? — which
  still needs a consumer's fixed code path and stays deferred. The original deferral
  blocked (2), not (1). Treating "v2" as one lump either re-defers buildable work or
  commits to unbuildable work; the right move is to **split on the exact predicate that
  caused the deferral** and ship the half that now clears it.
- **Implication:** When harvesting any "deferred to vN" item, don't re-litigate the whole
  deferral — re-derive the *specific* blocker (here: "no fixed code path") and test each
  sub-capability against it. Often a deferral that read as monolithic at defer-time has a
  fixture-testable sub-part that became (or always was) buildable. The shipped precedent
  to copy is the closest existing validator of the same shape (`validate-sast-coverage`
  for an artifact-content check), not a from-scratch design. This is the harvest-time
  analog of § 9/§ 10: split the buildable contract from the unbuildable one and ship the
  buildable half predict-clean.
- **Severity:** Low — a backlog-harvest-discipline lesson; the lump reading would have
  over-scoped the OPP to four validators + four companion rules, half of them still
  unbuildable.
- **Contributed-by:** Claude (OPP-0051 design, 2026-06-28)

### A conditional artifact requirement the schema can't express belongs in a reviewGate + optionalArtifacts — schema-permissive, human-enforced — not forced into required or dropped

- **Context:** `architectures/agent-defense-in-depth` (PRD-0030 implementation) needed
  its second artifact, `append-only-action-log.md`, to be **required *only when* the
  project declares any autonomous (non-draft) action** — the audit-substantiation of
  pattern #4. A `module.yaml` has exactly two levers: `requiredArtifacts` (always
  required when the module is active) and `optionalArtifacts` (never required). Neither
  expresses "required *if* the project has property X," and the harness cannot see
  property X (whether a consumer's agent acts autonomously) from the manifest.
- **Observation:** The right encoding is **`optionalArtifacts` (so the schema never
  over-requires the artifact for a draft-only product) + a `reviewGates` entry that
  states the conditional** ("when the project declares any autonomous action, the action
  log is required-by-convention; a non-autonomous product may mark it not-applicable").
  This keeps the mechanical validator honest (it only checks what is unconditionally
  true — the module is active, so the *primary* artifact must exist) while the
  conditional, judgment-dependent requirement lives where a human enforces it. Forcing it
  into `requiredArtifacts` would break every draft-only adopter; dropping it to plain
  optional would let an autonomous product silently skip its audit log.
- **Implication:** This is the § 10 classification discipline applied to *artifact
  requirements*: a claim that depends on a project property the harness can't observe is
  **Half-enforced by design** — the schema enforces the unconditional part, a review gate
  carries the conditional part. The same shape recurs whenever a requirement is
  contingent on consumer behavior (autonomous-vs-draft, handles-PHI-or-not,
  is-production-or-not). Reach for a reviewGate before contorting the schema or
  abandoning the requirement.
- **Severity:** Low — an artifact-modeling lesson; both naive encodings (force-required
  or plain-optional) are wrong in opposite directions.
- **Contributed-by:** Claude (PRD-0030 implementation, 2026-06-28)

### Closing a deferred-dependency loop is itself deferred — shipping the owner module does not retroactively tighten the consumer's optional→required artifact

- **Context:** `architectures/intelligent-model-routing` (PRD-0029 implementation) ships
  the module that **owns** `docs/architecture/model-routing.md` — the very artifact the
  earlier-shipped `architectures/ai-foundry-target` lists as **optional** under the
  deferred-dependency model (require the shipped sibling's artifact, make the unbuilt
  sibling's artifact optional). The intuitive move on shipping the owner is to "close
  the loop" by flipping foundry-target's `model-routing.md` from optional to required.
- **Observation:** That tightening was **deliberately not done** in this PR, and
  shouldn't be. Flipping an existing module's `optionalArtifacts` → `requiredArtifacts`
  is a **breaking change for every consumer that already activates foundry-target** —
  their previously-valid project would suddenly fail `validate-required-artifacts`.
  PRD-0028 already named this correctly: the artifact "moves from optional to expected"
  as a **v2 / companion-rule concern for foundry-target**, not a side effect of shipping
  its sibling. So the loop closes in *capability* (the artifact now has an owning module
  and a template) without closing in *enforcement* (foundry-target still treats it
  optional until a deliberate, separately-versioned change).
- **Implication:** The deferred-dependency model has a symmetric exit as well as a
  symmetric entry. Entry: don't couple to unbuilt work (make the sibling's artifact
  optional). Exit: when the sibling ships, don't retroactively harden the coupling in the
  same PR — that converts an additive release into a breaking one. Hardening optional→
  required is its own opt-in change-class (§ 7), gated behind its own version bump and
  ideally a companion rule that only fires for projects that opt into both overlays. A
  cluster tightens its internal coupling **monotonically and deliberately**, never as a
  silent consequence of a sibling landing.
- **Severity:** Low — a release-discipline lesson; the wrong move would have shipped a
  breaking change disguised as an additive one.
- **Contributed-by:** Claude (PRD-0029 implementation, 2026-06-28)

### An OPP's proposed enforcement mechanism is a hypothesis the PRD ratifies against shipped precedent and well-formedness — not a spec to transcribe verbatim

- **Context:** The design pass for the last two frontier-agent-cluster satellites
  (PRD-0029 model-routing / OPP-0030, PRD-0030 defense-in-depth / OPP-0031). Both OPPs,
  written months earlier, **proposed a companion rule** ("edits to routing-decision /
  action-execution code require a matching artifact update"). Taken verbatim, each PRD
  would have specified an *enforcing* v1.
- **Observation:** Two things made "defer the companion rule to v2" the correct
  ratification, and both are only visible at design time against current `main`:
  (1) **shipped precedent** — the two already-built siblings (`agent-observability`,
  `ai-foundry-target`) both shipped declarative-v1 with no companion rule, so an
  enforcing v1 here would make the cluster internally inconsistent; and
  (2) **well-formedness** — a declarative *architecture overlay* has **no fixed consumer
  code path** to anchor a companion rule's `triggerPaths` regex on (unlike a management
  overlay governing known doc paths). The OPP-proposed rule literally cannot be authored
  as a well-formed companion rule at v1. The OPP's mechanism was a *direction*, not a
  buildable spec.
- **Implication:** Treat the "Companion rule:" / "validator:" lines in a proposed OPP as
  **hypotheses to test at PRD time**, the same way agent-synthesized designs are drafts
  to check against the source of truth (see the build-map lesson below). The PRD pass
  reconciles the proposal against (a) what sibling modules actually shipped and (b)
  whether the mechanism is even expressible in the schema — and records the deferral
  explicitly in Non-Goals with the reasoning, so the v2 OPP inherits a clear contract.
  This is the design-time analog of "declare first, enforce later": the OPP declares the
  *intent* to enforce; the PRD decides *whether v1 can*.
- **Severity:** Low — a PRD-authoring-discipline lesson; following the OPPs verbatim
  would have shipped two modules inconsistent with their own cluster.
- **Contributed-by:** Claude (PRD-0029 + PRD-0030 design, 2026-06-27)

### The second module in a cluster, built back-to-back with a fresh sibling, is the lowest-drift build there is — copy-exact from a sibling-on-disk that resolved every convention one PR earlier

- **Context:** `architectures/ai-foundry-target` (PRD-0028 implementation) shipped
  immediately after its sibling `architectures/agent-observability` (PRD-0014,
  merged the same day). The PRD explicitly framed it as "mirrors `agent-observability`
  exactly," and the build bore that out: same id form (bare `ai-foundry-target`), same
  `type: architecture` / `stability: beta` / `dependsOn: [kernel/base]` shape, same
  `validators: [validate-required-artifacts, validate-companions]`, same declarative-v1
  (no companion rule / no validator), same propagation surface set.
- **Observation:** Building a second cluster module *right after* the first is the
  inverse of finalizing a month-old PRD. The high-drift case (canonical-position,
  agent-observability finalization) is a stale spec reconciled against a moved catalog —
  the convention traps (`type: architectures`→`architecture`, bare-id vs prefixed,
  newly-required `stability`, concern-named-not-type-named template subdir) all surface
  at implementation. Here those questions were **already answered on disk one PR
  earlier**, so the second build is a near-mechanical mirror with effectively zero
  convention drift. The only genuinely new decision was template placement, and even
  that followed the established rule: a concern template subdir **mirrors the
  artifact's `docs/` destination directory** (`docs/architecture/foundry-targets.md`
  → `templates/architecture/foundry-targets.md`, a new subdir + its
  list-completeness row), exactly as `docs/observability/` → `templates/observability/`.
- **Implication:** When a cluster has multiple modules to build, **sequence the
  siblings back-to-back** rather than interleaving unrelated work. The first build pays
  the convention-resolution cost; each subsequent sibling inherits the resolved
  conventions from a fresh on-disk exemplar, minimizing per-module drift and review
  load. The deferred-dependency wiring (requiring the shipped sibling's
  `trace-contract.md`, making the unbuilt `model-routing.md` `optional`) also carries
  **zero enforcement cost on the harness's own CI** — `validate-module-graph` checks
  only `dependsOn` resolution (kernel/base), and `validate-required-artifacts` is
  predict-clean because the harness never activates the module, so the cross-artifact
  coupling is invisible until a consumer opts in.
- **Severity:** Low — a build-sequencing and convention-inheritance lesson, applied
  cleanly with a green-first-push expectation.
- **Contributed-by:** Claude (PRD-0028 implementation, 2026-06-27)

### A cluster of mutually-reinforcing modules builds incrementally via the deferred-dependency model — require the shipped sibling's artifact, make the unbuilt one optional

- **Context:** The frontier-agent cluster (OPP-0027 anchor + observability /
  foundry-target / model-routing / defense-in-depth satellites) is *mutually
  reinforcing* — each satellite's value compounds with the others. PRD-0028
  (foundry-target) wants evidence artifacts owned by two siblings at different build
  stages: `trace-contract.md` (OPP-0029, **shipped**) and `model-routing.md`
  (OPP-0030, **not built**).
- **Observation:** The naive design — require all sibling artifacts — would block
  each satellite on *all* the others, forcing a strict build order or a flag day.
  The resolution is the **deferred-dependency model**: a satellite **requires the
  artifact of any sibling that has shipped** and **lists the artifact of any unbuilt
  sibling as `optional`** (it becomes required-by-convention, or a companion-rule
  check, once that sibling ships). So foundry-target requires `foundry-targets.md` +
  `trace-contract.md` and makes `model-routing.md` optional — buildable today,
  tightening automatically as the cluster fills in.
- **Implication:** This is the composition analog of the predict-clean /
  declarative-v1 patterns: don't couple a module to work that doesn't exist yet.
  Mutual reinforcement is expressed through *optional* cross-artifact references that
  harden over time, not a rigid dependency chain. A cluster designed this way can be
  built in *any* order, each piece shippable on its own, with the connective tissue
  (cross-references, later companion rules) added as siblings land. The opposite
  failure — over-coupling — would have made the whole cluster un-shippable until the
  last module existed.
- **Severity:** Low — a composition-design lesson, applied cleanly at PRD-time.
- **Contributed-by:** Claude (PRD-0028 design, 2026-06-27)

### Agents are reliable for research and mapping, unreliable for invention — verify their output against the source of truth, not the other way round

- **Context:** Building the agent-observability module, two subagents ran in
  parallel: one web-verified the current OpenTelemetry GenAI semantic conventions,
  one produced a "convention-correct build map" by reading sibling architecture
  modules.
- **Observation:** The split in reliability was sharp and instructive. The **web
  research was excellent and load-bearing** — it caught that the conventions moved
  to a dedicated repo, are still Development/Experimental, and underwent attribute
  renames that made the PRD's months-old example version pin stale; grounding the
  template at *implementation* time (not draft time) is what kept it accurate. The
  **build-map agent's *factual reads* were accurate** (line numbers, count sites,
  the propagation surfaces) — but its ***invented* module.yaml contradicted the
  PRD**: it added companion rules the PRD explicitly defers to v2, the wrong
  required artifacts, and three differently-named templates against the PRD's two.
  An agent asked to "produce a build map" will helpfully *fill in* a plausible
  design — which is exactly where it diverges from the authoritative spec.
- **Implication:** Use agents for what they're good at — **fan-out research** (web,
  multi-source) and **mechanical mapping/auditing** (find the sites, read the
  conventions, check disk) — and treat their *synthesized designs* as drafts to
  check against the source of truth (the PRD, the sibling on disk), never as the
  spec. The rule "verify agent output against disk/PRD before acting" earned its
  keep twice this session (the PRD-backlog review's mis-statements, and this build
  map). The reliable division of labor: agents gather and map; the controller
  decides and authors against the contract.
- **Severity:** Medium — following the invented build map verbatim would have
  shipped a module contradicting its own accepted PRD.
- **Contributed-by:** Claude (PRD-0014 implementation, 2026-06-27)

### Each always-on structural validator becomes a constraint every later module must satisfy — the governance floor rises under drafted PRDs

- **Context:** Finalizing PRD-0014 (agent observability, drafted 2026-05-26) for
  acceptance. It is a month-old design artifact, so the usual finalize-before-accept
  drift check applied. But one correction was not "drift" in the
  rename/recount sense — it was a *brand-new requirement that did not exist when the
  PRD was written.*
- **Observation:** PRD-0014's new module must now declare `stability: beta` —
  because `validate-module-stability.sh` (always-on, shipped 2026-06-26 via PRD-0027,
  *one day* before this finalization) blocks any `module.yaml` lacking the field.
  The draft could not have anticipated it. So finalize-before-accept has to
  reconcile against three classes now: (1) renamed/recounted **drift** (validator
  count 8→20, `type: architectures`→`architecture`), (2) reassigned **reservations**
  (the deferred §9 promotion → §13), and crucially (3) **newly-required fields** that
  *postdate* the draft. The catalog's own governance floor rises over time: every
  always-on structural validator added (publication-boundary, module-stability)
  becomes a gate every subsequent module and PRD must pass.
- **Implication:** A drafted PRD ages against a *moving* contract, not a static one.
  The reconciliation pass must re-check the design against the validators and
  required fields that exist *now*, not only against the counts and reservations the
  draft named. The longer a PRD waits, the more the floor will have risen beneath it —
  which is another argument for short draft→accept latency (the design-artifact-
  staleness candidate stub), and a reason the implementation should always mirror a
  *current* sibling module rather than the PRD's prose.
- **Severity:** Low — caught at finalization; the fix is one added field.
- **Contributed-by:** Claude (PRD-0014 finalization, 2026-06-27)

### An always-on structural validator forces its own backfill — the requirement and its satisfaction are inseparable in one PR

- **Context:** Module stability (PRD-0027) shipped as an *always-on* validator
  (`validate-module-stability.sh`) — a catalog check like
  `validate-list-completeness`, run against the harness's own modules — rather than
  a *predict-clean* module-gated validator (which no-ops on the harness because the
  harness doesn't activate the gated module).
- **Observation:** The two postures have opposite implementation shapes. A
  predict-clean validator (privacy / twin / work-package) ships **green on an empty
  harness** — the field it requires need not exist anywhere yet, because the
  harness never activates the module. An always-on structural validator ships
  **red until every entity complies** — the moment it lands, the harness's own 57
  modules must *all* declare the field or CI fails. So the validator and the
  full backfill are **one inseparable PR**: you cannot land the requirement and
  populate it separately. (Publication-boundary was the same shape — always-on,
  and it had to mark the one live brief in the same PR.)
- **Implication:** Choose the posture deliberately. *Always-on* is right for a
  property *every* module/file must have (a readiness tier, a leak gate) — and it
  buys you completeness for free (the validator guarantees no module is ever added
  without the field). *Predict-clean* is right for an *opt-in capability* property.
  And the honest backfill matters: of 57 modules only **10** earned `stable` — the
  ones the harness dogfoods via its own manifest, the strongest battle-testing
  evidence available — with everything-shipped-but-unproven defaulting to `beta`.
  A module-readiness signal is only worth shipping if it resists the pull to
  inflate `stable`; the bright line ("dogfooded-by-manifest OR foundational") is
  what keeps it honest.
- **Severity:** Low — a design-shape lesson, caught and applied cleanly.
- **Contributed-by:** Claude (PRD-0027 implementation, 2026-06-26)

### Evaluate an outside-in review against the controls that already exist — then distill the surviving signal, don't execute the template

- **Context:** An external (ChatGPT-lane) leadership review framed `auto-harness` as
  a "growing platform drifting toward a catalog monster without enough
  normalization, parity, and ergonomic control," and asked for a nine-section
  program: module maturity tiers, required metadata, compatibility declarations,
  composition rules, naming discipline, deprecation policy, etc.
- **Observation:** Checked each recommendation against the actual files. Most were
  **already built**: required metadata is uniform (`version`/`summary`/
  `conflictsWith`/`validators`/`companionRules`/`reviewGates` in 57/57 modules);
  compatibility declarations exist (`dependsOn` + `conflictsWith`, enforced by
  `validate-module-graph`); composition is governed (§ 12's three shapes +
  `compositions/`); "when is a new module justified" is the § 12 inclusion test;
  enforcement-to-policy parity is § 10 (Enforced / Half-enforced / Asserted-only).
  The review was an outside-in template that pattern-matched growth → sprawl
  *without seeing the controls* — it hedged with "appears to / may be" throughout.
  Only ~10% survived: there is genuinely **no module-readiness signal**
  (`stability` in 0/57), and `stacks/` is 3/4 JS-family. Filed that as OPP-0050.
- **Implication:** A growing platform attracts "you must be under-normalized"
  reviews; the discipline is to **verify each claim against the machinery that
  already exists** before acting. Executing the template wholesale would have
  rebuilt things we have and generated the exact taxonomy-museum the review warned
  against. The value of an external review is the *narrow gap it surfaces that you
  were blind to* — distill that, record the verdict on the rest honestly, and let
  the already-built parts stand. Module **stability** is a clean addition because it
  is a *third independent axis* (trust tier = risk; § 10 = per-claim enforcement;
  stability = battle-testedness) — it fills a real hole rather than duplicating one.
- **Severity:** Medium — the failure mode (dutifully executing a generic review) is
  expensive motion, not a defect.
- **Contributed-by:** Claude (OPP-0050 filing / external-review evaluation, 2026-06-26)

### Even a freshly-finalized PRD gets module conventions wrong — the implementation pass is where they surface

- **Context:** PRD-0007 was finalized against `main` one step before implementation
  (counts re-derived, stale numbers fixed, scope reconciled). Yet building the
  module from it still hit two convention mismatches the finalization hadn't caught,
  because they require reading sibling *modules*, not the catalog *counts*.
- **Observation:** (1) The PRD specified `id: management/canonical-position` and
  `dependsOn: [management/project-standard]`, but every module id in the catalog is
  **bare** (`canonical-position`, `project-standard`) and `dependsOn` references the
  bare id — the `management/` prefix is the *directory/type*, not part of the id.
  (2) The PRD specified `templates/management/…`, but there is **no type-grouped
  `templates/management/` subdir** — every template subdir is *concern-named*
  (`templates/program/`, `templates/knowledge/`, `templates/work-package/`), so the
  templates belong under `templates/canonical-position/`. Both are invisible to a
  finalization that only re-derives counts; they only surface when you mirror an
  actual sibling module on disk.
- **Implication:** Finalize-before-accept reconciles *catalog-level* drift (counts,
  reserved numbers, section references); a separate class of *convention* drift
  (id/dependsOn form, subdir-naming, field shapes) only surfaces at implementation,
  by copying the nearest real sibling rather than the PRD's prose. Build modules by
  mirroring an existing module of the same type, treating the PRD as the *what* and
  the sibling as the *how*. This is the implementation-time companion to the
  design-artifact-staleness candidate stub.
- **Severity:** Low — caught immediately by reading siblings; predict-clean meant
  no CI fallout. Worth recording so the next module build mirrors-a-sibling first.
- **Contributed-by:** Claude (PRD-0007 implementation, 2026-06-26)

### A drafted-but-unaccepted PRD is a drift liability — "finalize" must mean reconcile, not rubber-stamp

- **Context:** OPP-0007's PRD was drafted 2026-05-24 and sat at `Proposed` for a
  month while the catalog moved underneath it (5 validators → 19; templates 56 → 90;
  operating-principles §§ 9–12 added; ADR/diagram numbers reserved then reassigned).
  Finalizing it for acceptance required deciding whether "finalize" means flip the
  status or actually reconcile the body.
- **Observation:** Every forward-looking number and reservation in the draft had
  rotted: it cited "8 validators," catalog bumps `26→27` / `56→58` (vs. the real
  47 / 90), an anticipated "Diagram 10" and "ADR-0013" that were both since
  reassigned to other work, a stale v0.6.0/v0.7.0 release sequence (both shipped),
  and an FR (FR-006) that bundled an operating-principle promotion to a "§ 9" slot
  now occupied by a different principle. None of this is caught by a validator —
  PRDs aren't on any count or list-completeness surface — so a rubber-stamp
  acceptance would have carried all of it into the implementation PR as a spec the
  builder would dutifully (and wrongly) follow.
- **Implication:** Treat finalize-before-accept as a real reconciliation pass:
  re-derive counts against `main`, re-check every reserved ADR/diagram/section
  number, and re-test any *bundled* scope against principles added since the draft
  (here § 7 "Align File Boundaries" retroactively condemned FR-006's change-class
  mixing, so it was split out). This is the same discipline a stale copy-exact plan
  needs before execution (the cybersec-renumber rebase) — applied to a PRD instead
  of a plan. The longer a design artifact waits between draft and acceptance, the
  more reconciliation it needs, not less.
- **Severity:** Medium — silent spec-rot that a builder would faithfully propagate.
- **Contributed-by:** Claude (OPP-0007 / PRD-0007 finalization, 2026-06-26)

### Always-on vs predict-clean is a who-is-protected decision, not a stylistic one

- **Context:** Implementing PRD-0026's publication-boundary validator raised the
  wiring question every new validator faces here: is it **module-gated /
  predict-clean** (exits 0 when its module is inactive, like
  `validate-privacy-by-design` / `validate-twin-profile` / `validate-lane-integrity`)
  or **always-on** (runs for every harnessed project, like `validate-placeholders`)?
  The catalog's recent validators were all predict-clean, so the default pull was
  to make this one predict-clean too.
- **Observation:** That would be a latent hole. A predict-clean validator no-ops
  for any project that does not activate its module — which is correct for an
  **opt-in capability** (privacy, digital-twin, work-package: you only pay for what
  you turn on). But a **safety floor** must protect the projects that never opt in,
  precisely because those are the ones most likely to leak. A publication gate that
  only fired when a `publication-boundary` module was active would be absent from
  the repos that most need it. So the posture is dictated by *who the control
  protects*: opt-in capability → predict-clean; universal safety floor → always-on,
  wired into `kernel/base` and run unconditionally in CI.
- **Implication:** When adding a validator, ask "does this protect a consumer who
  opted into a feature, or every consumer regardless?" before copying the
  predict-clean template. The predict-clean pattern is the catalog's recent
  default, but it is the wrong default for safety controls.
- **Severity:** Medium — a mis-classified safety validator looks green while
  protecting nobody.
- **Contributed-by:** Claude (PRD-0026 implementation, 2026-06-25)
...
- **Context:** OPP-0046 triages two field reports (#121 from Codex, #122 a multi-agent observation) from live parallel cycles — Claude planning, Codex + Gemini executing in isolated git worktrees (a multi-agent planning project, `a municipal-web project`). The recurring failure was reconciling a *prose* work-package boundary against acceptance criteria, named symbol locations, worktree setup, and per-tool defaults.
...
- **Context:** In a consumer project's sibling draft worktree, when using clickable local `file:///` paths inside `docs/README.md` or a draft ADR, the `validate-doc-references.sh` validator failed to parse them as an external protocol, appending each as a relative link and erroring out.
...
- **Context:** Staging document files in a consumer project's draft worktree failed on `validate-skill-content.sh` because the validator triggered on security-training content containing standard prompt-injection keywords (e.g., system-level instructions in `SKILL.md` documents).
...
### A breaking rename whose blast radius is 100% in-repo is cheaper to cut over atomically than to bridge with alias machinery

- **Context:** OPP-0012 renamed the relational data module `data/relational-postgres` → `data/relational-sql` (the SQL engine now declared in the artifact, not baked into the id). The OPP's proposed mitigation was a deprecated-alias facility so old manifests keep resolving. Disk verification found the entire blast radius in-repo: 4 compositions, 1 module `dependsOn` (`domains/supabase`), 2 sample manifests, and ~30 prose references — and **no external manifest on the old id** (the motivating brownfield consumer declares no data module at all).
- **Observation:** When every reference lives in the same repo, a hard atomic single-commit rename is the low-risk path and an alias/deprecation facility is net-new machinery serving *nobody* — there is no external consumer to bridge. CI (composition resolution + the validator chain) proves the cutover is clean in one commit; a half-migrated tree fails fast rather than resolving to a stale id silently. The OPP's own "migration cost compounds with delay" argues for acting precisely while adoption is 100% internal — every consumer added before the cutover is one more edit later.
- **Implication:** For a breaking identifier change, *first* establish whether the blast radius is fully in-repo (grep every consumer surface — compositions, `dependsOn`, sample manifests, prose). If yes: atomic cutover, no alias, major-version bump, all consumers migrated in the same commit; reserve alias/deprecation machinery for when an external consumer has already adopted the old id. Companion insight from the same change: because `module.schema.json` is `additionalProperties: false`, engine-like sub-fields belong **in the required artifact** (the foundries-enum precedent), not on `module.yaml` — generalizing a module's identity need not touch the closed core schema.
- **Confidence:** high. The blast radius was disk-verified; the closed-schema and no-alias-facility facts were confirmed against `module.schema.json` and `harness_registry.rb`.
- **Severity:** process
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-09 (satisfies the PRD-0004 distillation rule fired by the `data/relational-sql/module.yaml` rename + the OPP-0012 status flip; substantive connection — names the in-repo-blast-radius / atomic-cutover-vs-alias economics and the closed-schema engine-in-artifact rule, rather than restating the rename)

### An inter-agent contract splits cleanly into a consumer authority-of-record and a harness enforcement half — and a field-defect pair is the strongest promotion evidence

- **Context:** OPP-0052 harvests the **review-lane** substrate (provider-neutral verdict schema + `validate-coordination-verdicts.sh` + a `platform/templates/coordination/` scaffold) as the enforcement complement to OPP-0046's scope-lane. Its design was ratified upstream not in the harness but in a *consumer supervisor repo's* ADR ("Monolithic Core, Federated Adversaries"): the consumer ADR is authority-of-record, the harness OPP/validator is the enforcement mechanism every governed repo then inherits.
- **Observation:** A cross-provider coordination contract naturally stratifies into two layers with different homes. The *design authority* belongs where the multi-agent workstream is adjudicated (a consumer supervisor), because that is where the writer-of-record and the adjudicated ledger live; the *enforcement* belongs in the harness, because a validator + template is the only way sibling repos inherit the substrate rather than re-deriving it. Ratifying the design in the harness would over-reach (the harness does not adjudicate any one workstream); enforcing it only in the consumer would strand every other repo. The two OPP-0046/OPP-0052 halves (scope + review) are the same declare-then-enforce contract retargeted onto two seams of parallel multi-agent work.
- **Implication:** When harvesting a coordination pattern, locate the authority-of-record and the enforcement mechanism *separately* — do not force both into one repo. And treat a concrete field-defect pair as the promotion bar: OPP-0052's two Enforced rules (canonical shared `taskId` binding; the decorrelated-coverage check — with decorrelated-provider *routing* the asserted companion) are each the fix for a specific observed failure (verdict label-swap; core-only adjudication) in a real federated-review cycle — "field-proven before authored" is a stronger Origin/Evidence basis than a speculative framework, and each rule having a concrete failure it must catch *frames* the Enforced-vs-asserted split the PRD then finalizes (the coverage *check* is mechanizable; the *routing* that feeds it is behavioral).
- **Confidence:** high. The two-layer split and the defect pair were both taken from an executed cross-provider review cycle, not designed a priori.
- **Severity:** process
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-09 (satisfies the PRD-0004 distillation rule fired by adding OPP-0052 to the opportunity catalog + its `docs/README.md` index row; substantive connection — names the consumer-authority/harness-enforcement stratification and the field-defect-as-promotion-bar economics that OPP-0052 embodies, rather than restating the OPP)

### The observation ledger enforces that a trace exists and connects, but never its shape — so a ratified schema drifted for want of a validator

- **Context:** OPP-0053 reconciles with OPP-0052 and files the knowledge-ledger enforcement half. `management/knowledge-capture` forces two things on any addition to this file — an audit-trail pointer (change-log) and, on ADR/OPP/module edits, a paired distillation entry (PRD-0004) — but never checks the ADR-0002 *shape*. The evidence that presence ≠ conformance is this very ledger: of 105 observations, **62 (59%) carry an off-enum `Severity`** (`process` 28, `low` 15, `medium` 6, `architecture` 8, `programming-discipline` 4, `security` 1), the most-severe canonical level `risk-bearing` is used **0×**, 21 entries misfile `Confidence`-vocabulary (`low`/`medium`) into `Severity`, and ~20% omit `Confidence` or `Contributed by` entirely — including the two entries immediately above this one, which both use the off-enum `process`.
- **Observation:** A declare-then-enforce contract can be **half-installed**: existence and linkage are mechanically enforced while the record's *internal schema* is left unguarded, so a ratified structure rots because ratification is not enforcement. The same species of gate — a *structured-agent-ledger* validator that lints each newly-added record against a declared schema, diff-based against the base branch — is needed on **both** append-only agent ledgers (the verdict ledger of OPP-0052 and this knowledge ledger); but the reuse is best realized as a **named convention, not shared code**, because a Markdown field-parser and a JSON schema-validator barely overlap and a shared library would be a thin abstraction over two dissimilar parsers.
- **Implication:** When auditing a declare-then-enforce module, check that something enforces the **schema itself** and not merely a trace's presence and linkage — otherwise the pair is silently incomplete. Reconcile same-species validators at the **convention layer** (name the species in `stigmergy.md`; keep separate module homes) rather than forcing a shared implementation — concrete-first over premature abstraction. And enforce a ratified enum **as-locked**: drift is the defect, not the schema, so make off-enum values errors rather than widening the enum to bless the drift.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-10 (satisfies the PRD-0004 distillation rule fired by adding OPP-0053 to the opportunity catalog + its `docs/README.md` index row; substantive connection — names the presence-vs-shape enforcement gap and the same-species-at-the-convention-layer reconciliation with OPP-0052, and deliberately uses a canonical `governance-relevant` `Severity` to model the enforce-as-locked norm the OPP proposes, rather than restating it)

### A shape validator on its own ledger closes a dogfood loop — the implementing PR's own distillation entry is the first record it lints

- **Context:** Implementing PRD-0034 shipped `validate-observation-hygiene.sh` and registered it under `management/knowledge-capture` — an **active** module on the harness. Unlike the requirement-set content validators (`validate-trace-contract` etc.), which are predict-clean because the harness activates no requiring module, this validator gates a ledger the harness itself writes to. Editing `knowledge-capture/module.yaml` to register it fired the PRD-0004 distillation trigger, requiring this very observation — which the new validator then lints in the same CI run.
- **Observation:** When a validator gates the same append-only ledger its own change-set must write to, the enforcement is **self-referential in a good way**: the implementing PR cannot merge unless its own mandated distillation entry already conforms to the schema being enforced. The dogfood loop is not a nicety — it is the tightest possible acceptance test, because the PR that introduces the check is the first thing the check runs against. This only holds because the gate is diff-based (grandfathering the non-conformant history) and module-gated on an *active* module; a predict-clean gate would no-op on its own introducing PR and lose the property.
- **Implication:** Prefer to land a ledger-shape validator via a PR that is *itself* forced to emit a conforming record, so the first real-world exercise of the gate is the merge that ships it. More generally, when a validator's trigger surface overlaps its own module's companion `triggerPaths` (here: additions to `shared-observations.md`), PR-#88 self-coverage is automatic and the dogfood is free — look for that overlap when siting a new validator, and register it on the module whose artifact it guards.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-11 (satisfies the PRD-0004 distillation rule fired by adding `validate-observation-hygiene` to `knowledge-capture/module.yaml` + `kernel/base/module.yaml`; substantive connection — names the self-referential dogfood-loop property that a diff-based, active-module-gated ledger validator gains, distinct from the predict-clean content validators, rather than restating the implementation)

### An OPP's acceptance status-flip belongs in the implementing PR, not a standalone closeout — because a lone OPP edit trips the distillation companion for trivial work

- **Context:** OPP-0053's implementation (PR #173) shipped Layer 1 and, per PRD-0034's acceptance criteria, should have flipped OPP-0053 `proposed → accepted` in the same PR — the precedent set by OPP-0012, whose flip rode on its implementing PR #168. That flip was omitted, leaving an accepted-and-shipped opportunity marked `proposed` on `main` until this standalone closeout. But any edit to `docs/opportunities/OPP-*` fires the PRD-0004 distillation-trigger companion, so a bare status flip cannot merge without a paired knowledge entry — forcing a distillation on what is otherwise a one-token change.
- **Observation:** Lifecycle status transitions on a governed artifact (OPP `proposed → accepted`, and its index-row mirror) are **not free-standing edits** under a stigmergic companion regime — they inherit the same "leave a trace" tax as substantive work, because the trigger regex cannot distinguish a status byte from a rewrite. Bundling the flip into the PR that *earns* the transition avoids the tax entirely: that PR already carries a substantive distillation entry, and the flip is a zero-marginal-cost addition to it. A standalone flip, by contrast, must manufacture a satisfier — and the only non-cargo-cult satisfier available is usually the meta-observation that the flip should not have been standalone (this entry).
- **Implication:** When a PRD's acceptance criteria name a status transition ("OPP flips to accepted at implementation-merge"), edit the status field **in that same implementing PR** — add it to the propagation checklist alongside the catalog-count bumps. Reserve standalone status-flip PRs for cases where the transition genuinely post-dates the delivery. And when reviewing a shipped-but-still-`proposed` artifact, check the index-row mirror too: the README status column drifts independently of the OPP-body status (OPP-0012's row still read `proposed` while its body read `accepted`).
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-12 (satisfies the PRD-0004 distillation rule fired by flipping OPP-0053 `proposed → accepted` + its `docs/README.md` index row; substantive connection — the entry *is* the lesson of this closeout, naming why a status flip belongs in the implementing PR and how the index-row status drifts independently, rather than an unrelated observation appended to pass the rule)

### A derived index exempted from companion enforcement drifts silently — 10 of 53 OPP index annotations disagreed with their record's canonical status

- **Context:** Closing out OPP-0053 surfaced one apparently-cosmetic loose end: an index annotation still reading `proposed` for a shipped opportunity. Reconciling every `OPP-NNNN-*.md` record's canonical `Status` field against its annotation in `docs/opportunities/candidates.md` found the drift was systemic, not incidental — 10 entries (OPP-0012 / 0025 / 0027 / 0028 / 0029 / 0030 / 0031 / 0032 / 0051 / 0053) were annotated `proposed` or `exploring` while their records read `accepted`, some for over six weeks, with stale sub-claims riding along (`PRD-0014 in flight`, `PRD-0013 in flight`). ADR-0012 split this directory deliberately: `README.md` is *structural* and ADR-gated, `candidates.md` is *organizational* and explicitly exempt from the companion-rule floor so headings can be regrouped freely.
- **Observation:** The exemption that makes a derived view cheap to edit is the same property that lets it drift, and nothing in the harness reconciles it back. This is precisely the failure `validate-catalog-counts.sh` mechanizes away for *counts* — a number restated in prose diverges from the artifact set unless a validator recomputes it — but the same shape recurs for *status* with no equivalent gate. The drift is not evenly distributed either: it clusters where work moves fastest (the whole frontier-agent cluster OPP-0027..0031 shipped and none of its index annotations followed), so the index is least accurate exactly where a reader most needs it. Worth recording against my own prior entry (2026-07-12), which predicted this drift but misidentified its home as a `README.md` status column — no such column exists; the index is `candidates.md`, and the real scope was ten entries, not one.
- **Implication:** When a decision exempts a derived view from enforcement on the grounds that it is "organizational," price the exemption: the view will drift toward its source's past, and the cost lands on readers rather than authors. Either accept the drift explicitly and stop treating the view as authoritative, or mechanize the reconciliation — a status-parity check comparing each record's `Status` against its index annotation is the natural sibling of `validate-catalog-counts.sh`, is derivable with the same recompute-and-compare recipe, and would have caught all ten. This is a candidate for an OPP, not a silent habit of periodic manual sweeps. Until then, fold the index annotation into the same propagation checklist as the status flip itself, so the mirror moves with the record it mirrors.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-15 (satisfies the PRD-0004 distillation rule fired by updating OPP-0053's `Status` / `Last Updated` / Disposition to record Layer 2's delivery; substantive connection — the reconciliation that closed OPP-0053 is what exposed the systemic index drift, and the entry names the structural cause, corrects a factual error in my own prior entry on the same subject, and proposes the mechanized fix rather than restating the edits)

### An OPP's lifecycle status is mirrored into (at least) two derived index surfaces, not one — and manual reconciliation of a single surface silently leaves the other drifted

- **Context:** Scoping OPP-0054 (a status-parity validator) required enumerating every surface that mirrors an OPP record's `Status`. There are (at least) **two** derived surfaces, in **different** files whose names collide in memory: `docs/opportunities/candidates.md` (a freeform annotation token) and `docs/README.md` (a status *column* in the opportunities index table). A third file, `docs/opportunities/README.md`, is the write-policy doc and carries **no** status rows at all. My PR #177 reconciled 10 drifted `candidates.md` annotations by hand but never touched `docs/README.md`, whose OPP-0012 row still read `proposed` while the record read `accepted` — found only while scoping OPP-0054, and fixed in the same PR as this entry.
- **Observation:** When a source-of-truth field is mirrored into N derived surfaces and none is mechanically reconciled, a human reconciling "the index" fixes whichever surface is top-of-mind and silently leaves the rest drifted — the miss scales with N, and the surfaces are least accurate exactly where work moves fastest. This entry also **supersedes and reconciles two of my own prior entries** that each described this drift for a *different* file and mislabeled each other: the 2026-07-12 entry correctly named a "README status column" (that is `docs/README.md`, which does carry one), and the 2026-07-15 entry wrongly "corrected" it by conflating `docs/README.md` with `docs/opportunities/README.md` (which has no status column) and scoping the drift at one row. Both were half-right about a different file; the accurate picture is three files, two of them status-bearing derived surfaces. A memory-held mental model of "which file holds what" is exactly the thing that rots between sessions and cannot be trusted over disk.
- **Implication:** Treat every mirror of a source-of-truth field as a reconciliation target, and prefer to *mechanize* the reconciliation rather than sweep it periodically by hand — a recompute-and-diff validator over all surfaces at once is the only thing that does not scale its miss-rate with the surface count. This is filed as OPP-0054 (always-on `validate-status-parity.sh`, the status-parity sibling of `validate-catalog-counts.sh` and the row-status completion of `validate-list-completeness.sh`). Until it exists, when reconciling OPP status, fix **both** `candidates.md` and `docs/README.md` in the same pass, and verify the file→field mapping against disk rather than recalling which README carries the column.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-16 (satisfies the PRD-0004 distillation rule fired by filing OPP-0054; substantive connection — the surface enumeration that motivated OPP-0054 is what exposed the second drifted surface and the mislabeling in my own two prior entries, which this entry supersedes rather than silently edits, per the append-only ledger convention)

### A load-bearing § 10 posture fork can invert once grounded on the derived surface's live state — abstract reasoning picked the wrong default

- **Context:** Promoting OPP-0054 to PRD-0036 required resolving its first § 10 fork: how a status-parity validator should treat an index entry that carries **no** status annotation. Three options were on the table — (a) require an annotation on every entry, (b) match-if-present only, (c) treat a missing token as an implicit `proposed`. Read abstractly, option (b) looked like the right *wedge*: gentlest, no forced backfill, "only check what's actually written." That preference held only until I read the live `candidates.md` against the record set and found **OPP-0002 and OPP-0003 carry record `Status: accepted` with no annotation at all** — real drift that option (b) would pass silently, because "match-if-present" treats a missing mirror as absence-of-claim rather than a stale claim. Option (c) fails exactly those two (implicit `proposed` ≠ record `accepted`) while still passing a genuinely-raw entry like OPP-0001 (record `proposed`, no annotation).
- **Observation:** A § 10 enforcement-posture decision is not safely made from the artifact's *description*; it must be made against the artifact's *current contents*, because the whole point of the gate is the drift that has already accumulated, and that drift is only visible on disk. The abstract framing of the three options ranked them by ceremony (b < c < a) and so favored the lightest — but the correct ranking is by *which live defects each option catches*, and on that axis (b) half-closes the class while (c) closes it at the same footprint. The controller-authors-against-the-source-of-truth discipline is usually invoked to catch garbled *facts* (counts, statuses); this is the same discipline applied one level up, to a *design choice* — the posture that looks gentlest in the abstract is the one that would have shipped a gate that misses the exact cases that motivated it.
- **Implication:** When classifying a § 10 fork for a validator whose job is to catch existing drift, enumerate the live violations on disk *before* choosing the posture, and rank the options by defects-caught, not by ceremony imposed. Concretely: run the candidate extraction against the real corpus, look at what each policy would pass and fail, and let the surviving live drift decide. The lightest-footprint option that still catches every present defect wins — here implicit-`proposed`, which needs no blanket backfill yet fails the two accepted-but-unannotated records. Fold "check the live corpus against each posture option" into the PRD-authoring step for any recompute-and-diff validator, the same way catalog-count recipes are verified against the artifact set rather than reasoned about.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-18 (satisfies the PRD-0004 distillation rule fired by editing OPP-0054 `proposed → exploring` and filing PRD-0036; substantive connection — the entry names *how* PRD-0036's first § 10 fork was actually decided, a disk-grounded inversion of the abstract default, distinct from the four prior entries which establish that the drift exists rather than how a posture choice over it should be made)

### A recompute-and-diff validator's own first run against the live corpus needs the same verify-before-acting discipline — 3 of its 5 reported "drifts" were parser false positives

- **Context:** Implementing `validate-status-parity.sh` (PRD-0036), the first run against the live repo reported **5** drifted `candidates.md` annotations. I had predicted exactly **2** (OPP-0002 / 0003, the un-annotated `accepted` records the PRD's evidence names). The three extra — OPP-0037 / 0048 / 0049 — were **not** drift: each is correctly annotated `*(accepted …)*`, but the annotation wraps onto the *line after* the `- [OPP-NNNN](file)` link, and my parser only inspected the link line, so it read those entries as un-annotated → implicit `proposed` → a spurious mismatch against their `accepted` records. Had I trusted the report and "reconciled" all five, I would have *rewritten three correct `accepted` annotations*.
- **Observation:** The discipline "verify a drift report against the raw lines before acting on it" (established when a bad extraction regex produced phantom `MISSING` rows in the PR #177 manual sweep) applies with equal force to a *validator I just wrote* — the tool does not exempt its own output from the check, because the failure mode is identical: an extraction assumption that does not hold across the corpus. The specific assumption that broke is structural to wrapped-Markdown ledgers: **the anchor (`- [OPP-NNNN](file)`) and the value it governs (`*(status …)*`) are not guaranteed to share a physical line** — soft-wrapping puts them on adjacent lines routinely. A line-scoped parser silently conflates "value on the next line" with "no value," and under an implicit-default rule (missing → `proposed`) that conflation manufactures false drift precisely on the correctly-maintained entries. The tell was the prediction miss: 5 reported vs. 2 expected is a signal to inspect, not to bulk-fix.
- **Implication:** When authoring any ledger-scanning validator, (1) parse the whole logical *record/entry block* (anchor line through the next sibling/blank/heading boundary), never just the anchor line, whenever the source format soft-wraps; and (2) treat the validator's first live run as a drift *report to verify*, not a fix list — reconcile expected-vs-actual counts and eyeball the raw lines for each hit before changing a single mirror. A fixture that specifically exercises the wrapped-onto-next-line case (added here as `test_wrapped_annotation_on_next_line_is_matched`) locks the parser against regressing to the line-scoped assumption. This is the same lesson as the PR #177 bad-regex false positives, now generalized from a one-off sweep script to a shipped validator.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-18 (satisfies the PRD-0004 distillation rule fired by registering `validate-status-parity` in `platform/core/kernel/base/module.yaml`; substantive connection — the entry is the concrete build-time lesson of shipping that validator, naming the wrapped-annotation parser bug its own first run surfaced and the whole-block-parse + verify-the-report fixes, distinct from the design-time § 10 entry above it)

### A completeness/parity validator must anchor on a token unique to the surface it guards — the bare record-id fails that test three different ways across three surfaces

- **Context:** Three index/parity gates shipped in quick succession forced the same low-level decision — *which token proves a record is present/consistent in a surface* — and it broke, differently, on all three: (1) PR #177's hand-sweep used an extraction regex that grabbed prose mentions of `OPP-NNNN` and reported phantom `MISSING` rows; (2) `validate-status-parity.sh` (OPP-0054) first matched only the link line and read `accepted` entries whose `*(…)*` annotation wrapped to the next line as un-annotated → false drift, and separately had to reject a prose `[OPP-NNNN](file)` mention so it wouldn't be taken for an index entry; (3) `validate-list-completeness.sh`'s new SUMMARY-nav check (OPP-0055) could not use the bare id at all — `PRD-0014` and `OPP-0007` each appear twice in `SUMMARY.md` (once in a module *description*'s prose, once in the nav row), so a bare-id `grep` would pass with the nav row deleted (a silent false negative), whereas the link-target path `](docs/<dir>/<file>)` occurs exactly once per record.
- **Observation:** The generalizable rule is: **the token a completeness/parity check keys on must appear *only* in the surface being validated, and must span the whole logical record when the source soft-wraps.** A bare record-id fails this in structurally different ways depending on the surface — it over-matches prose in a nav/description file (false negative: the check is satisfied by a non-authoritative mention), it under-matches when the value it governs sits on a wrapped continuation line (false positive: a correct record reads as broken), and it can be mistaken *for* an index entry when it appears in prose (false positive of the opposite kind). The link-target path is the reliable anchor for record→surface presence because a path is only written where a link is actually made; the whole-entry-block scan is the reliable reader for wrapped values. The tell in every case was a count mismatch between what the tool reported and what was independently expected — which is why "verify the drift report against raw lines before acting" is not optional polish but the load-bearing check.
- **Implication:** When authoring any recompute-and-diff or presence validator over Markdown governance surfaces: (1) anchor on a token unique to the target surface — prefer the link-target path over a bare id whenever the id is also cited in prose in that file; (2) parse the whole record/entry block (anchor line through the next sibling/blank/heading), never a single line, wherever the format soft-wraps; (3) add a fixture that specifically feeds the token in a *non-authoritative* position (bare id in prose; wrapped annotation; prose link) and asserts the check still does the right thing — these three fixtures (`test_summary_nav_bare_id_in_prose_does_not_satisfy`, `test_wrapped_annotation_on_next_line_is_matched`, `test_prose_mention_is_not_matched`) are the regression locks; and (4) treat the first live run as a report to verify, reconciling reported-vs-expected counts before changing a single mirror. The recurrence across three surfaces in three days is the signal that this is a validator-authoring invariant, not three unrelated bugs.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-19 (satisfies the PRD-0004 distillation rule fired by filing OPP-0055; substantive connection — OPP-0055's load-bearing design decision *is* the third instance of this anchoring invariant, and the entry generalizes the pattern from three concrete surface-specific failures into a reusable validator-authoring rule with its three regression-lock fixtures, distinct from the two prior status-parity entries which cover the design-time § 10 fork and the single wrapped-annotation build bug respectively)

### A count validator only guards the sites in its assertion table — count mirrors outside it (Mermaid node labels, prose products like "3 × N") drift silently, and a bump propagation misses exactly those

- **Context:** The 25 → 26 validator-count bump (status-parity ship, #181) was propagated by using `validate-catalog-counts.sh` as the oracle: create the new `.sh`, let the validator fail, fix every site it names. That caught the six sites *in its `ASSERTIONS` table* — but the 2026-07-19 doc-watch run then found three it did **not** name: a Mermaid node label reading `(24 validators)` in `docs/architecture/diagrams.md` (a carryover open since 2026-07-13 that widened to off-by-2 because two separate bumps both touched that file and both missed the label), a `× 25 validators` basis and a `72 dynamically generated` product in `platform/validators/README.md`, and — most consequentially — the absence of `validate-status-parity.sh` from the test suite's `VALIDATOR_SCRIPTS` registry, which silently denied the new validator its three `--help`/exit-contract tests. All three are count mirrors that were simply never added to the `ASSERTIONS` table, so the oracle was blind to them by construction.
- **Observation:** Using a count validator as the propagation oracle has a sharp, self-concealing limit: **it can only fail on sites it already knows about, so any count claim not yet registered in its assertion table is invisible to the exact process meant to keep counts honest.** The blind spots cluster in two predictable places — (a) counts embedded inside *strings* that other tooling treats opaquely (Mermaid node labels, SVG text, code-fence prose), which never look like the `>N scripts`-style tokens the regexes target and which markdownlint/link-checkers also ignore; and (b) *derived* counts stated as products or sums in prose ("3 per validator × N validators", "N dynamically generated"), which track a base count but aren't the base count. A test-registry list (`VALIDATOR_SCRIPTS`) is the same failure in code form — a hand-maintained enumeration of the validator set that no validator reconciles against the `validate-*.sh` files on disk, so a new validator can ship fully wired into CI yet with zero contract-test coverage. This is the § 3 mirrored-field law again, one level down: the *coverage set of the count oracle* is itself a mirror of the artifact set, and it drifts.
- **Implication:** When adding a count claim anywhere, add its `ASSERTIONS` row in the same commit — treat "is this site gated?" as part of authoring the claim, not a later audit. Actively hunt the two blind-spot classes: grep the count inside Mermaid/SVG/prose strings (`(N validators)`, `<br/>N scripts`) and gate them with literal-paren/anchored regexes; and gate derived counts with a computed recipe (here `help_tests = 3 × validators`) rather than a frozen literal. For hand-maintained enumerations that mirror a disk set (`VALIDATOR_SCRIPTS` vs. `validate-*.sh`), the durable fix is a completeness check that reconciles the list against disk — a candidate extension of `validate-list-completeness.sh` (every `validate-*.sh` must appear in `VALIDATOR_SCRIPTS`), noted for later. Until then, the rule of thumb: the count oracle's own coverage is a mirror — audit what it does *not* watch, because that is precisely where a bump leaks.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-19 (the count-mirror fix touches no ADR/OPP/module, so no distillation trigger fires; this is a voluntary capture whose audit-trail floor is met by the paired `docs/project/change-log.md` entry. Substantive connection — the entry is the distilled lesson of that fix, naming why the catalog-counts oracle missed three sites the propagation pass should have caught, the two structural blind-spot classes, and the register-vs-disk drift, and proposing the list-completeness extension rather than restating the repairs; distinct from the anchoring-invariant entry above, which is about which token to match, not about which sites a count oracle fails to cover)

### Classify an external agent standard by layer before judging competitor-vs-complement — a wire protocol with a permission *mechanism* but no *policy* is an integration target, not a rival

- **Context:** Evaluating the Agent Client Protocol (ACP — Zed + JetBrains, an LSP-analog JSON-RPC standard for editor↔coding-agent sessions) as possible competitor or complement to auto-harness. The instinct on hearing "a protocol that standardizes agents and includes a permission model" is to read it as overlapping the harness's governance mandate. Reading ACP's own spec dissolved that: it standardizes the *runtime wire* (session lifecycle, streamed tool-call reporting, a per-call `request_permission` prompt) and **explicitly has no audit logging, no policy enforcement, and no trust levels**. auto-harness is the mirror image — all policy (tiers, companion rules, audit, drift validators), enforced at CI and via agent-read docs, with *no runtime wire*. The apparent overlap (permissions) is in fact the cleanest seam: ACP owns the *mechanism* (the prompt), auto-harness owns the *policy* (which options that prompt should even offer).
- **Observation:** Competitor-vs-complement for an external agent-ecosystem standard is decided by **layer, not by feature-list surface area**. Two things that both say "permissions" can be a mechanism and a policy for that mechanism — orthogonal, composable, not rival. The reliable test is feature-negation read against your own genre: enumerate what the standard *explicitly does not do*, and if those absences are precisely your core competencies, it is an integration target whose gaps you fill (and whose mechanism can carry your policy), not a competitor. This also sharpens rather than threatens positioning: a well-scoped runtime protocol is the concrete foil that makes the harness's "governance, not runtime" genre claim legible — the thing the harness is deliberately *not*. The corollary risk is symmetric: the harness must not drift into re-implementing the wire (transport, session framing) any more than the protocol should grow a policy engine; each staying in its layer is what keeps them complementary.
- **Implication:** When assessing any agent-runtime standard, MCP-style protocol, or IDE-integration spec, run the layer classification first: (1) name the layer it occupies (wire/transport, tool-context, policy/governance, orchestration); (2) list its explicit non-goals; (3) if its non-goals overlap the harness's competencies, treat it as an integration surface and look for the *seam* where harness policy drives its mechanism — here the seam is ACP's `session/request_permission`, which a tier-policy engine can populate from the manifest (OPP-0056). Ship the integration as an agent-adapter module in the existing `agents/*` family rather than a bespoke abstraction, and let the adopter overlap (agents that are already harness modules *and* protocol participants) be the wedge into the ecosystem. Only call something a competitor when it occupies the *same* layer with a *competing* abstraction.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-20 (satisfies the PRD-0004 distillation rule fired by filing OPP-0056; substantive connection — the entry generalizes the specific ACP analysis into a reusable layer-classification lens for evaluating external agent standards, which is the decision method behind OPP-0056's complementary-not-competitive framing and its `agents/acp` disposition, rather than restating the OPP's mapping tables)

### A governance harness integrates an external runtime protocol by shipping declarative policy + a companion rule + reference helper sketches — never the runtime itself

- **Context:** Building `agents/acp` (PRD-0037) to bridge harness governance to the Agent Client Protocol raised a scoping question with a non-obvious answer: what does it mean to put a *runtime* protocol integration "into" a governance harness whose entire genre is CI-and-doc enforcement, not runtime? The wrong instinct is to build the proxy/client that actually sits on the ACP wire. What actually shipped is declarative: a `module.yaml` (a consumer policy-binding requiredArtifact + a companion rule making `.acp/**` a sensitive path), a machine-readable `tier-policy.yaml` (the kind→tier→permission-options mapping a proxy reads), and README implementation-helper *sketches* (the proxy architecture, policy-engine pseudocode, an example binding, the audit bridge). The runtime enforcement point lives in the *consumer's* proxy/editor, not in the harness.
- **Observation:** For a governance harness, the deliverable of an external-runtime-protocol integration is **the enforceable contract and the adoption path, not the executable**. The pattern has four parts: (1) a declarative policy artifact that maps the protocol's primitives onto the kernel model (here ACP tool-call `kind` → trust tier → `request_permission` options); (2) a companion rule that treats the consumer's binding of that policy as a sensitive path, so the runtime gate can't silently drift from the declared tiers; (3) reference *sketches* for the runtime (clearly marked not-shipped-code) so adopters can build the enforcement point; and (4) a § 10 classification of **Half-enforced** — the harness owns the policy, the consumer's runtime honors it — which is the honest posture whenever the enforcement point is outside the harness's own CI (the same shape as the SAST and security-static-analysis contracts). This keeps the harness in its layer (policy) and out of the protocol's layer (transport), which is exactly what makes the two composable rather than rivalrous.
- **Implication:** When integrating any external runtime protocol, IDE surface, or agent standard, resist shipping the runtime; ship the policy mapping as a declarative artifact, guard the consumer's binding with a sensitive-path companion rule, provide runtime *helper sketches* rather than a binary, and classify the runtime-enforced claims as Half-enforced in § 10. Model the module on the existing `agents/*` adapter shape (a pack that translates a client's semantics into trust tiers) rather than inventing a new module category — ACP is just the generalization of that translation from one vendor's config to a protocol. The recurring temptation to "actually build the thing" is the boundary to watch: a governance harness that ships runtimes has left its genre, and the audit/positioning confusion that follows is the cost.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-07-20 (satisfies the PRD-0004 distillation rule fired by adding `platform/agents/acp/module.yaml` to the catalog; substantive connection — the entry names the declarative-policy-plus-helper-sketches pattern that shaped what `agents/acp` actually shipped versus the runtime it deliberately did not, with the Half-enforced § 10 posture, distinct from the layer-classification lens above which decides competitor-vs-complement rather than how to build the integration once complement is established)

### Reconciling a machine runtime-audit ledger into a curated knowledge ledger is a distillation problem (promote a governance-relevant subset as draft-then-ratify), not a format-mapping one

- **Context:** Designing OPP-0057, the bridge from the ACP proxy's runtime audit (`.acp/audit/session-log.jsonl` — append-only, machine-generated, per tool call) into the ADR-0002 observation ledger (`shared-observations.md` — curated, synthesis-grade, enforce-as-locked enums). The naive framing is a translator: map each audit row to one observation. That framing is wrong and actively harmful — it would flood the ledger and defeat ADR-0002's whole synthesis purpose (its stated "over-friction" and "looks-rigorous-when-it-isn't" negatives). Reviewing the draft against the actual artifacts also surfaced a second-order finding: the module already ships *two divergent audit schemas* (the declarative `tier-policy.yaml` `audit.record` contract vs. the reference proxy's actual `_AuditSink` output), and neither carries the `sessionId`/`timestamp` the per-session grain requires — a drift that persisted precisely because no validator gates contract-vs-implementation agreement.
- **Observation:** When one governed ledger feeds another, the design axis that matters is **grain and curation, not field-mapping**. A high-volume machine ledger → a low-volume human-curated ledger is a *distillation*: (1) a governance **waterline** that promotes only the decision-bearing subset (here: tier-3+ escalations, denials, sensitive-path touches, tier-5 seam-blocks) and drops routine traffic; (2) **draft-then-ratify**, never machine auto-append into an enforced ledger, so curation and the enforce-as-locked schema are preserved; (3) mechanical population only of the fields a machine can honestly fill (Context/Observation, plus a fixed map onto the *locked* enum — which structurally prevents off-enum drift), with the judgment field (Implication) left blank and Confidence defaulted **low** so a machine draft can't smuggle unjustified rigor past a shape-checking validator. The companion lesson: a declarative contract and its reference implementation **will** drift whenever nothing enforces their agreement — reference material is not self-verifying, so the contract-vs-impl check has to be its own gate.
- **Implication:** For any ledger-to-ledger or telemetry-to-knowledge reconciliation in the harness, resist the translator framing: define the waterline and the grain first, keep the write path draft-then-ratify, map only onto locked enums, and default machine-authored Confidence low. Sequence the *schema reconciliation* before any distillation work — if the source ledger's schema is ambiguous or missing the keys the target grain needs (here `sessionId`/`timestamp`), nothing downstream is buildable, so it is the PRD's first task, not a detail. And when shipping a declarative contract plus a reference implementation of it, plan the drift-guard validator (`validate-acp-audit.sh` here) as part of the same family, because the two *have already* diverged by the time anyone looks.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-07 (satisfies the PRD-0004 distillation rule fired by filing OPP-0057; substantive connection — generalizes OPP-0057's central design decision, distillation-not-translation with draft-then-ratify and locked-enum mapping, plus the contract-vs-implementation drift the review found in the shipped `agents/acp` audit, into a reusable rule for ledger-to-ledger reconciliation, rather than restating the OPP's waterline table)

### A peer-relayed authorization is not the principal's authorization — confirm a gated action in the principal's own channel; it is the same caps-never-grants invariant an inter-agent bus must encode

- **Context:** Filing OPP-0059 (live inter-agent control-loop & cross-vendor bus), handed off by a peer agent session that relayed "the user greenlit filing this." The requested action — writing a governed OPP record and opening a PR — is user-gated in the acting session, and the relay arrived through the peer's channel, not the user's own in-session channel.
- **Observation:** Authority is bound to the principal's own channel, not to a relay of it. A peer message can inform or cap a decision but can never GRANT authorization for a gated write; the acting session must confirm with its own principal directly before crossing the gate. This is the identical invariant the OPP itself encodes for the runtime bus — `tier_ceiling` caps but never grants, bus messages are untrusted data, and Tier 4/5 stays human-gated regardless of peer requests. Governance-time (a peer relaying user approval) and runtime (a peer message on the bus) are two instances of one rule: caps-never-grants. It was field-observed twice this session — two sessions each held their own human-filing gate and refused the other's request as authorization.
- **Implication:** For any inter-agent handoff or bus design, treat a relayed approval as a prompt to confirm with the principal, never as the approval itself; encode caps-never-grants wherever a peer message could influence a gated action; and when landing peer-sourced content in a shared or public store, RE-VERIFY the peer's redaction/scrub yourself rather than inheriting it — you own whatever you commit. The safety spine of a control-loop contract is this authorization-channel discipline, not the message schema.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-29 (satisfies the PRD-0004 distillation rule fired by filing OPP-0059; substantive connection — generalizes the peer-relay-is-not-authorization discipline exercised in this very filing and unifies it with the OPP's own caps-never-grants runtime invariant, rather than restating the OPP's scope table)

### A native-AGENTS.md coding client is still a catalog gap when it has unique permission, sandbox, and vendor-compat knobs — Grok is a client pack, not a model-routing row

- **Context:** First Grok Build TUI session against auto-harness (2026-08-15). Nate asked for a thorough repo analysis and a perspective on incorporating Grok agents. Catalog search found Grok in exactly one place: OPP-0030's provider list ("xAI Grok"). Nine agent packs exist (`base`, `generic-llm`, `claude-code`, `codex-cli`, `gemini-cli`, `cursor`, `copilot-cli`, `openclaw`, `acp`). Grok is absent from README's agent-persona line and the Module System agents row. This session loaded `AGENTS.md` natively and invoked `harness-governance` from `.agents/skills/harness-governance/` with no Grok-specific pack present.
- **Observation:** "Support Grok" collapses three independent axes the catalog already separates: (1) *model* — grok-4.5 / grok-4.6 as an LLM, already named in `architectures/intelligent-model-routing` / OPP-0030; (2) *client* — Grok CLI / Grok Build TUI as an operating environment, missing pack; (3) *protocol* — Grok as an ACP agent, already covered by `agents/acp`. Grok already reads `AGENTS.md` natively (and `CLAUDE.md` for compatibility), already discovers `.agents/skills/`, and already speaks ACP, so *ad-hoc* use needs no pack — `agents/base` plus `AGENTS.md` is sufficient, the same rule Codex documents. A pack becomes justified when the project commits Grok-specific policy files (`.grok/config.toml`, `.grok/sandbox.toml`, `.grok/hooks/`, `.grok/rules/`). Grok's unique knobs are the class Codex and Gemini already police: `--always-approve` / `--yolo` / `bypassPermissions`, default sandbox `off` (unlike Codex's workspace-write recommendation), `--system-prompt-override`, and deeper-wins nested `AGENTS.md`. The correct pack shape is Codex-not-Claude: no required `GROK.md` shim (a restatement of `AGENTS.md` is a drift site; Codex explicitly discourages `CODEX.md` for this reason), optional artifacts under `.grok/`, a sensitive-path companion on `^\.grok/`, review gates that prohibit yolo-plus-sandbox-off outside disposable environments and require committed sandbox `workspace` as the project default. Stability starts at `experimental`. Do not activate `grok-cli` on auto-harness's own manifest until a project `.grok/config.toml` is actually committed here.
- **Implication:** File `agents/grok-cli` as an OPP (`extending-the-harness.md`: new agent packs start as OPPs), not as a model-routing change and not as a `GROK.md`. Cite this observation as Origin / Evidence. The next Grok OPP number is **0059** if untracked OPP-0058 (dreaming) lands first, else 0058. Do not add `.grok/skills/` as a third `link-skills.sh` target in v1 — Grok already scans `.agents/skills/`. Port the Claude distillation Stop hook to `.grok/hooks/` as an optional artifact, matching `claude-code`. Use `agents/acp` for runtime enforcement rather than a Grok-only proxy. Same-pass catalog updates (SUMMARY Agents list, README Module System + persona line, `multi-agent-tool-coordination.md` row) are part of the pack, not follow-up.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Grok (grok-4.6), 2026-08-15 (voluntary compact-ahead capture; no PRD-0004 trigger — no ADR/OPP/module/manifest in this pass. Audit-trail companion is the paired `docs/project/change-log.md` entry. Substantive connection — this is the distilled catalog-and-pack-shape judgment from the analysis session, not a restatement of Grok's user guide.)

### Vendor-compatibility loaders create Copilot-class dual-authority — Grok reads Claude and Cursor surfaces in addition to AGENTS.md

- **Context:** Grok's project-rules, skills, hooks, and permissions docs (`~/.grok/docs/user-guide/12-project-rules.md`, `08-skills.md`, `10-hooks.md`, `22-permissions-and-safety.md`) document that Grok loads, by default: `AGENTS.md` plus `Claude.md` / `CLAUDE.md` / `CLAUDE.local.md`, `<dir>/.claude/rules/`, `<dir>/.cursor/rules/`, `.claude/skills/`, `.cursor/skills/`, `.claude/settings.json` permission rules, and Claude/Cursor hooks. `link-skills.sh` already targets `.agents/skills` and `.claude/skills`. This repo has both `AGENTS.md` and `CLAUDE.md`, plus an untracked `.agents/skills/harness-governance/` adapter. `platform/workflow/multi-agent-tool-coordination.md` has no Grok row (table current as of 2026-05).
- **Observation:** Compatibility is a product feature that becomes a governance risk the moment two instruction files disagree. Copilot CLI already has a pack whose whole job is "multi-instruction-file precedence is non-deterministic — don't let shims contradict `AGENTS.md`." Grok is the same shape with a twist: Claude and Cursor files are *intentionally* loaded, not accidentally. A `CLAUDE.md` that restates trust-tier policy, or a `.claude/settings.json` allowlist looser than kernel policy, becomes Grok's policy too. Deeper `AGENTS.md` files also win over the root, Codex-style. Treating `.claude/settings.json` as Grok's source of truth would encode Copilot's precedence problem on purpose.
- **Implication:** The `grok-cli` pack's review gates must include: (1) `CLAUDE.md` stays a thin load-order shim and `AGENTS.md` remains the contract; (2) committed Grok-native permission policy lives in `.grok/config.toml`, not as a shadow of Claude settings; (3) nested `AGENTS.md` / `.grok/rules/` must not silently relax root tiers. Add Grok as a row in `multi-agent-tool-coordination.md` in the same change as the pack. Do not duplicate harness skills into `.grok/skills/` — a third install target is another enumeration drift surface (operating-principles § 3).
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Grok (grok-4.6), 2026-08-15 (paired with the client-pack observation above; this is the Copilot-shaped dual-authority lesson, distinct from the pack-shape / three-axis judgment.)

### Compact-ahead and tool-local memory are not knowledge destinations — this session's capture belongs in the harness ledger

- **Context:** After the Grok analysis session, Nate asked to capture all project learnings ahead of compacting and rebooting. Grok has experimental cross-session memory (`[memory] enabled`, default off) and a gitignored `.remember/remember.md` handoff. The knowledge-capture write policy is heartbeat-only; the audit-trail companion on observation adds is a dated `memory/<date>.md` daily log or `docs/project/change-log.md`. Untracked OPP-0058 (2026-08-11) proposes out-of-band dreaming / consolidation as the missing session- or time-boundary synthesis layer; operating-principles § 6 already says agent-local folders are projection surfaces, not sources of truth.
- **Observation:** Compacting a session destroys in-context analysis unless it is written to a *governed* destination first. Tool-local stores (Grok experimental memory, `.remember/remember.md`, Claude JSONL) are useful for the next turn: not reviewable, not companion-ruled, not redaction-scanned, not in CI. This compact-ahead request is the in-band capture PRD-0004 already wired — a session-boundary instance of the same "write it to the ledger before the context dies" need that cycle-end distillation serves at PR boundary. OPP-0058's dreaming layer would later mine transcripts *across* sessions; it cannot recover analysis that was never written down in this one. Enabling Grok `[memory]` as a substitute would split institutional knowledge into a reviewed ledger and an unreviewed personal cache.
- **Implication:** Before compact/reboot, append ADR-0002 observations to `shared-observations.md` and satisfy the audit-trail companion with a change-log pointer. Keep `.remember/remember.md` as a short next-session handoff (gitignored, operational). Do not enable Grok `[memory]` as a substitute for the ledger, and do not park this session's Grok-pack analysis in OPP-0058 to wait for dreaming. Dreaming remains a later, propose-only, cross-session layer; Grok transcripts would be a *provider* the way Claude JSONL is named as v1 in the dreaming plan — a satellite of that OPP, not of the `grok-cli` pack.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Grok (grok-4.6), 2026-08-15 (this entry *is* the instance of the rule it names: compact-ahead capture into the governed ledger, with `.remember/remember.md` as the operational handoff only.)

### A Claude plugin memory tree is not a shared handoff — overwriting `.remember/remember.md` is a cross-agent control failure

- **Context:** After Nate asked to capture learnings ahead of compact, this Grok session loaded the Claude Code `remember` skill (`~/.claude/plugins/.../remember/SKILL.md`) and overwrote `<repo>/.remember/remember.md` with a Grok handoff. That file had been read first: it was Claude's 2026-08-07 ACP/OPP-0057 continuation note (merge gate, audit-schema defect, CI-outage playbook, untrusted `AGENTS.md` Cowork block). Nate then named the miss. The file was restored from the session transcript. Cargo-cult revision-tracker rows M-16/M-17 (catalog gap treated as a review finding) were reverted. Finding H-15 already records why a shared ledger exists: agent-private memory does not flow across agents.
- **Observation:** `.remember/` is the Claude Remember plugin's local memory (SessionStart injects `remember.md` / `now.md` / identity; gitignored; not a harness artifact). The harness's cross-agent contract is `AGENTS.md` plus `docs/knowledge/shared-observations.md`. Operating-principles § 6 already classifies agent-local folders as projection surfaces. Loading a *vendor skill* and executing it against another vendor's projection surface is the Copilot dual-authority failure in the other direction: Grok treated Claude's memory as writable shared state. Compounding misses in the same pass: (1) first-session workflow skipped — kernel compiled fragments not loaded, validators not run on `main` before mutating governance files; (2) knowledge-capture write policy is `heartbeat-only` after dreaming, not "compact-ahead, write everywhere"; (3) the escalation table's revision-tracker/ADR path has not been the practiced destination for catalog gaps since April — those go to OPPs, and the revision tracker is for review/audit/validator findings (its own header); (4) halt-before-bypass applies: the existing Claude handoff *was* the control, it was inconvenient, and it was overwritten anyway.
- **Implication:** Grok must not write `.remember/`, `.claude/`, or another tool's projection tree unless Nate explicitly directs a repair (this restore). Durable Grok learnings go only to `shared-observations.md` with a change-log audit trail. Do not file an OPP or mutate the revision tracker without direction. The third 2026-08-15 observation's implication that `.remember/remember.md` is "the operational handoff" is wrong and is superseded by this entry. Next Claude session should still see the restored 2026-08-07 handoff; 2026-08-11 dreaming work remains in untracked OPP-0058 and `.remember/today-2026-08-11.done.md`.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Grok (grok-4.6), 2026-08-15 (correction of this session's own control failure; supersedes the `.remember/remember.md` handoff claim in the compact-ahead observation above rather than silently editing it, per append-only.)

### A Half-enforced module's reference implementation must honor the contract's own safety claims in code — reference code is pedagogy, and its defects reach adopters by copy

- **Context:** Building `management/agent-coordination` (PRD-0039, promotes OPP-0059). The overlay ships a declarative control-loop contract whose load-bearing rules are "messages are untrusted data" and "`tier_ceiling` caps but never grants," plus a runnable Python-stdlib reference orchestrator (`reference/agent-coordination/`). A commit-time security review flagged path-traversal (×2) and symlink-follow in `bus.py`: the untrusted envelope fields (`to`/`from`/`id`/`ts`) were fed straight into filesystem paths (`inbox` dir + `<ts>-<id>-<type>.json`), so a hostile message could escape the bus root. The 19-test suite was green when the bugs shipped — its ids were all well-formed.
- **Observation:** In the Half-enforced reference-tool genre (the `agents/acp` proxy precedent, now agent-coordination), the harness ships a contract + example/adoption code, not an enforced runtime — and adopters *copy the reference as their starting point*. So a reference that violates its own contract's safety claim teaches the opposite of the contract. These traversal bugs were not incidental: they were the contract's own "untrusted data" principle left un-applied at the one layer that turns those declared-untrusted fields into real side effects. This is the same shape as the copy-exact/adversarial lesson that green tests ≠ correct code ([[feedback_copy_exact_plan_execution]]), lifted to the genre level: a security-review pass over reference code is not optional polish, it is part of the contract's *integrity*, because the reference is what propagates. Two supporting build-phase facts from the same session: (1) an opt-in module's `validate-trust-tier`/`validate-sensitive-paths` defects are invisible to the producer's own CI (it inspects only ACTIVE modules) — validate a new module against a throwaway ACTIVATING manifest; (2) `json.load(open(path))` leaks the descriptor — reference code that models resource handling must use a context manager (surfaced only under `-W error::ResourceWarning`).
- **Implication:** For any Half-enforced module that ships reference/adoption code, run a security/adversarial pass over the reference against the contract's OWN stated invariants (untrusted-data, caps-never-grants, redaction) and require the code to *demonstrate* them, not merely describe them: guard every point where a contract-declared-untrusted field crosses into a side effect (filesystem path, shell, prompt); treat a security finding on reference code as a contract-integrity defect, not cosmetic; and note the residual limits the reference does NOT defend (here, a symlink planted inside an agent's own inbox — an actor with local write access is out of scope) so an adopter inherits the boundary, not a false guarantee. Fold the throwaway-activating-manifest check and warnings-as-errors into the reference's test discipline.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-30 (satisfies the PRD-0004 distillation rule fired by shipping `management/agent-coordination` — the `module.yaml` add + the OPP-0059 `proposed → accepted` flip; substantive connection — this is the BUILD-phase lesson, deliberately distinct from the 2026-08-29 FILING observation's authorization-channel point: it generalizes this session's actual reference-code security findings into a genre-level integrity rule for Half-enforced reference-tool modules, rather than restating the OPP's caps-never-grants scope.)

### A shared record schema must be reconciled to a contract-of-record and designed tamper-evident BEFORE its consumers lock in — integrity added later is a migration, not a field

- **Context:** Authoring PRD-0040 (promotes OPP-0057, the ACP audit → knowledge-capture bridge). A grounding pass found the ACP audit surface is internally DIVERGENT, not flatly missing fields: the declared contract (`platform/agents/acp/tier-policy.yaml` `audit.record`) lists `sessionId`+`timestamp`, but the reference proxy (`_AuditSink` in `reference-proxy/proxy.py`) emits neither. Three consumers will build on this one record — dreaming (OPP-0058, which names it a Phase-0 precondition), the coordination bus (PRD-0039), and the verdict ledger (OPP-0052) — and each has an integrity concern (transcript-injection, tamper-evidence, no-silent-verdict-swap) that no surface satisfies today.
- **Observation:** Two design moves belong in the record's FIRST authoritative definition, not a later revision. (1) **Reconcile the divergence to a contract-of-record** — adopt the declared `tier-policy.yaml` schema as authoritative and make the emitter conform, rather than letting a declared contract and an actual emission drift (the drift is a silent-break waiting to happen the moment a consumer trusts the contract). (2) **Design the record tamper-evident up front** with a **keyless SHA-256 `prev_hash` chain** (ordered, replayable, reconstructible; zero key management) — deferring cryptographic signatures, which need actor-key custody the reference must not fake (the `validate-agent-bus.sh` deferral discipline). `prev_hash` is the ONE genuinely-new field (verified: no hashing exists on any surface — proxy, bus, tier-policy, either OPP); `sessionId`/`timestamp`/`actor` already exist somewhere under some name. The costly-to-reverse decision is the hash chain: add it after N consumers depend on the format and it is a migration on a shared schema, not an additive field. This is the audit/replay (G6) dimension of the interop-governance taxonomy ([[project_protocol_layer_governance]]), applied as a keystone-first sequencing rule: design the shared substrate once, first.
- **Implication:** When a schema will be depended on by ≥2 consumers, treat its first definition as load-bearing: reconcile any declared-vs-emitted divergence to a single contract-of-record, and bake integrity (a keyless hash chain) in at definition time; defer only what needs external machinery (key custody). Compose new correlation/integrity fields WITH existing disciplines (here the bus's dispatch-minted `id` and the verdict ledger's canonical `taskId`) rather than minting a parallel primitive — `prev_hash` chains within a session, the existing ids thread across sessions. Sequence the keystone schema PRD ahead of the consumers that need it (OPP-0057/PRD-0040 before dreaming's hash-verifiable citations).
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-31 (satisfies the PRD-0004 distillation rule fired by filing PRD-0040 + flipping OPP-0057 `proposed → accepted`; substantive connection — the reusable schema-keystone sequencing rule this filing embodies, distinct from the same-day Layer-4-taxonomy observation which positions the harness against the interop protocols; this one is the "reconcile-and-harden-the-shared-record-first" design discipline.)

### Auto-harness is a Layer-4 governance implementation in the interop-protocol taxonomy — adopt G1–G6 as a checklist of what to express/record, not a mandate to decide by quorum

- **Context:** A design review (Kang & Diponegoro, "Governance Gaps in Agent Interoperability Protocols," arXiv 2606.31498, Jun 2026) scores five interop protocols (MCP, A2A, IBM's Agent *Communication* Protocol, ANP, ERC-8004) against a six-dimension governance taxonomy — G1 membership, G2 deliberation, G3 voting, G4 dissent, G5 human-escalation, G6 audit/replay — and argues governance is a missing architectural **Layer 4** *above* coordination: protocols transport governance messages as opaque payloads but cannot interpret/validate/enforce them. Every protocol scores ≤2/12. (Acronym collision: that paper's "ACP" is IBM's Agent *Communication* Protocol; auto-harness's `agents/acp` is the Agent *Client* Protocol — different protocols, same three letters.)
- **Observation:** Auto-harness *is* the Layer-4 the paper describes — the OPP-0059 split of governed control-semantics from swappable transport is exactly "governance is not protocol-native." The shipped control-loop contract (PRD-0039), scored the paper's own way and verified field-by-field against disk: **G5 Supported** (`tier_ceiling` caps-never-grants; Tier 4/5 human-gated regardless of any peer message — the harness's strongest dimension), **G2/G6 Partial** (stated `verdict`/`block` reasons; correlation-id + `ts` audit with no tamper-evidence), **G1/G3/G4 Absent** (addressing without a roster; no tally; a `reject` is an ephemeral gitignored bus message, not a preserved record). ~5/12 contract, ~7/12 harness-wide — already above every protocol scored. Two honest correctives fall out: (1) the control-loop 7-message schema substantially **overlaps A2A's Task lifecycle** — the harness-native move is to reframe `agent-coordination` as a governance *binding* onto A2A (as `agents/acp` binds the Agent Client Protocol), not a parallel bus; (2) the taxonomy is Western-deliberative-theory for *collective peer decision-making*, whereas the harness is **principal-gated** — so G3 voting-as-a-decision-rule does not transfer; adopt only its recording half.
- **Implication:** Treat G1–G6 as an audit checklist for what a governance contract must *express and record*, cherry-picking primitives that serve principal-gating — never a mandate to reach 12/12 (letting an external taxonomy drive the design is the repetitive-technology trap turned inward). **G4 (dissent) and G6 (audit) are the next contract revision's requirements**, and their minimal deltas are additive, not structural: a committed dissent artifact (OPP-0052) and a keyless `prev_hash` chain (OPP-0057/PRD-0040 — the shared keystone three consumers depend on, so design it once, first). The strategic option: package the envelope + trust-tier escalation + tamper-evident audit as a candidate **A2A governance extension** (OPP-0001), neutral above A2A and MCP by construction — a narrowing but not-urgent window.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-31 (design-review absorption; distinct from the 2026-08-30 build-phase observation — that one generalized reference-code security integrity; this one positions the harness in the interop-protocol governance-layer taxonomy and sets G4/G6 as the v2 contract requirements. No PRD-0004 trigger fires from a review memo alone; this lands as voluntary capture with a change-log audit-trail pointer.)

### A strategic OPP blocked on FRAMING (not evidence) unblocks when an external event supplies the frame + a clock — flip to `exploring` and ship the cheapest clarifying artifact first, never a commitment

- **Context:** OPP-0001 (exportable governance contract for runtime harnesses) sat in the backlog since 2026-05 "awaiting framing" — its Risks section could not resolve whether the contract should be enforceable vs. declarative, or who would adopt it. It was blocked on *frame*, not on evidence or feasibility. The "Governance Gaps" paper (arXiv 2606.31498) then supplied both the frame (governance is a missing Layer 4; the harness already implements it) and a clock (standardization likely via A2A's extension mechanism in 6–12 months, none done yet), so OPP-0001 was flipped `proposed → exploring`.
- **Observation:** Two OPP-lifecycle moves generalize. (1) An OPP blocked on *framing* rather than evidence/feasibility can sit indefinitely; the unblock is **exogenous** — it arrives when an external development crystallizes the frame — so the right posture once it lands is **`exploring`** (a watch + a cheap probe), not forcing a premature `accepted`. (2) The first exploration deliverable should be the **cheapest clarifying artifact**: here a mapping doc (harness governance primitives → the external G1–G6 taxonomy) costing only documentation-scale, which names exactly what would be contributed-as-neutral vs. kept-proprietary **regardless of whether standardization is ever pursued**. That buys the *option* (clarity + a watch) without paying for the *commitment* (a standards proposal). The neutrality is structural leverage: a governance layer *above* competing vendor protocols (A2A/Google, MCP/Anthropic) is vendor-neutral by construction — the anti-vendor-bias move the harness's genre wants.
- **Implication:** For a strategic OPP blocked on framing, do not force it forward on internal effort — watch for the exogenous event that crystallizes the frame, then flip to `exploring` and ship the cheapest clarifying artifact (a mapping/analysis) as the first bite, never a full commitment. **Temper any external "window" urgency**: the clarifying artifact is valuable independent of the deadline, so produce it, register interest, and watch — do not rush a half-formed proposal to an external body on someone else's clock. Distinct from the same-day Layer-4-taxonomy observation (which positions the harness *in* the taxonomy); this is the OPP-lifecycle discipline for *unblocking* a framing-blocked strategic OPP plus the cheap-first-bite rule.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-31 (satisfies the PRD-0004 distillation rule fired by flipping OPP-0001 `proposed → exploring`; substantive connection — the reusable OPP-lifecycle lesson this exploration embodies: a framing-blocked strategic OPP unblocks exogenously, and the first move is the cheapest clarifying artifact + a watch, not a commitment.)

### Pick a governance mechanism by its check-class — relational checks are companion rules, content/shape checks are validators — and route a machine-authored change's provenance requirement to the machine-authored surface, never a shared file

- **Context:** Designing OPP-0058 (out-of-band "dreaming" consolidation) and hardening its module sketch against disk. Dreaming must (a) require an evidence block on every machine proposal and (b) leave ordinary human observation edits untouched. The first sketch expressed the evidence/provenance requirement as a companion rule keyed on the shared target files (`promotion-candidates.md` / `shared-observations.md`).
- **Observation:** That mis-keys the mechanism twice over. First, **companion rules are path→path and cannot detect authorship** — keying a machine-provenance requirement on a file humans also edit forces the requirement onto every human edit, breaking the base workflow. The fix is to key it on the machine-authored *surface* (`dreaming-runs/`), which only the machine writes. Second, an **evidence-block content requirement is not expressible as a companion rule at all** — a path→path rule can assert file X changed ⇒ file Y changed, but not that a block contains N independent-session citations; that is a *validator's* job (mirroring `shared-observations.md`'s split: audit-trail companion rule + `validate-observation-hygiene` content lint). So mechanism follows check-class: relational → companion rule; content/shape → validator; and provenance-of-machine-output → key on the machine-authored surface.
- **Implication:** When adding any governance rule, classify the check first. If it is "when X changes, Y must change," it is a companion rule — and if the requirement should bind only machine-authored changes, its trigger path must be a surface only the machine writes, not a shared human/machine file. If it is "this content has the right shape/enum/evidence," it is a validator with a diff-scoped, module-gated `--scan-file`-seamed design (the `validate-observation-hygiene` template). Conflating the two either over-fires on humans or tries to enforce content a path rule cannot see.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-31 (satisfies the PRD-0004 distillation rule fired by filing OPP-0058; substantive connection — generalizes the governance-logic defect the design-verification pass caught, mechanism-selection-by-check-class plus machine-surface provenance-keying, into a reusable rule, rather than restating the OPP's thesis)

### A non-native transport adapter can only honor trust tiers as far as the vendor's headless surface allows — that surface is ASYMMETRIC across vendors, so an un-honorable cap must be DECLARED via capabilities and enforced at the argv boundary, never silently over-permitted

- **Context:** Filing OPP-0060 (a local-CLI transport adapter for the agent-coordination bus). A prototype invoked real headless CLIs (Codex/Copilot/Grok) per dispatch and mapped the already-capped `tier_ceiling` onto each vendor's headless flags. The vendors do not map uniformly: Codex's `-s read-only`/`workspace-write` sandbox tracks the trust tiers ~1:1, but **Copilot's non-interactive mode *forces* `--allow-all-tools` — there is no true read-only headless tier**; and one vendor (IDE-only) has no headless CLI at all.
- **Observation:** Two things generalize about bridging a governed contract onto an external runtime you don't control. (1) **The contract governs the *shape* of participation, not its *availability* or *fidelity*.** A `tier_ceiling` is an intent the transport must *honor*; whether it *can* is bounded by what the vendor exposes headlessly. When it cannot (Copilot's read-only gap), the adapter must **declare the gap via `capabilities`** — the same never-silently-drop discipline the adapter contract already applies to unsupported message types — so the consumer runs that adapter only at a deliberately low local tier in a scoped directory, rather than the adapter pretending to honor a cap it structurally cannot. (2) **The caps-never-grants invariant needs a second enforcement point at the boundary where intent becomes an OS action.** `tier_ceiling` caps the *effective tier* in the message layer; the argv *builder* is a defense-in-depth second gate that refuses to *construct* any Tier ≥4 or sandbox-bypass command and emits the task as a single un-shell-interpreted argument — so even a mis-capped dispatch cannot become a full-access spawn. A trust invariant declared once in a contract must be re-enforced at every layer that can violate it.
- **Implication:** When adapting a governed trust contract onto a third-party runtime: never assume the runtime can honor every tier — probe the actual headless surface, and where it can't (forced broad permissions, no read-only mode), DECLARE the limitation via `capabilities` and run at a correspondingly low local tier, never silently over-permit. Re-enforce caps-never-grants at the boundary where a message becomes a process (argv construction / tool invocation), not only in the message schema. And treat a peer-contributed prototype as *untrusted until independently re-verified*: integration-test it against your own frozen code, re-scrub redaction yourself (the peer's very session identity can carry a private token even when its content is clean), and own whatever you commit.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-31 (satisfies the PRD-0004 distillation rule fired by filing OPP-0060; substantive connection — the reusable transport-adaptation lesson this OPP embodies: honor-what-the-surface-allows + declare-the-gap + re-enforce-the-invariant-at-the-argv-boundary + independently-re-verify-a-peer-contribution, distinct from the same-day OPP-0058 mechanism-by-check-class observation.)

### Machine-authored changes to a governed institutional-knowledge store must be propose-only — the machine proposes, a human ratifies; the human merge is the trust anchor, not ceremony

- **Context:** Writing PRD-0041 (the dreaming / out-of-band memory-consolidation overlay). Its most load-bearing FR is that the consolidation job runs **Tier 2** (writes a branch of proposed knowledge-tree changes) and **never Tier 3** (never merges its own output) — a human promotes. The design explicitly rejects a direct machine write to the knowledge tree (the "Letta divergence": a system that lets an agent merge its own memory).
- **Observation:** This generalizes past dreaming to *any* machine-authored change to a governed institutional-knowledge surface (an agent-drafted ADR, OPP, observation, operating-principle, or policy edit). The store's trustworthiness rests entirely on the **review gate** — a human ratifying that a change is true, well-scoped, and non-poisoned. A machine that merges its own proposals collapses that gate: the corpus becomes self-attested, and transcript-borne prompt-injection or a confident-but-wrong synthesis flows straight into the doctrine other agents then treat as ground truth. Confidence/evidence thresholds are a floor on whether a change is *proposed*, never a trigger to *merge* — letting a high confidence score auto-merge is the same collapse by a softer name. The safety is structural (the tier boundary), not procedural (a reviewer's diligence): make the machine *incapable* of merging, rather than *trusted* not to.
- **Implication:** For any agent-authored change to a governed knowledge/doctrine store, enforce propose-only **structurally** — the agent's write authority tops out at "open a branch/PR," the human merge is the gate, and no confidence threshold may cross it. Preserve rejected proposals as a durable dissent record so the same pattern is not re-proposed and the institutional reason it was declined survives. This is the machine-memory counterpart to the harness's principal-gated governance stance ([[project_protocol_layer_governance]]): agents act within declared tiers; a human owner is the authority. Distinct from the same-day OPP-0058 mechanism-by-check-class observation (which is about *where* to key the enforcement) — this is about *what* the enforcement must guarantee.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-31 (satisfies the PRD-0004 distillation rule fired by filing PRD-0041 + flipping OPP-0058 `proposed → accepted`; substantive connection — generalizes the PRD's load-bearing propose-only requirement into a store-agnostic safety principle for all machine-authored governance changes, rather than restating the dreaming design.)

### A reference orchestrator for an LLM-cored process is a SCAFFOLD with the cognition injected as a seam — the harness demonstrates and tests the governed shape with a fake, shipping zero intelligence

- **Context:** Building the `management/memory-consolidation` (dreaming) reference orchestrator (PRD-0041). The process's core is genuine LLM cognition — sub-agents distilling patterns out of transcripts — which a dependency-free stdlib reference cannot (and should not) contain. The reference is `policy.py`/`consolidate.py`/`run.py` with the distillation step taken as an **injected `distill_fn`** (a sub-agent in production; a scripted fake in the 14 tests, zero external calls), the same seam the agent-coordination `native_adapter` uses for `send_fn` and the ACP proxy uses for its agent.
- **Observation:** For any Half-enforced reference whose real work is model cognition, the reusable pattern is **inject the cognition, test the governed structure around it.** What the harness demonstrates and *proves in tests* is the SHAPE the governance cares about — blind fan-out over disjoint partitions, consolidate-only-after-all-return, the evidence bar (prevalence across independent sessions, de-dup same-session sightings, prose-lossy down-weighting), propose-only (writes a branch, `wrote != "merged"`), target-surface containment, permission-scope mirroring, and rejected-proposal suppression. None of that needs an LLM; all of it is the part a consumer must get right and the part an adopter would otherwise reinvent wrongly. The intelligence is the consumer's to supply; the *governed scaffold* is what the harness ships and verifies. This is the reference-tool genre ([[project_agent_coordination]], [[project_protocol_layer_governance]]) applied to a process-with-an-LLM-core: the fake seam is not a testing shortcut, it is the correct boundary — the harness governs structure, not cognition.
- **Implication:** When shipping a reference for an LLM-cored governed process, factor the model call out as an injected function and write the tests against the *structure* (the safety invariants, the evidence/containment rules, the propose-only boundary) with a scripted fake — never against model output. The test suite then proves exactly the governed contract and runs offline in milliseconds, and the reference stays honest about where the intelligence lives (the consumer) vs. where the governance lives (the scaffold). Distinct from the same-day propose-only-trust-anchor observation (which is about *what* the safety must guarantee); this is about *how* to build and test the reference that carries it.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-31 (satisfies the PRD-0004 distillation rule fired by adding the `management/memory-consolidation` module.yaml; substantive connection — the reusable reference-design pattern this build embodies: inject the cognition as a seam, test the governed structure, ship no intelligence — generalizing the injected-seam approach across the harness's reference orchestrators.)

### A trust-tier / permission ceiling that gates a typed (numeric) field is defeated by untrusted STRING data one layer down — enforce a security invariant at every layer the value flows through, not only where it is declared

- **Context:** PR #201 patched a live caps-never-grants defeat in the shipped `agent-coordination` non-native CLI adapter. `build_argv` capped the trust-tier **number** (both `tier_ceiling` and a defense-in-depth second gate checked the number), but appended the untrusted `task` as a **bare positional** (codex) / `-p` value (grok, copilot). A leading-dash task (`--dangerously-bypass-approvals-and-sandbox`, `--sandbox=danger-full-access`) therefore parsed as an **option**, re-opening the sandbox **beneath** the tier gate. Found by a peer adversarial review, independently re-verified against `main`, fixed.
- **Observation:** The invariant "a tier ceiling can only *lower* authority" was enforced where authority is a structured **number**, but not where the same authority is re-expressed as untrusted **string** data one layer down — the argv a process is spawned with. A gate that inspects a typed field is blind to an equivalent capability smuggled through a string that field never sees. The shape recurs at every boundary where untrusted data becomes a more-privileged instruction: argv (flag injection), a path (traversal / a symlinked *directory* component that a leaf-file `O_NOFOLLOW` never covers), SQL, a shell. The harness had already FORESEEN this abstractly — the same-cycle [[project_agent_coordination]] / OPP-0060 observation said "re-enforce caps-never-grants at the boundary where a message becomes a process (argv construction / tool invocation), not only in the message schema" — and #201 is the concrete, exploited-class instance that proves the foresight and exposes the mechanism.
- **Implication:** Enforce a security invariant at EVERY layer the untrusted value flows through, not only where it is first declared or typed. At the argv boundary: bind untrusted arguments so they can never separate into their own token — a `--` end-of-options separator, or a single `=`-joined value — and enforce per-tool capability floors in CODE (refuse a read-only dispatch to a CLI with no read-only headless mode; `CLI_MIN_TIER`), never as a doc note. A test DANGER-list must include the tokens the fix targets but must NOT ban a flag a tool legitimately needs (copilot's `--allow-all-tools` is valid at tier ≥ 2 — the over-permission is closed by refusing read-only, not by banning the flag). This observation plus the OPP-0060 foresight now form a **promotion candidate** for `operating-principles.md` (a general "enforce the invariant at every layer, not only where it is declared" security principle) — flagged for maintainer review, not self-promoted.
- **Confidence:** high
- **Severity:** architectural
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-08-31 (satisfies the shared-observation audit-trail floor via the same-PR change-log entry for the security-hardening doc pass; substantive connection — the concrete argv-flag-injection vulnerability class and its fix pattern, the evidence-backed realization of the OPP-0060 argv-boundary foresight, distinct from that abstract transport-adaptation observation.)

### A contract over a RUNTIME/ephemeral artifact is brought under §10 by a fixture-tested `--scan-file` linter, not a per-PR CI gate — moving it Asserted-only → Half-enforced, not Enforced

- **Context:** Designing OPP-0061 (`validate-agent-bus.sh`), the deferred conformance linter for the agent-coordination bus. The bus's messages are **runtime, gitignored, ephemeral** (`.coordination/bus/`), yet the control-loop contract's schema/lifecycle/`tier_ceiling` semantics were shipped **§10 Asserted-only** — honored by adapters, confirmed at review, but with no mechanism that *checks* an actual session against the contract.
- **Observation:** A per-PR CI validator is the harness's usual enforcement mechanism, but it presupposes a **committed artifact to scan** — and a runtime/ephemeral surface produces none, so the reflexive "write a CI-gated validator" does not apply. The mechanism that *does* bring such a contract under governance is the `validate-observation-hygiene` shape: a **`--scan-file` linter over a transcript** that (a) an operator runs against a live session and (b) CI exercises against **committed valid/invalid fixtures** to prove the linter itself is correct. That combination is what moves the contract from **Asserted-only** (nothing checks it) to **Half-enforced** (a checking mechanism exists, operator-invoked, its correctness CI-proven) — it does **not** reach fully **Enforced** (a per-PR gate), because there is no per-PR artifact to gate. Claiming Enforced here would be the §10 over-classification error ([[feedback_section_10_prd_application]]) in reverse: asserting a gate that structurally cannot fire.
- **Implication:** When a governed contract covers a runtime/ephemeral artifact (bus messages, an ACP audit sink, `dreaming-runs/` output, any live session log), do not reach for a per-PR CI validator — build a `--scan-file` conformance linter plus a committed fixture corpus, wire the fixtures (not a live artifact) into CI, and classify the result **Half-enforced**: operator-run conformance, CI-proven correctness. The fixtures are the honest enforcement surface; the live scan is the operator's tool. This is the runtime-artifact companion to [[feedback_governance_mechanism_by_check_class]] (pick the mechanism by the check) and the `--scan-file` test-seam pattern ([[feedback_validator_test_seam_pattern]]) generalized from "fire a validator in a fixture test" to "the fixture test *is* the CI enforcement for a contract with no committed artifact."
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-01 (satisfies the PRD-0004 distillation rule fired by filing OPP-0061; substantive connection — the reusable governance-mechanism pattern the OPP embodies: a runtime/ephemeral-artifact contract is made checkable by a fixture-tested `--scan-file` linter at Half-enforced, distinct from the same-cycle argv-injection observation which is about enforcing an invariant at every layer.)

### A conformance linter partitions its checks into ERROR (structural contract breaches) and WARN (advisory/newly-introduced caps) — so a contract that shipped "no cap in v1" can gain caps without breaking existing conformant transcripts

- **Context:** Implementing `validate-agent-bus.sh` (OPP-0061, accepted + built in one change). The control-loop contract shipped the `sync` size/rate caps as "**no cap in v1**, deferred to the linter." Building the linter is the moment those caps get *defined* — but making them a hard failure would retroactively fail transcripts that were conformant under the "no cap" contract.
- **Observation:** The resolution is a two-tier severity + a three-state exit code. Structural contract breaches (bad envelope, a missing per-type payload key, an orphan response, a double-terminal, a response-before-`ack`, a non-integer `tier_ceiling`) are **ERROR → exit 1**: these were always violations, so failing them breaks nothing legitimate. Newly-introduced or advisory limits (the `sync` size/rate caps, a Tier ≥ 4 dispatch flag) are **WARN → exit 0 with a notice**: visible and tunable, but non-blocking, honoring the "no cap in v1" promise the contract already made. Usage errors are a third state (**exit 2**). This lets the committed-fixture self-test assert three distinct outcomes — *valid* (0, no warn), *violation* (1), *advisory* (0 + WARN in output) — which is what makes the fixtures a real enforcement surface rather than a smoke test. The build also confirmed the check-set was 100% mechanically derivable from the contract (the one subtlety — a `block`→retry reusing the same id reopens a "terminal" — was already explicit in the contract's lifecycle diagram), evidence the OPP was a complete design contract.
- **Implication:** When a shipped contract deferred a limit to "no cap in v1," introduce that limit at **WARN** (advisory, non-blocking) in the linter that lands it, reserving **ERROR** for breaches that were always invalid — and encode the split in a 0/1/2 exit contract so a fixture suite can distinguish valid / violating / advisory. This is the backward-compatible way to tighten a governed contract: the cap becomes visible and measurable immediately, and a later change can promote it WARN→ERROR once operators have adjusted. Companion to [[feedback_section_10_prd_application]] (classify the enforcement level honestly) and the runtime-artifact-linter observation above (the fixture corpus is the enforcement surface).
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-01 (satisfies the PRD-0004 distillation rule fired by flipping OPP-0061 proposed → accepted + implementing it; substantive connection — the WARN-vs-ERROR / three-state-exit pattern for introducing a deferred cap into a shipped contract without breaking conformant history, distinct from the same-record filing observation about the fixture-tested scan-model.)

### A governance framework must apply its own claim-classification doctrine to its OWN documentation — the docs that describe what the harness enforces are themselves an enforcement surface that drifts toward overclaiming

- **Context:** A 2026-09-01 adversarial audit found the harness's own docs overstating what its validators enforce: the README said `validate-trust-tier.sh` "closes safety claims 10–11 (no self-elevation; tier-ceiling fixed)" when the validator only does PR-boundary declaration-coherence (it cannot stop a running session self-elevating, and a module omitting `maxTier` is not ceiling-checked); `threat-model.md` still listed trust-tier + skill-content scanning under "Mitigations Not Yet Deployed" months after both shipped; and the onboarding skill defined "Harness Ready" as CI-green, silently dropping the kernel's second-human-review requirement.
- **Observation:** This is the harness failing its own operating-principles **§10** ("Classify Claims Before Enforcing Them") — but turned inward. §10 was written to stop the harness overclaiming that a governance property is Enforced when it is only Asserted; the same failure recurs one level up, in the *prose that describes the validators themselves*. A doc claiming a check "closes" a safety class is making an enforcement claim, and — exactly like a module's asserted-vs-enforced status — that claim drifts toward optimism over time (a validator ships partial, the doc rounds it up to "closed"; a mitigation ships, the "not yet deployed" list is never trimmed). No validator guards the accuracy of these meta-claims (a count validator checks that "17 diagrams" is 17, not that "closes claim 11" is true), so they rot silently and an evaluator trusting the README believes the centerpiece safety properties are machine-checked when they are honor-code.
- **Implication:** Apply §10 to the harness's own docs: when a doc says a validator "closes"/"enforces"/"prevents" a named property, state the **enforcement boundary** in the same breath (PR-boundary vs runtime; active-module vs all-shipped; declaration-coherence vs behavior). Keep a "shipped since" discipline on any "not yet deployed" list (the file's own update rule). Treat the definition of a readiness/done state (e.g. "Harness Ready") as load-bearing — dropping a gate from it (the second-human review) silently weakens the contract. The durable fix is a validator that lints enforcement-claim prose against a registry of what each validator actually checks — the meta-level of the register-prose-counts bullet ([[feedback_section_10_prd_application]], operating-principles §3) — tracked as a candidate; until then this class is caught only by adversarial audit + holistic review.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-02 (satisfies the shared-observation audit-trail floor via the same-PR change-log entry; substantive connection — the §10-turned-inward lesson the doc-honesty remediation embodies: a governance framework's descriptions of its own enforcement are an unguarded claim surface that drifts toward overclaiming, distinct from the module-level §10 classification discipline it generalizes.)

### A content-scanning validator's own PARSER and SCOPE are attack surfaces — and a fixture suite of one canonical line per pattern is exactly what the evasions slip past

- **Context:** The 2026-09-01 audit found four bypasses in the harness's scanning validators, none of which the (green) self-test suite caught: `validate-skill-content.sh` matched its prompt-injection denylist **per line**, so `ignore\nprevious instructions` (or a markdown-wrapped word) evaded it; its role-header patterns were case-sensitive with a mandatory post-colon space (`/^System:\s/` missed `system:` and `System:do`); its scan-set was **active-modules only**, so the root entrypoints an agent loads at session start (AGENTS/CLAUDE/HARNESS/TOOLS/operating-principles) and most SKILL.md were never scanned; and `validate-knowledge-redaction.sh` skipped any diff line matching `+++*` as a "file header", so an added line beginning `++` (diff-marked to `+++`) bypassed the denylist.
- **Observation:** Every one of these is a defect in the validator's **own text handling**, not in what it was asked to check — the parser (line-vs-whole-text, case, diff-header disambiguation) and the **scope** (which files are even in the scan set). A content scanner is itself parsing adversary-influenced input, so its tokenization is an attack surface exactly like the code it guards. And the reason the suite stayed green is structural: the adversarial fixtures were **one canonical line per pattern id** (`p01-*.txt` = the exact phrase on one line), which tests that the *happy-path* pattern fires but never the evasion forms — the split, the case variant, the header collision, the out-of-scope file. A green scanner suite proves the patterns match their own fixtures, not that an adversary can't restructure the payload around them.
- **Implication:** For any validator that scans adversary-influenced text: (1) scan a **whitespace-normalized whole-text** form for phrase patterns in addition to a per-line pass for anchored ones, so a cross-line split cannot bypass; (2) make the scan-**set** explicit and independent of activation for surfaces that are loaded regardless (root entrypoints, all shipped skills), because "only active modules" silently drops the highest-value surface; (3) disambiguate protocol framing (diff headers, fences) **structurally** (the `---`/`+++` header pair, matched with its trailing space), never by a glob that content can spoof; and (4) write the adversarial fixtures as the **evasion matrix** — for each pattern, the canonical hit AND the split / case-shifted / wrapped / header-collided / out-of-scope variants — because the single canonical line is the one form guaranteed not to be the exploit. Sibling to the §10-turned-inward observation above (docs overclaim; here the checks themselves under-cover) and to the "green tests ≠ correct code" discipline.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-02 (satisfies the shared-observation audit-trail floor via the same-PR change-log entry; substantive connection — the validator-parser/scope-as-attack-surface lesson the PR-3 hardening embodies, and why a one-canonical-line-per-id fixture suite systematically misses it, distinct from the doc-overclaim observation which is about claims rather than coverage.)

### A governance proxy that rewrites what it PRESENTS but forwards the response verbatim is a UI filter, not a gate — enforce the rewrite on the return leg; and an invariant written as a comment ("never loosen") is not enforced until code rejects the violation

- **Context:** The 2026-09-01 audit of the ACP reference proxy (`platform/agents/acp/reference-proxy/`) found the permission seam defeatable four ways: `classify` did `tier = max(matched)`, which REPLACED the execute baseline so a benign keyword anywhere (`rm -rf dist # run test`) dropped a destructive command to auto-approvable Tier 1; the governance-entrypoint escalation was gated on the agent's self-declared `kind`, so an `execute` of `sed -i … HARNESS.md` never hit the Tier-5 rule; `load_policy` accepted an override that *loosened* a tier, with the "may tighten, never loosen" rule living only as a YAML comment; and the proxy rewrote the `request_permission` option set shown to the client but forwarded the client's RESPONSE verbatim — so a client could answer `allow_always` to a request the proxy had rewritten to offer only `reject_once`.
- **Observation:** Two of these are a general class. (1) **Presentation ≠ enforcement.** The option-set rewrite changed what the user *saw* but not what the agent *received back*; the decision returns on a second leg (the response), and a gate that only edits the outbound request is a UI filter an adversary bypasses on the inbound response. A mediating proxy must enforce its rewrite on BOTH legs — track the request id → the options it actually offered, and clamp the returning selection to that set. (2) **A comment is not a constraint.** "Overrides may only tighten" as prose in `tier-policy.yaml` bound nothing; the loader did a shallow merge and accepted a tier-lowering override. An invariant is only enforced when code *rejects* its violation (here: `load_policy` now raises on any override that lowers a tier or lifts a ban). The other two (floor-not-replace on a downgrade; escalate on the actual operation not the self-declared `kind`, de-anchoring a path regex to search a command string) are the argv-injection lesson's siblings — a security decision derived from untrusted input must not be lowerable by that same input.
- **Implication:** For any proxy/interceptor that rewrites a security-relevant message: (a) if the protocol has a request→response round trip, enforce the rewrite on the response, not just the request (correlate by id, clamp the return); (b) encode every "must never" invariant as a code check that rejects the violation, never as a comment or doc line; (c) a tier/permission *downgrade* is a floor operation — a benign match may lower only a wholly-benign, structurally-verified command, never replace the baseline; (d) escalate on the actual operation (the command string, the target path) rather than a self-declared classification the governed party controls. Sibling to the argv-injection observation (enforce the invariant at every layer) and the scanner-parser observation (the check's own handling of adversary input is the attack surface).
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-02 (satisfies the shared-observation audit-trail floor via the same-PR change-log entry; substantive connection — the presentation-vs-enforcement + comment-is-not-a-constraint lessons the ACP-proxy hardening embodies, distinct from the scanner-parser observation which is about coverage of a one-way scan.)

### When a component you shipped overlaps an emerged standard, bind the OVERLAPPING layer to the standard and declare the parts the standard cannot express as labelled extensions — not rip-and-replace, and not force-fit

- **Context:** Filing OPP-0062. PRD-0039 shipped the inter-agent bus with a bespoke per-agent descriptor layer (who each participant is, what it can do). The industry then consolidated on A2A (an LF-governed agent-interop standard) for exactly that descriptor layer — its Agent Card is identity + skills + capabilities + securitySchemes. But A2A (a) binds only to network transports (HTTP/gRPC — no local/stdio/file binding), so it cannot carry the bus's local heterogeneous fleet, and (b) has no concept of a permission ceiling / trust tier (the Layer-4 governance gap, arXiv 2606.31498) — so `tier_ceiling` has no standard field.
- **Observation:** The reflexive responses are both wrong. *Rip-and-replace* (drop the bus for A2A) fails because A2A can't transport a local fleet and can't express `tier_ceiling`. *Ignore the standard* (keep everything bespoke) throws away legibility — an A2A-aware tool could read a participant's identity/skills if the descriptor spoke Agent Card. The correct move separates the stack into LAYERS and treats each on its merits: adopt the standard for the layer it actually governs (the **descriptor** — align to Agent Card), keep the layer it does not bind (the **transport** — the file bus stays, declared as a non-standard interface), and model the thing it structurally cannot express (the **governance ceiling** — `tier_ceiling` as a named Agent Card *extension*, not force-fit onto a standard field). This reframes the bus not as a reinvention competing with A2A but as governance *binding onto* A2A ([[project_protocol_layer_governance]]) — the standard covers one layer; you adopt it there and declare, honestly and separately, the layers and capabilities it does not cover.
- **Implication:** When bespoke work overlaps an emerged standard: decompose into descriptor / transport / governance (or the equivalent layers), map each to "the standard governs this / the standard is silent here / the standard structurally can't express this," adopt the standard exactly where it governs, and declare the rest as **labelled** additions (a non-standard transport interface; a named extension) so an ecosystem tool sees both the standard part it understands and the clearly-flagged parts it does not. Do not force a novel capability onto a standard field (it will be misread), and do not reject a standard for failing to cover everything (adopt the part that fits). Pair with strict provenance discipline on the external-standard facts themselves: hold secondary-source claims as pending-primary-confirmation and refuse to cite an unpinned version/date normatively — a governance record must not launder a research summary into asserted fact ([[feedback_peer_handoff_verification]]).
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-02 (satisfies the PRD-0004 distillation rule fired by filing OPP-0062; substantive connection — the bind-the-overlapping-layer / extend-for-the-rest strategy the OPP embodies, generalizing the protocol-layer-governance stance into a reusable rule for reconciling any shipped component with an emerged standard, distinct from the same-record provenance discipline.)

### A control's OFF-SWITCH must be governed at least as tightly as the control — an unowned exemption file, an unpinned CI action, and a default that tracks an upstream tip are all silent ways to disable a gate with a green PR

- **Context:** The CI/bootstrap hardening pass (audit PR-5). Three findings shared a shape: (1) the validator **exemption files** (`.skill-content-ignore`, `.publication-boundary-ignore`, `.knowledge-redaction-ignore`, `.doc-reference-ignore`) were in **no module's `sensitivePaths` and fired no companion** — a contributor could add `.` or `.*` to one and blind a security validator, CI staying green; (2) the CI workflow pinned every third-party action to a **floating major tag** (`actions/checkout@v7`), so an upstream tag re-point silently changes what runs in the trusted CI context; (3) `install.sh` taught consumers `git submodule update --remote` as the default, which **tracks the upstream `main` tip** — an upstream force-push lands in a consumer unreviewed.
- **Observation:** Each is the *disable path* of a control left less-governed than the control itself. An exemption file is the OFF switch for a scanner; an action pin is what decides which code runs your gates; the update policy is what decides which version of the whole harness a consumer trusts. A governance system spends its effort hardening the gate (the denylist, the validator, the tier rule) and leaves the mechanism that *turns the gate off* — or swaps the gate's implementation, or the whole ruleset — as ordinary config. That asymmetry is the exploit: you don't defeat the check, you defeat the thing that controls whether the check runs. The fix in each case is to raise the off-switch to the control's own governance level: the ignore files are now kernel `sensitivePaths` + companion-triggered (editing one needs the same review as editing `HARNESS.md`); the actions are pinned to full commit SHAs (with the tag in a comment); the CI token is least-privileged to `contents: read`; and the consumer default is a pinned release tag, not the `main` tip.
- **Implication:** For any enforcement mechanism, ask "what turns this off, swaps it, or changes what it runs — and is THAT path governed as tightly as the mechanism?" Concretely: put every exemption/ignore/allowlist file under `sensitivePaths` + a companion rule (silencing a gate is a governance change, not a config tweak); pin every external CI action to an immutable SHA and least-privilege the workflow token (`permissions: contents: read`); and make the consumer-facing update default a pinned version, never a branch tip. The gate and its off-switch are one control surface — govern them together.
- **Confidence:** high
- **Severity:** governance-relevant
...
### A shipped record left at `proposed` is a status drift the parity validator cannot catch — it enforces that a record's status *surfaces agree*, never that the status reflects *shipped reality*

- **Context:** OPP-0060 (the non-native local-CLI transport adapter) still read `proposed` across all four of its surfaces — its own record, `candidates.md`, `README.md`, and `SUMMARY.md` — long after its implementation shipped (#198 reference-orchestrator port) and was security-hardened (#201 argv-flag-injection fix), both on `main`. `validate-status-parity` had passed the whole time, precisely *because* all the surfaces agreed on the same wrong value.
- **Observation:** Status parity is an *internal-consistency* check (record token == candidates annotation == README column), not a *currency* check. It closes the drift class where surfaces disagree with each other, but it is structurally blind to the class where every surface agrees on a status that no longer matches the code — the mirror image of the `finalize-before-accept` drift (a record flipped to `accepted` while carrying stale claims). Both live in the same gap: no validator reconciles a design record's declared status against whether its implementation actually shipped, because that link (OPP ⇄ merged PR) is not expressed on any validated surface. The cheap discipline is to flip the status **on the implementing PR** (the PR-boundary trigger the `finalize-before-accept` candidate already argues for), so the record never diverges from the code in the first place; the periodic status audit is the backstop for records that already drifted.
- **Implication:** Treat "status-parity is green" as saying only that the surfaces agree, never that any status is current. When an implementation PR closes an OPP/PRD, flip that record's status in the same PR. For records that already drifted, reconcile by verifying the code is on `main` first (a reconciliation to a shipped fait accompli, not a fresh acceptance decision) and flip every surface — including the non-validated ones like `SUMMARY.md` — as one set, per the reconcile-the-set-not-the-row discipline.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-24 (satisfies the PRD-0004 distillation rule fired by editing the OPP-0060 record; substantive connection — the observation is the reusable lesson behind the very reconciliation this PR performs, and names the validator-blind status-currency gap distinct from the internal-agreement parity check that stayed green over it.)

### A maintainer's green validator chain does not predict a consumer's — the harness's always-on structural checks silently assume its own dogfooded layout, and a breaking catalog change owes a one-release compat affordance

- **Context:** Upgrading four consumer repositories from older pins to current `main` surfaced four consumer-facing breaks plus a process gap, none visible in the harness's own green CI. Re-derived against the real validators: `validate-list-completeness` asserts a bare repo-root `SUMMARY.md` (a GitBook-from-`docs/` consumer keeps its TOC at `docs/SUMMARY.md` → fails despite a complete nav); `validate-catalog-counts` runs `find platform/…` relative to its given root with no detection of a `.harness/` submodule mount (a wrong-root run reads as *count-drift*, not a path error); `validate-placeholders`' ISO-date arm matches that literal date token anywhere, including code spans and string examples; and the `data/relational-postgres`→`data/relational-sql` rename (PRD-0033) deleted the old module id with no alias, crashing 9 of 14 validators for a consumer that actively selects the old id. Two of the four repos were already CI-red on `main` unnoticed.
- **Observation:** The failures share one root cause — the harness's *always-on structural validators* encode the harness's own dogfooded layout (root `platform/`, root `SUMMARY.md`, `docs/` at root) as if it were universal, and at least one breaking change shipped with no consumer-compat affordance. Because a consumer's activation set and doc layout differ, the maintainer's green chain is not evidence about any consumer — each of four repos broke *differently*. Two distinct disciplines follow. (1) *Layout-agnosticism:* an always-on structural check must tolerate a **declared** non-root layout (honor `.gitbook.yaml root:` / accept either `SUMMARY.md` location / auto-detect or loudly error on a missing `platform/`) rather than assume the dogfood shape, or exit `2` with an explicit wrong-root message instead of silently misreporting drift. (2) *Compat affordance:* a breaking catalog change — a module rename above all — owes a **one-release deprecation affordance** (an old→new alias resolved with a WARN) plus a BREAKING changelog note, exactly as the harness already demands of other breaking changes; deleting an id in a single cutover is the defect.
- **Implication:** Treat "the harness CI is green" as saying nothing about consumer upgrades. Add a consumer-upgrade preflight that runs the FULL chain from the paths the consumer's CI uses (the `consumer-upgrade-runbook.md` already exists — augment it with the module-rename / new-index-surface / path-assumption break classes rather than build anew). When authoring an always-on structural validator, ask "what does this assume about where files live?" and make the assumption a declared, overridable input. When renaming or removing a shipped module id, ship the alias in the same release. This is the same defect-vs-deliberate-posture triage as the 2026-09 adversarial audit — F1/F3/F4 are partly documented posture (a consumer affordance not used), while the F2 rename cutover is an unambiguous breaking-change defect.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-16 (satisfies the PRD-0004 distillation rule fired by filing OPP-0063; substantive connection — the layout-agnosticism and compat-affordance disciplines are the reusable core of the five findings that OPP triages, field-reported across four consumer-repo upgrades and re-derived against the named validators on disk.)

### When a peer field-reports an external tool's CLI behavior, verify against the VENDOR's own docs and issue tracker before encoding the specifics — the load-bearing fact can confirm while the exact flags contradict

- **Context:** A peer coordinator session field-reported that AntiGravity now ships a headless CLI (`agy`), making the harness's canonical "AntiGravity has no headless CLI (IDE-only)" example (used in ≥5 agent-coordination docs) false, and proposed a concrete non-native adapter: a `build_argv` branch with `--print=<task>` (=-bound), a tier-2 floor, `--dangerously-skip-permissions`, plus `--mode plan` and `--add-dir` flags — all from a live `agy` run on the peer's side. Web-verifying against AntiGravity's **own** headless-mode documentation and the public `google-antigravity/antigravity-cli` issue tracker confirmed the load-bearing fact (headless `agy -p`/`--print` exists; `--dangerously-skip-permissions` and `--model` are real; issues #408/#318/#548 corroborate exit-0-empty-when-piped, non-TTY hang, and `permissions.allow` being ignored) — but **contradicted** several specifics: the docs show the prompt value is space-separated (`agy -p "<prompt>"`), not the reported `=`-bound `--print=`, and `--mode plan` / `--add-dir` are undocumented.
- **Observation:** A peer's report of an external tool's behavior splits cleanly into a *load-bearing fact* (does the capability exist at all — here, is there a headless CLI) and *exact mechanics* (the precise flag forms, exit-code contract, argument binding). The fact is cheap to confirm from the vendor's primary sources (official docs + the tool's own issue tracker) and, once confirmed, is enough to act on the doc-honesty consequence (correct the stale §10-inward claim). The mechanics are a live-run detail that a secondary report frequently gets subtly wrong — and here the vendor's own docs directly contradicted the reported argv form, which is exactly the input the `build_argv` caps-never-grants gate must bind safely. So file the confirmed correction and the design, but **defer the code that depends on the contradicted mechanics until a live run resolves them** — never encode a peer's exact argv into a security-sensitive adapter on trust. Same discipline as OPP-0060/OPP-0062 (a peer's live-run details are re-derived, not adopted), now with the vendor's *own* docs as the adjudicator rather than a second research pass.
- **Implication:** For any peer- or research-sourced claim about an external CLI/protocol, resolve it against the vendor's primary artifacts (docs + issue tracker + source) before it drives code, and record the confirmed / contradicted / unconfirmed split explicitly in the OPP's verification table so the deferral is legible. The exit-code lesson generalizes past this one tool: a documented contract ("fails exit non-zero") and the real behavior ("exit 0, empty stdout when piped") can disagree, so a headless runner should judge success on captured OUTPUT, not exit status, and bound every invocation with a timeout.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-16 (satisfies the PRD-0004 distillation rule fired by filing OPP-0064; substantive connection — the confirmed-fact-vs-contradicted-mechanics split and the vendor-primary-source adjudication are the reusable core of that OPP's verification table and its deferral of the F4 adapter code, extending the peer-handoff-verification discipline to an external tool's own documentation.)

### Classify an external agent tool by LAYER before deciding integrate-vs-govern-vs-avoid — a runtime with algorithmic "trust/audit" vocabulary is a governed TARGET, not a governance peer and not a dependency to absorb

- **Context:** Evaluating whether to integrate `ruflo` (ruvnet/rUv — `claude-flow` rebranded/rebuilt) into auto-harness. Three parallel research agents, web-verified against ruflo's own repo/README/LICENSE/package.json, established: ruflo is a **runtime agent meta-harness** (spawns/coordinates 100+ -agent swarms around Claude Code/Codex, vector memory, federation, WASM sandboxes, ~210–314 MCP tools; MIT; ~daily alpha releases; single-owner). It ships governance-*flavored* features — a numeric behavioral "trust score," HIPAA/SOC2 "audit" modes, an AIDefence plugin, even an ADR plugin and a setup-grading "metaharness" — but these are algorithmic/security mechanisms, with no human-ratified policy or human-approval gate.
- **Observation:** The integrate decision turned entirely on **layer**, not feature-name overlap (the same lesson as the ACP analysis). ruflo occupies the runtime-execution layer (the LangGraph/CrewAI/AutoGen genre); auto-harness is the governance layer. That makes ruflo a **complement**, and specifically a *governed target* — its headless CLI is the exact shape the non-native CLI adapter (OPP-0060) already drives, and its MCP server is what the ACP bridge (OPP-0056) fronts, so "governing" it is a reference adapter + tier-policy, not new machinery. Two traps that feature-name matching would walk into: (1) its "trust/audit/ADR" vocabulary looks like a governance peer, but the mechanism is algorithmic/security, not human-authorization — adopting its ADR/metaharness plugins would duplicate auto-harness's core with the weaker mechanism; (2) *absorbing* it as a dependency would invert a load-bearing posture (dependency-free → Node+WASM+web-server+LLM-keys). The right verb for a runtime-with-pseudo-governance is **govern it, don't vendor it, don't import its governance.**
- **Implication:** When assessing any external agent framework for integration, first classify its architectural layer (runtime-execution vs wire-protocol vs governance) and inspect whether its trust/audit/policy features are *human-ratified* or *algorithmic*. A runtime whose governance is algorithmic is the ideal governed target for a human-authorization harness — integrate it *beneath* the trust tiers via the existing adapter/bridge seams, keep the governance layer dependency-free, and never adopt the target's own governance surface. Govern a fast-moving single-owner target at a *pinned* version, never its `main`/`@latest` (the OPP-0063 consumer-compat discipline applied to a dependency-as-target).
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-23 (satisfies the PRD-0004 distillation rule fired by filing OPP-0065; substantive connection — the layer-classification-before-integration doctrine and the govern-don't-absorb verb are the reusable core of that OPP's thesis, extending the ACP layer-classification observation to a runtime-execution harness with algorithmic pseudo-governance.)

### A control's ceiling deserves the same justification discipline as its floor — mirror the threshold; and an ADR that defers a policy "to avoid retrofitting shipped config" is deferring the config-justification work, not the design

- **Context:** ADR-0020 corrected the `maxTier` inversion (a ceiling, caps-never-grants) and *deferred* a "high ceiling requires rationale" policy, explicitly because adding it would "retrofit rationales onto the three shipped packs and mix a new policy into an inversion fix." Revisiting it later: all three shipped agent packs (`base`, `generic-llm`, `openclaw`) declare `maxTier: 5` with no justification — i.e. the deferred trigger ("broad ceilings proliferate without justification") was already quietly true of the harness's own packs. Implementing the policy at the `>= 3` threshold (mirroring the existing `tier.declared >= 3` rule) turned out to be *mostly* writing those three justifications, with a small helper change.
- **Observation:** Two reusable points. (1) *Mirror the threshold across a control's floor and ceiling.* The trust model already required a rationale when a module's *declared* tier reached the autonomously-actionable band (`>= 3`); its *ceiling* (`maxTier`) is the same kind of authority claim and should cross the same justification line at the same number — one threshold is easier to reason about than two, and a ceiling reaching git-writing-and-above is exactly as worth justifying as a floor there. (2) *A deferral "to avoid retrofitting shipped config" is a deferral of the justification work, not the policy.* When you come back to it, the design is trivial (one comparison) and the real cost is writing the honest per-artifact rationales — which is the point, not overhead: the packs that had unexplained broad ceilings now carry a reason, and future/consumer agents must supply one too. Keep the logic in the unit-tested helper (a new `:missing_rationale` state, blank-counts-as-missing, structural incoherence reported first) so the rule is verified fixture-free.
- **Implication:** When hardening a governed threshold, check whether the control has a *sibling* bound (floor vs ceiling, declared vs max) and give them one shared rationale/justification rule rather than divergent ones. When an ADR defers a policy citing "retrofit onto shipped config," read that as "the justifications aren't written yet" — schedule the follow-on as a config-authoring task, verify the new guard against the already-shipped config first (it must not red-fail the packs once they carry rationales), and confirm the safe/most-restrictive case still passes.
- **Confidence:** high
- **Severity:** governance-relevant
- **Contributed by:** Claude Code (claude-opus-4-8[1m]), 2026-09-25 (satisfies the PRD-0004 distillation rule fired by editing the three agent packs' module.yaml and the trust-tier validator; substantive connection — the mirror-the-threshold principle and the deferral-is-config-work realization are the reusable core of the ADR-0020 follow-on this PR implements, distinct from the maxTier *inversion* record which is about a check's direction rather than a justification threshold.)
