...
> Owner: @unclenate
> Last updated: 2026-06-10 *(§ 3 gained a bullet — a new counted surface couples its entrypoint count in the same change: adding an architecture diagram increments the canonical diagram count that `validate-catalog-counts.sh` asserts against HARNESS.md/README prose, so the entrypoint bump ships in the same commit (worked example: the `management/digital-twin` family diagram, #14) even when the motivating capability is a catalog-only, default-off overlay. Prior: § 3 gained the register-prose-counts bullet 2026-06-07 during the QA pass that fixed the HARNESS.md/README.md "twelve→thirteen" diagram-count drift; § 10 "Classify Claims Before Enforcing Them" added 2026-05-28 from the four-instance [[claim-vs-enforcement-classification]] observation chain (OPP-0037); § 9 "Split Design from Implementation" added 2026-05-26; the curated longitudinal destination role established 2026-05-25 per ADR-0014)*
