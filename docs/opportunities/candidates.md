...
- [OPP-0058](OPP-0058-out-of-band-memory-consolidation-dreaming.md) *(accepted 2026-08-31; session-shape §4 unifier; promoted via PRD-0041)* — **Out-of-Band Memory Consolidation ("Dreaming").** Governed, propose-only batch consolidation of cross-session transcripts into an evidence-backed proposed knowledge-tree diff a human promotes — the out-of-band complement to in-band PRD-0004 distillation.
- [OPP-0059](OPP-0059-inter-agent-control-loop-cross-vendor-bus.md) *(accepted 2026-08-30; field report; un-defers OPP-0046 bus; promoted via PRD-0039)* —
  **Live inter-agent control-loop & cross-vendor bus.** OPP-0046 governs the static
  ...
  `validate-agent-bus.sh` landed as OPP-0061 (accepted 2026-09-01).
- [OPP-0060](OPP-0060-cli-transport-adapter.md) *(accepted 2026-08-31; shipped #198/#201; field-reported with a live-validated prototype; fills PRD-0039's deferred non-native-adapter slot)* — **Non-native (local-CLI) transport adapter.** A headless CLI (Codex/Copilot/Grok) participates in the bus over the FROZEN store + contracts — no contract change: a per-dispatch runner (poll → ack → apply `tier_ceiling` → invoke the CLI with the task as data → done/block; Tier ≥4 auto-blocks) plus a `build_argv` defense-in-depth second gate. Grok live round-trip proven. Encodes the copilot `--allow-all-tools` read-only gap → declared via `capabilities`.
- [OPP-0061](OPP-0061-validate-agent-bus-linter.md) *(accepted 2026-09-01; the deferred bus linter, OPP-0059's "validate-agent-bus.sh" record — implemented on acceptance)* — **`validate-agent-bus.sh` contract-conformance linter.** A `--scan-file` checker over a bus-transcript JSONL: envelope shape, per-type payload keys, the correlation-id rule, lifecycle validity per id, `tier_ceiling` sanity, and `sync` shape — a direct transcription of `control-loop-contract.md`. Bus messages are runtime/gitignored so it is shaped like `validate-observation-hygiene`: operator-run against a live `.coordination/bus/` session + CI-tested against committed valid/invalid fixtures. Moves the schema/lifecycle/`tier_ceiling` checks §10 Asserted-only → Half-enforced and **sets the `sync` size/rate caps** the contract explicitly deferred to it. Ratification questions: cap values + WARN-vs-ENFORCE, lifecycle strictness, transcript-source shape.
- [OPP-0062](OPP-0062-a2a-agent-card-descriptor-layer.md) *(proposed 2026-09-02; field-reported + independently re-derived; parent PRD-0039/OPP-0060)* — **A2A Agent-Card descriptor layer + `tier_ceiling` extension.** Align the bus's ad-hoc per-agent descriptors to the A2A Agent Card (identity / skills / capabilities / securitySchemes) WITHOUT changing transport or the control schema — because A2A binds only to HTTP/gRPC (no local/stdio/file binding, so the file bus stays) and `tier_ceiling` has no standard equivalent (the Layer-4 governance gap, arXiv 2606.31498), modelled as a named Agent Card extension. Annotate the 7 message types to A2A task states (no wire change); `sync` has no A2A equivalent. External-spec claims held **pending primary-source confirmation**; the A2A v1.0 date is not cited normatively.
- [OPP-0063](OPP-0063-consumer-upgrade-safety.md) *(proposed 2026-09-16; field-reported across four consumer-repo upgrades + independently re-derived against the real validators; parent OPP-0025)* — **Consumer-upgrade safety: compat affordances + layout-agnostic validators.** One root cause — the harness assumes its dogfooded layout (root `platform/`/`SUMMARY.md`/`docs/`) is the consumer's, and a breaking rename shipped with no compat affordance. Five findings, triaged defect-vs-posture: **F2 (defect)** `data/relational-postgres`→`relational-sql` hard cutover with no alias crashes 9/14 validators for a consumer selecting the old id → ship a one-release deprecation alias + BREAKING note; **F1** `validate-list-completeness` hard-codes root `SUMMARY.md`, rejecting a GitBook-from-`docs/` TOC → honor `.gitbook.yaml root:` or accept either location; **F3** `validate-catalog-counts` misreads a `.harness/`-mount consumer as count-drift → auto-detect `platform/` or exit 2; **F4** `validate-placeholders` ISO-date arm false-positives on legit date-format docs → narrow the arm / document `.placeholder-ignore`; **F5** the existing `consumer-upgrade-runbook.md` doesn't cover these break classes → augment it. A green maintainer chain doesn't predict a consumer's.
- [OPP-0064](OPP-0064-antigravity-headless-adapter.md) *(proposed 2026-09-16; field-reported + web-verified against AntiGravity's own headless docs + the `google-antigravity/antigravity-cli` issue tracker; parent PRD-0039/OPP-0060)* — **AntiGravity headless (`agy`) CLI: correct the stale "no headless CLI" claim + a non-native adapter.** The agent-coordination docs use AntiGravity as the canonical "no headless CLI / IDE-only" example in ≥5 places — now **factually false**: `agy -p "<prompt>"` is a documented headless mode. **F1 (confirmed)** correct the stale claim (§10-inward, like #207); **F2** add a third permission archetype (sandbox-tiered / forced-broad / prompt-collapsing) to the constraint-1 taxonomy; **F3** judge headless success on OUTPUT not exit code (docs say fail→non-zero, but issue #408 shows exit-0-empty when piped, #318 shows hang); **F4 (design-only, deferred)** an `agy` adapter branch in `cli_invokers.py` — but the peer's `--print=<task>` form is **contradicted** by the vendor docs (space-separated `-p "<prompt>"`), and `--mode plan`/`--add-dir` are undocumented, so the argv must be resolved against a live `agy` run before implementation. Verify-against-the-primary discipline, OPP-0060/0062 precedent.
- [OPP-0065](OPP-0065-govern-ruflo-runtime.md) *(proposed 2026-09-23; analyzed via 3 parallel research agents + web-verified against ruflo's repo/README/LICENSE/package.json; parent OPP-0060/OPP-0056)* — **Govern the ruvnet/claude-flow (`ruflo`) runtime as a non-native adapter + positioning artifact.** `ruflo` is `claude-flow` rebranded — a runtime agent meta-harness (swarms, vector memory, federation, ~210–314 MCP tools; MIT), the LangGraph/CrewAI genre, the opposite of auto-harness's governance genre. Its "trust/audit" are algorithmic/security, not human-ratified — so it's a governed TARGET, not a competitor or a dependency. **Move 1:** govern it via the existing non-native CLI adapter (its headless `ruflo` CLI) + ACP bridge (its MCP server) under trust tiers — near-zero new primitives, dogfoods the Layer-4 thesis. **Move 2:** a positioning artifact (analog of OPP-0056). **Out of scope:** do NOT vendor/build-on ruflo (Node+WASM+web-server+LLM-keys inverts the dependency-free posture) or adopt its ADR/metaharness plugins (weaker, algorithmic duplication of auto-harness's core). Pin a version, never track `main` (OPP-0063 discipline).
