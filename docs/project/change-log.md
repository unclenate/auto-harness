...
the scan**, per maintainer decision. Investigation found a private org repo-path (an `<org>/<repo>` form),
plus a commit hash and an org-named record id, on ~50 lines across 12 tracked docs — a real leak #206
missed — while the project codename and the public roadmap codenames (`YouBase`/`OpenEMR`/`Tula`) also
appear as legitimate design context (roadmap versions, OPP motivation) across hundreds of lines. Since the
repo is public + forked + release-tagged (history can't be recalled — working-tree redaction is cosmetic,
per #206's own reasoning), the response is split. **Existing text:** genericized only the sharpest
identifying tokens — dropped the `<org>/` org-path prefix (→ the bare project codename, kept), replaced the
commit hash with `redacted`, and the org-named record id with the org-free `CANON-000` — across ADR-0012,
diagrams, OPP-0007, OPP-0010, OPP-0036, candidates, change-log, PRD-0006, PRD-0007, roadmap,
shared-observations, and the audit sweep. Left the lower-signal codename narrative references and the legit
denylist-documentation mentions. **Prevention:** `validate-knowledge-redaction.sh` now runs a second,
**broadened WARN scan** over the design/publication corpus (ADRs, OPPs, PRDs, diagrams, roadmap) for a
**sensitive-class** subset (the org-path prefix + private project names + gitignored names) — NOT the public
codenames, which legitimately appear there — to catch a *future* re-introduction; the per-file diff walk is
refactored into a shared `scan_added_lines` function used by both the strict (2-file, full denylist) and
broadened passes, so they cannot drift. Diff-based (never re-flags the existing corpus) + WARN (surfaces,
never fails CI). Distillation (once a leak is public, redaction is cosmetic — genericize the sharpest
tokens, invest in scoped prevention) in `shared-observations.md`.
...
production fleet topology had been copied out of parked (correctly untracked) briefs into tracked governance
docs — OPP-0044, ADR-0019, ADR-0012, ADR-0015, OPP-0046, several superpowers plans/specs, the DT overlay
docs, and one shared-observations context line. Redacted all of them to neutral descriptors (e.g. a partner
geospatial-twin project, a municipal / civic planning platform, an IaC fleet repo, the production host
fleet), preserving readability and the governance content — the lessons were always the reusable primitives,
not the client identities. A collision was handled with care: the generic bootstrap "Hermes" example is a
different token from the fleet host of the same name and was left untouched. Recurrence prevention: added a
**gitignored local denylist** mechanism (`.knowledge-redaction-denylist`, OPP-0048 mechanism 2) to
...
state would be a separate ADR-gated decision, deliberately not taken here.

---

## OPP-0013 partial promotion — healthcare FHIR + SMART-on-FHIR wedge

OPP-0013 moves from `proposed` to `accepted` (partial promotion): the FHIR + SMART-on-FHIR
sub-modules are promoted to a v1 wedge via PRD-0017. The other ten sub-modules remain
proposed within this OPP. Rationale and design context: PRD-0017 +
...
| ---- | ---- | ------ | ------ | ----- | ------- |
| 2026-05-28 | Scope | Added `mcp-server-typescript-oss.yaml` starter composition — sibling to `mcp-server-typescript.yaml` covering the maintained-OSS posture: `delivery/self-hosted-oss` (npm provenance, SECURITY.md, version-pin discipline) instead of `delivery/prototype`, `management/project-standard + knowledge-capture` instead of `management/interview-driven`, and `agents/claude-code` alongside `agents/base`. Catalog rows added to `platform/compositions/README.md`, root `README.md`, and `SUMMARY.md` per the list-completeness validator. No module or template changes — consumes the existing catalog. | External consumers forking and republishing an MCP server (toast-mcp-2026-complete as first witness) need npm-provenance, change-log, dependency-log, and distilled-learning discipline out of the box. The prototype composition doesn't supply those, and adding the OSS posture as a sibling composition avoids overloading `mcp-server-typescript.yaml` with conditional comments. | @unclenate | n/a |
