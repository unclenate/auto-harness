...
| `validate-list-completeness.sh` | `[<project-root>]` | For every ADR / PRD / OPP / composition / template subdirectory / profile module / agent module on disk, asserts the matching row exists in its canonical index file (docs/README.md, candidates.md, compositions/README.md, templates/README.md, SUMMARY.md). ADR/PRD/OPP additionally must have a `SUMMARY.md` nav row, anchored on the record's link-target path (not the bare id, which SUMMARY descriptions cite in prose) — closes the list-completeness drift class (SUMMARY nav per OPP-0055) |
...
| `validate-trust-tier.sh` | `[<manifest>] [<project-root>]` | For each active module, validates the optional `tier.declared` field (range 0–5; rationale required for ≥3); computes the inferred tier from `sensitivePaths` regex patterns against representative production-shape sample paths (highest match wins); asserts declared ≥ inferred. For agent modules, `maxTier` is an upper **ceiling** on autonomous reach (caps, never grants); the validator asserts it is in range (0–5) and not below the agent's own declared tier, and emits an informational note (not a failure) when the ceiling is below the manifest's highest non-agent tier — that work defers to the human gate (ADR-0020; corrects the earlier `maxTier ≥ workload` inversion). Cross-cutting: declared tier 5 requires `project.criticality` ∈ {high, critical}, unless `project.maturity == platform`. PRD-0006 / ADR-0017 Wave 5.1 / ADR-0020 |
...
**`test/test_validators_integration.rb`** — hard-coded tests + 81 dynamically
generated `--help` / `-h` coverage tests (3 per validator × 27 validators) that
