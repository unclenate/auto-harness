...
2. **In-Session Hooks (Active Scaffolding):** The `distillation-prompt.sh` Stop hook runs before commits are finalized. It does not merely remind — per PRD-0035 it **scaffolds**: when the branch carries distillation-worthy work with no satisfier yet, it emits an ADR-0002-shaped *inert stub* (six fields, `Context` and the attribution date pre-filled from git context, judgement fields left as fill-tokens) and persists a copy to a gitignored `.claude/drafts/` file. The agent fills a correct skeleton instead of recalling the schema — and the stub is inert by construction (its placeholder `Confidence`/`Severity` fail `validate-observation-hygiene`, and its strict-uppercase tokens fail `validate-placeholders`), so it can never satisfy the very trace it scaffolds until completed. This is the ergonomics complement to the shape-enforcement gate in point 3.
