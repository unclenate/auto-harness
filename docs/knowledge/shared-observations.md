...
**Last Updated:** 2026-06-07 *(Greenfield conservatism (PRD-0021): appended the observation that an evidence-based rule needs an evidence model that fits the lifecycle stage — the onboarding skill's "conservative module selection" was brownfield-shaped (evidence = files present) and so failed open on greenfield, treating a verbal description as evidence; the fix names greenfield as a mode and inverts the default. Satisfies the PRD-0004 distillation rule fired by the OPP-0042 status flip. Prior: 2026-06-06 bootstrap hardening (friction-vs-safety false-tradeoff observation, PRD-0020); 2026-06-05 onboarding safety + install prerequisites; 2026-06-04 AEC wedge Phase 2; the OPP-0038 attribution-boundary observation; and consumer-adoption observations from the fork-held-consumer pin-bump session.)*
