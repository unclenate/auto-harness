...
| `management/agent-coordination` | Two or more agents must coordinate *live* (dispatch work between sessions, broadcast shared state, supervise for stalls) and you want a vendor-neutral control-loop contract + `tier_ceiling` caps-never-grants governing the channel | `docs/coordination/control-loop-contract.md`; `docs/coordination/adapter-contract.md` |
| `management/memory-consolidation` | You run many agent sessions and want cross-session patterns mined *out-of-band* into evidence-backed PROPOSALS a human promotes (propose-only — Tier 2, never merges); the second layer complementing in-band PRD-0004 distillation | `docs/knowledge/dreaming-policy.yaml` |
