...
- **Validator chain** — twenty-seven shell scripts you run locally or in CI that enforce all of the above
...
flowchart TD
    ...
    subgraph ENFORCE["Enforcement (CI)"]
        Validators["<b>Validators</b><br/>27 scripts"]
...
Twenty-seven validators, each targeting a specific governance layer:
...
| `validate-list-completeness.sh` | Every ADR / PRD / OPP / composition / template subdirectory / profile module / agent module on disk is referenced by its canonical index file — and every ADR / PRD / OPP additionally has its `SUMMARY.md` nav row (path-anchored, so a prose id mention can't mask a missing row) — closes the list-completeness drift class |
...
| `validate-trust-tier.sh` | Each active module's declared trust tier (0–5) is coherent with its inferred tier (from `sensitivePaths`); agent `maxTier` ceilings respect the active manifest's highest non-agent tier — **PR-boundary declaration-coherence** for safety claims 10–11. It checks the declared numbers agree at PR time; it does **not** prevent a running session from self-elevating (that stays honor-code, per `trust-model.md`), and a module that omits `maxTier` is not ceiling-checked |
...
| `validate-observation-hygiene.sh` | Diff-based ADR-0002 shape linter — gated on `management/knowledge-capture` (dogfooded on the harness); lints each observation added to `docs/knowledge/shared-observations.md` for six fields + the `Confidence`/`Severity` enums (Severity enforce-as-locked) + an ISO-dated attribution, grandfathering history — the knowledge-ledger *structured-agent-ledger gate* (PRD-0034 / OPP-0053) |
...
| `validate-foundry-target.sh` | Opt-in: when `architectures/ai-foundry-target` is active, validates `docs/architecture/foundry-targets.md` declares ≥1 foundry from the enum, each with a `live`/`roadmap` status — cluster v2 enforcement phase 2 (PRD-0032) |
| `validate-model-routing.sh` | Opt-in: when `architectures/intelligent-model-routing` is active, validates `docs/architecture/model-routing.md` declares ≥1 task→model route (providers free-form) — cluster v2 enforcement phase 3 (PRD-0032) |
| `validate-agent-defense-in-depth.sh` | Opt-in: when `architectures/agent-defense-in-depth` is active, validates `docs/security/agent-defense-in-depth.md` names all four defense-in-depth patterns — cluster v2 enforcement phase 4 (PRD-0032) |
