...
**Last Updated:** 2026-06-06 *(Bootstrap hardening (PRD-0020): appended the observation that the friction-vs-safety tradeoff is mostly a false one for boundary guards — a precondition that fires only on the mistake path costs the happy path nothing, so it can be hard-fail-by-default with a narrow escape hatch; the real friction/risk lives in happy-path checks and environment-altering actions (auto-install), which is why dependency auto-install was made opt-in. Satisfies the PRD-0004 distillation rule fired by the OPP-0040/OPP-0041 status flips. Prior: 2026-06-05 onboarding safety + install prerequisites (two observations behind OPP-0040/0041/0042); 2026-06-04 AEC wedge Phase 2; the OPP-0038 attribution-boundary observation; and consumer-adoption observations from the fork-held-consumer pin-bump session.)*
