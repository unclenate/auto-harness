...
| [0038](requirements/PRD-0038-acp-governance-proxy-reference.md) | ACP Governance Proxy — Reference Implementation (`agents/acp/reference-proxy`) | Accepted | [OPP-0056](opportunities/OPP-0056-agent-client-protocol-governance-bridge.md) |
| [0039](requirements/PRD-0039-agent-coordination-control-loop.md) | Inter-Agent Control-Loop & Cross-Vendor Bus (`management/agent-coordination`) | Accepted | [OPP-0059](opportunities/OPP-0059-inter-agent-control-loop-cross-vendor-bus.md) |
| [0040](requirements/PRD-0040-acp-audit-bridge.md) | ACP Audit → Knowledge-Capture Bridge, with a Tamper-Evident Audit Record (`agents/acp` follow-on) | Accepted | [OPP-0057](opportunities/OPP-0057-acp-audit-knowledge-capture-bridge.md) |
| [0041](requirements/PRD-0041-out-of-band-memory-consolidation-dreaming.md) | Out-of-Band Memory Consolidation ("Dreaming") (`management/memory-consolidation`) | Accepted | [OPP-0058](opportunities/OPP-0058-out-of-band-memory-consolidation-dreaming.md) |
...
| [0060](opportunities/OPP-0060-cli-transport-adapter.md) | Non-Native (Local-CLI) Transport Adapter — a headless CLI (Codex/Copilot/Grok) participates in the bus over the frozen store/contracts; per-dispatch runner + `build_argv` second gate; fills PRD-0039's deferred non-native-adapter slot; shipped #198/#201 | accepted |
...
| [0062](opportunities/OPP-0062-a2a-agent-card-descriptor-layer.md) | A2A Agent-Card Descriptor Layer + `tier_ceiling` Extension — align the bus's per-agent descriptors to the A2A Agent Card without a transport/schema change (A2A is HTTP/gRPC-only; `tier_ceiling` is unmappable → a named extension); annotate the 7 message types to A2A task states; external-spec claims pending primary confirmation | proposed |
| [0063](opportunities/OPP-0063-consumer-upgrade-safety.md) | Consumer-Upgrade Safety: Compat Affordances + Layout-Agnostic Validators — one root cause (the harness assumes its dogfooded root `platform/`/`SUMMARY.md`/`docs/` layout is the consumer's; a breaking rename shipped with no affordance); F2 (defect) `relational-postgres`→`relational-sql` hard cutover crashes 9/14 validators → deprecation alias + BREAKING note; F1 root-hardcoded `SUMMARY.md` rejects GitBook-from-`docs/`; F3 catalog-counts misreads a `.harness/` mount as drift; F4 `YYYY-MM-DD` placeholder over-match; F5 augment the existing upgrade runbook; field-reported + re-derived on disk | proposed |
| [0064](opportunities/OPP-0064-antigravity-headless-adapter.md) | AntiGravity Headless (`agy`) CLI — Correct the Stale "No Headless CLI" Claim + Non-Native Adapter — the agent-coordination docs cite AntiGravity as the canonical "no headless CLI / IDE-only" example in ≥5 places, now factually false (`agy -p` is a documented headless mode); F1 correct the claim (§10-inward), F2 add a prompt-collapsing permission archetype, F3 judge on output not exit code (issue #408/#318), F4 an `agy` adapter deferred — the peer's `--print=` argv is contradicted by the vendor docs, resolve against a live run; field-reported + web-verified against AntiGravity's own docs | proposed |
| [0065](opportunities/OPP-0065-govern-ruflo-runtime.md) | Govern the ruvnet/claude-flow (`ruflo`) Runtime as a Non-Native Adapter + Positioning Artifact — `ruflo` is `claude-flow` rebranded, a runtime agent meta-harness (LangGraph/CrewAI genre; MIT); its trust/audit are algorithmic, not human-ratified, so it's a governed TARGET not a competitor/dependency. Move 1: govern it via the existing non-native CLI adapter + ACP bridge under trust tiers (near-zero new primitives); Move 2: a positioning artifact (OPP-0056 analog). Out of scope: don't vendor it (inverts the dependency-free posture) or adopt its ADR/metaharness plugins (weaker duplication) | proposed |
