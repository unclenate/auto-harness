...
- **Implication:** When harvesting a coordination pattern, locate the authority-of-record and the enforcement mechanism *separately* — do not force both into one repo. And treat a concrete field-defect pair as the promotion bar: OPP-0052's two Enforced rules (canonical shared `taskId`; mandatory decorrelated-provider coverage) are each the fix for a specific observed failure (verdict label-swap; core-only adjudication) in a real federated-review cycle — "field-proven before authored" is a stronger Origin/Evidence basis than a speculative framework, and it pre-classifies Enforced-vs-asserted because each rule already has a failure it must catch.
