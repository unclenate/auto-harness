...
**Last Updated:** 2026-06-04 *(AEC wedge Phase 2: PRD-0019 implementation; appended the second-domain-instance observation (compound forcing artifact + first domain × cross-cutting composition), satisfying the PRD-0004 distillation rule fired by the three new `domains/aec-*/module.yaml` additions. Also merged: the adopter-artifact/host-IP attribution-boundary observation behind OPP-0038 (git first-authorship as the computable partition signal), and two consumer-adoption observations from the fork-held-consumer pin-bump session — (1) a submodule pin bump silently widens the recommend-vs-adopt gap between the CI template and the consumer's invocation list (governance-relevant; OPP candidate) and (2) forked consumers hit a wrong-base default in `gh pr create` (informational; onboarding note).)*
