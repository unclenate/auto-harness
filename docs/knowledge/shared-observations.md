...
**Last Updated:** 2026-06-05 *(Onboarding safety + install prerequisites: appended two observations this cycle — (1) the harness validates a consumer's file *content* but never its *location / repository identity*, grounded in a live incident where a contextless greenfield consumer was bootstrapped inside the platform working tree and committed into the platform's own history, caught only when a routine commit nearly pushed a private site into the public platform repo (behind OPP-0041 containment safety + OPP-0042 greenfield conservatism); and (2) a declared prerequisite that lives only in reference docs and is preflighted asymmetrically is, in effect, undeclared at the moment it matters (behind OPP-0040 cross-platform install prerequisites). Both satisfy the PRD-0004 distillation rule. Prior: 2026-06-04 AEC wedge Phase 2 — second-domain-instance observation (compound forcing artifact + first domain × cross-cutting composition); the adopter-artifact/host-IP attribution-boundary observation behind OPP-0038; and two consumer-adoption observations from the fork-held-consumer pin-bump session.)*
