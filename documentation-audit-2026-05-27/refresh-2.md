...
|---|---|---|
| **`platform/profiles/delivery/managed-fleet/`** | New 5th delivery posture for teams operating live host fleets (Ansible / Terraform / Puppet IaC). Sits between `internal-platform` and `production-saas`. | Fills a real catalog gap (live-host operations) surfaced by `an IaC fleet repo` brownfield onboarding. Brings 3 new templates: fleet-inventory, change-control, config-rollback. |
